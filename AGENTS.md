...
**Where to start reading:** [`docs/superpowers/specs/README.md`](docs/superpowers/specs/README.md) for module behavior (source of truth for every surface above) · [`docs/agent-navigation.md`](docs/agent-navigation.md) for the engine module map and CR lookup (`docs/CR_INDEX.md`, regenerate with `just engine-cr-index`) · [`docs/client-canvas-map.md`](docs/client-canvas-map.md) for the canvas board (paint vs hits vs flights vs DOM overlays).
...
- **Client interaction: assert outcomes, not only presence.** When changing pointer, keyboard, hover, drag, Mount hosts, lobby/host flow, or BFF env defaults, add or extend a unit/Scene test for the user-visible result (pin set, tile hidden, selected deck matches, art URL swapped, default URL works). Do not frame tests as migration/"parity" checks — name the product behavior. See [`client-interaction-test-policy`](docs/superpowers/specs/2026-07-22-client-interaction-test-policy-design.md).
