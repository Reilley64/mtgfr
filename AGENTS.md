...
- **Server:** Axum + SSE (server→client state) + POST (client→server intents). Live games are in-memory only (ADR 0021); Postgres `mtgfr` holds users, sessions, decks (ADR 0010). Pre-game lobby + `table_routes` live in SolidStart on Postgres `mtgfr_web` (Drizzle). BFF routes in-game by table id → pod DNS (ADR 0030). Server-side per-player visibility filtering is a hard rule (hands/libraries are private).
