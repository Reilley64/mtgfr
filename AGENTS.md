...
- **Client state is Effect-first Foldkit.** Async work — wire calls, streams, polling — stays in Effect services/streams at runtime boundaries; Foldkit `Model`/`update`/`view` owns UI state and dispatches messages. Keep `effect` and `@effect/*` packages pinned to the same exact beta, and `@foldkit/ui` pinned exactly in lockstep with `foldkit` and `effect` — `@foldkit/ui@0.132.0` peer-requires `effect 4.0.0-beta.101` and `foldkit ^0`; `0.133.0` requires `beta.102`, so take it only when the whole `effect` / `@effect/*` set moves together. Styled components live in `client/app/domain/ui/` — see [ui-component-layer](docs/superpowers/specs/2026-07-28-ui-component-layer.md). BFF Drizzle is Effect-native via `drizzle-orm/effect-postgres` + `@effect/sql-pg` (no pg-proxy).
