...
describe("lookupCardsByIds", () => {
  ...
  it("chunks long id lists so each request stays under LOOKUP_CHUNK", async () => {
    ...
    vi.mocked(client.lookupCards).mockImplementation(({ params }) =>
      Effect.succeed(params.ids.map((id) => ({ id }) as never)),
    );
    const result = await Effect.runPromise(lookupCardsByIds(ids));
    expect(client.lookupCards).toHaveBeenCalledTimes(2);
    expect(vi.mocked(client.lookupCards).mock.calls[0]?.[0].params.ids).toHaveLength(LOOKUP_CHUNK);
    expect(vi.mocked(client.lookupCards).mock.calls[1]?.[0].params.ids).toHaveLength(3);
