    ...
    <For each={splitOracleText(props.text)}>
      {(part) =>
        part.kind === "text" ? (
          ...
          <i class={cn("ms", "ms-cost", "ms-oracle", `ms-${part.ms}`)} aria-label={`{${part.code}}`} />
        )
      }
    </For>
  );
          ...
          <div class="relative flex shrink-0 flex-col items-center">
            ...
            <Show when={canGoBack() || hasBack()}>
              <div class="absolute top-0 right-0 left-0 flex -translate-y-full items-center justify-between pb-2">
                <Show when={canGoBack()}>
                  <Button type="button" onClick={goBack} variant="link" class="flex items-center gap-1 text-label text-white/50 no-underline hover:text-white/80 hover:underline">
                    <svg
                      aria-hidden="true"
                      viewBox="0 0 16 16"
                ...
                <Show when={hasBack()}>
                  <Button type="button" onClick={() => setFace((f) => (f === "front" ? "back" : "front"))} variant="game-quiet" class="ml-auto">
                    Flip
                  </Button>
                </Show>
              </div>
