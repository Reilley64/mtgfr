...
export function combatCoachText(input: CombatCoachInput): string | null {
  ...
  if (input.step === DECLARE_ATTACKERS && input.activePlayer === input.viewer && !attackDone) {
    return "Drag onto an opponent to attack · Shift drag a stack to send all · Click attacker to cancel · Confirm with Attack";
  }
  ...
  if (input.step === DECLARE_BLOCKERS && input.attackingViewer && !blockDone) {
    return "Drag onto an attacker to block · Shift drag a stack to block with all · Click blocker to cancel · Confirm with Block";
  }
  return null;
}
