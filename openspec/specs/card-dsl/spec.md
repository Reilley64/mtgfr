...
Numeric magnitudes SHALL use a polymorphic amount type that includes fixed values, cast X, board- and resolution-derived counts, and compositional forms for arithmetic and conditional substitution. Threshold comparisons that compare two magnitudes SHALL use a generic compare condition with inclusive operators rather than one-off named threshold variants whenever a scalar comparison suffices. A condition whose subject is implicitly "you" SHALL be re-pointed at another seat by a wrapping condition that supplies the perspective, rather than by a parallel per-seat copy of the inner condition. Effects that land on seats SHALL take a player-set parameter naming the recipient set (you, targeted seats, each opponent/player, trigger-filled seats, and related sets) instead of proliferating per-recipient modes. Multi-seat simultaneous changes SHALL resolve seats in turn order; choice-bearing edicts and multi-seat library searches SHALL proceed in APNAP order.
