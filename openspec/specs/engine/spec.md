...
Every draw taken while the game is in progress MUST route through one draw path that can replace the draw, including with a replacement that raises a pending choice. Because a replacement may pause, that path MUST park the remaining draws of a multi-draw batch — across multiple players where the effect draws for more than one seat — and resume them once the choice is answered, rather than dropping or pre-minting them. A replacement MUST NOT apply to the draw it itself grants (CR 614.5). The engine MUST track per player how many draws they have taken in their own draw step, counting draws attempted from an empty library, so an effect can exempt the first such draw; that count MUST NOT assume the turn-based draw is the one that fills it, since a skipped draw step leaves a later draw as the player's first. The pre-game mulligan deal and land smoothing are deliberately outside this path: no battlefield exists to hold a replacement effect at that point.

A player who leaves the game while a parked batch waits on their answer forfeits only their own remaining draws: the batch MUST resume for the seats behind them rather than stranding on the choice that left with them, and MUST still deliver whatever the batch owed its caller. Where a replacement substitutes another action for the draw, that action is caused by the replacement's own source, not by the effect whose draw it replaced — so a discard substituted for a draw MUST be attributed to the permanent that replaced it, and MUST NOT leave the replaced effect's controller stamped on the discard.

#### Scenario: A replacement pauses a multi-draw batch
- **WHEN** a spell instructs a player to draw several cards and a replacement effect raises a choice on the first of them
- **THEN** the remaining draws are held and resume after the choice is answered, and any draw the replacement itself grants is not replaced again

#### Scenario: A player concedes on a parked draw batch
- **WHEN** a multi-seat draw batch is paused on one seat's replacement choice and that seat concedes
- **THEN** the remaining seats still take their draws and the batch's follow-up still runs

#### Scenario: A substituted discard is attributed to the replacing permanent
- **WHEN** a replacement effect turns a draw into a discard and the discarded card watches for an opponent's spell or ability causing it to be discarded
- **THEN** the watch reads the controller of the permanent that replaced the draw, not the controller of the effect that asked for it
