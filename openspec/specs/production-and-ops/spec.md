...
The checked-in `mtgfr-debug` CLI SHALL expose all six service workflows as `tables`, `inspect <table> [--out <file>]`, `mutate <protobuf-json-file> [--out <file>]`, `checkpoint <table> <name>`, `restore <table> <name>`, and `journal <table>`, with command-specific guards and output flags, a global endpoint flag, `MTGFR_DEBUG_ENDPOINT` fallback, and the local debug listener as its default. An optional mutation table-sequence flag SHALL agree with any value already present in the protobuf JSON. The CLI SHALL emit protobuf JSON, render stable typed failures with operation indexes, write requested output atomically with private default permissions, and SHALL NOT log request bodies automatically. The corresponding `debug-tables`, `debug-inspect`, `debug-mutate`, `debug-checkpoint`, `debug-restore`, and `debug-journal` recipes SHALL preserve every argument as a literal argv element without shell evaluation.
...
The production projection and self-contained stream publication shape SHALL accept a transient exact-object Printing UUID map without adding it to serialized `Game` state or to the wire payload. Live ownership of that map SHALL exist only inside the debug-assertion-only table transaction state; release tables and every release opening, delta, and hold publication SHALL supply an empty map. Debug checkpoints SHALL snapshot and restore the map with the game and logical chrome so replacement projection and publication use one atomic presentation state. Carrying the always-present inert publication input SHALL NOT expose a print-override mutation, debug service, or browser debug route; any future debug transaction that supplies a nonempty map remains subject to the debug-only service and release-isolation requirements.
