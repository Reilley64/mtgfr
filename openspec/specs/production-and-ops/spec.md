...
Release compilation SHALL omit debug protobuf exposure, all six service implementations and registrations, engine/server raw-editor hooks, checkpoint and journal storage, and identifying service, route, server-implementation-marker, and cfg-gated stack-editor-marker strings from the production server. The stack-editor marker SHALL be owned and retained by the debug-only constructor/editor module rather than by profile-stable stack enums. A release build of the checked-in CLI target SHALL be only a non-operational stub, and the production image SHALL contain the gated release server rather than the CLI. A build script MAY generate separate debug protobuf output as an intermediate file, but release compilation SHALL leave it unlinked and unregistered; the production descriptor SHALL omit the `mtgfr.debug.v1` package, browser wire generation SHALL exclude that package and `DebugService`, and production service authentication and per-viewer visibility requirements SHALL remain unchanged.

CI SHALL invoke the shared release-isolation gate to build and scan the production-equivalent target (`server` package, release profile, `server` binary), proving that the debug service FQN, RPC path prefix, both implementation markers, and cfg-gated stack editor are absent from its bytes and that exactly one production descriptor contains no debug package. The production Docker build SHALL invoke the same shared gate and copy the exact scanned release server into the runtime image. Browser generation exclusion, release all-target compilation of the CLI stub, and CLI recipe argument safety SHALL remain verification gates.
...
- **THEN** no operational debug service, debug package, debug RPC path, server or stack-editor implementation marker, browser binding, public-ghost constructor, or production-runtime debug executable is present, while stable stack identities and the inert production projection shape remain available
