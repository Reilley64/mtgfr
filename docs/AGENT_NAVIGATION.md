...
2. Or run `rg 'CR 601.2c' crates/engine` for citation text.
3. For structural code shape (calls, matches, type uses), prefer `ast-grep` over text grep, e.g. `ast-grep run -l rust -p 'Game::submit($$$)' crates/engine`.
4. Read the module `//!` header for chapter ownership (coarse map below).
5. When behavior spans modules, follow [`pipeline.rs`](../crates/engine/src/pipeline.rs): SBA → trigger enqueue → APNAP placement → refresh actions.
...
| `characteristics` (+ cache) | Effective P/T/keywords; internal `PtLayer` 7b/7c for P/T; CR 614 slices; full CR 613 still deferred ([engine](../openspec/specs/engine/spec.md)) |
| `pending` | Targets / modes / assignment; CR 601.2c, CR 608 pauses (`raise`/`answer`/`forced`) |
| `query` | Meaningful actions ([engine](../openspec/specs/engine/spec.md)); CR 605 carve-outs |
...
| [`docs/fidelity/`](fidelity/) | Per-deck fidelity reports + active increments backlogs |
| [OpenSpec capabilities](../openspec/specs/) | Living requirements (source of truth) |
| [`CLIENT_CANVAS_MAP.md`](CLIENT_CANVAS_MAP.md) | Client board paint / hits / flights / overlays |
...
- No engine graph DB or embedding RAG — the corpus is small; `rg` + `ast-grep` + this index is enough.
