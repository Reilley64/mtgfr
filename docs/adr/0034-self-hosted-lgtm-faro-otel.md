...
- **Propagation rule:** Effect parent spans live in fiber Context. The gRPC client uses a **separate** `ManagedRuntime`, so context does not survive `runPromise` into outbound calls. Capture once per request via `grpcRequestEnv` (`{ sessionToken, traceparent }`) under `runTracedRequest`, then pass that bag into every gRPC call (`grpcClientFor` / `RpcEnv` / lobby helpers). Do not use Node AsyncLocalStorage or per-helper optional `traceparent` args.
