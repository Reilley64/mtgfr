...
- New behavior = new `Effect` variant + resolution behind [`Game::run`] (pause via
  `pending::ChoiceRequest` / transitional `begin_*`, or pure mint via private `execute_effect` /
  family helpers in `resolution/`) + Event `apply` + TOML authoring. Callers never bypass `run`
  for Effect→board.
