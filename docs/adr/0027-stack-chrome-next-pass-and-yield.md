...
- **Non-empty stack:** hide **Next**. While this seat holds priority and has a meaningful action, show **Resolve card** (one `pass_priority` — lets the top object resolve if priority passes around) and **Resolve stack** (arm one-shot stack yield for the rest of this stack). Space/Enter mirrors **Resolve card** in that window.
- **Resolve stack** (stack yield) is one-shot from chrome: press to arm, then the control disables until the server clears it when the stack empties. No "Cancel" on the bar.
- **Cannot act** on the stack (helpless / not your priority): hide Resolve card and the Resolve stack arm (unless already armed — then show disabled). Resolution uses hold + helpless dwell (0026) and server auto-pass (0007).
...
- Yield stays a per-stack opt-out (no in-chrome revoke); Resolve card stays a normal priority pass — two distinct intents/routes.
