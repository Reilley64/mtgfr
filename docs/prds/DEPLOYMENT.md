...
1. **Build:** `rust:1-bookworm` — install `protobuf-compiler`, copy `proto/` + crates, then `cargo build -p server --release --no-default-features` (drops test-only sqlite; cache mount on `target/`). tonic stubs land in Cargo `OUT_DIR` via `build.rs` (never committed).
