...
**Landed 2026-07-25:** non-spell battlefield-entry events now route their ETB counter placements through the same replacement reads as cast/land entry, so a permanent reanimated, blinked, searched, or otherwise put onto the battlefield from hand still gets its own `enters_with_counters` / vanishing counters and any live "creatures you control enter with additional counters" statics, all grown by the same counter-replacement registry (Hardened Scales, Benevolent Hydra, Ozolith, the Shattered Spire). Regression: `reanimated_enters_with_counters_apply_as_enters_replacements`.
