...
2. **Product language only** — Name tests for the desired product behavior. Do not frame them as migration or “Solid parity” checks.
...
| Alt-down pins card under cursor; Alt-up clears; hand aux preferred | `client/app/board/inspect-pile-concede.test.ts` | Done — rename leftover migration wording |
| Drag-play commit hides hand card + seeds flight; cancel restores | `client/app/board/hand-drag.test.ts` | Done — rename leftover migration wording |
...
| Lobby: selected deck matches pre-pick / `?deck=` among ≥2 decks (not silently `decks[0]`) | `client/app/shell/lobby/entry.test.ts` | Gap — add binding assertion |
| Lobby entry Bring presentation (text/card, no misleading select) | lobby entry + surfaces | Deferred to the separate lobby bring-deck UX brainstorm; until then the binding assertion above is the policy minimum |
...
5. **Lobby with a pre-picked deck** (`/play?deck=…`) — shown deck matches the pick (Bring text/card once that UX lands; until then select value matches).

## Out of scope

- New Playwright (or other) CI browser matrix
- Cold-env CI job that unsets `WEB_DATABASE_URL` on every PR
- Lobby Bring UX redesign (separate brainstorm / spec)
...
5. Unit test follow-ups in the same policy PR:
   - Rename leftover migration-framed strings in inspect / hand-drag tests to product wording.
   - Lobby binding test: ≥2 decks, `selectedDeckId` = non-first → `#lobby-deck` selected option (or Bring copy) matches that deck.
