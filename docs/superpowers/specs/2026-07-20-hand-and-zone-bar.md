...
- Hovering a bar tile elevates that tile's root above all other action-bar tiles (`[z-index:var(--hand-z)]` resting + `hover:[z-index:50]` on the slot; resting z is not inline). Discard-selected (and the same chrome for pending hand puts / face-down cast) raises and rings Llanowar but does not elevate z on its own; hover still brings a selected tile to the front. Legal unselected choices ring Island blue; non-choices stay off that target chrome.
...
- `hand.test.ts` locks hover elevate on `hand-tile-{id}` and asserts discard-selected does not add selection z elevate (hover:[z-index:50] remains for hover+selected).
