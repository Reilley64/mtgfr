...
4. **CombatDamage** — if any first-strike or double-strike creature is attacking or blocking, a first-strike sub-step runs first. Each sub-step is one turn-based action (CR 510.1): every attacker in that batch that is blocked by two or more creatures and has positive power raises `PendingChoice::AssignCombatDamage` first, and no damage in the batch is dealt until all of those divisions are answered. The division is read here, not at declare blockers, so it totals the power the attacker has in the damage step (CR 510.1a) — a rampage bonus or a pump cast in response to the blocks is assignable. The assignment order is the blocker declaration order (CR 509.2); the defending player answers instead of the attacking player when a banding blocker is involved (CR 702.22j).
