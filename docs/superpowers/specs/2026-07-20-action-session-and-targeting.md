...
- Local pre-submit graveyard-exile costs (`gyExilePick`) open the shared GY pile when every choice is in one graveyard (`gyExileCostPile` / `gy-exile-cost-aim`); pile cards with Island Blue rings emit `GyExileChosen`. One-click when `max <= 1`; exact `min === max` accumulates then auto-settles; `min < max` accumulates until Exile / Enter / Space (`GyExileConfirmed`). Mixed owners keep the modal `gy-exile-pick` grid.
