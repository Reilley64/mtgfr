...
- As a player attacking with a banding creature, I can group my staged attackers into an attacking band before I confirm the attack.
...
- Non-active players see the Until my turn rocker. The rocker is a `role="switch"` button whose chrome keys off its own `aria-checked` (`aria-checked:` on the root, `group-aria-checked/yield:` on track/thumb inside the named `group/yield` — the variant must name the group, since bare `group-aria-checked:` compiles to `:where(.group)[aria-checked] *` and never matches), so the accessible and visual state cannot drift.
...
- `board-band-panel` sits above the primary button while attackers are being declared, and only when at least one staged attacker has an effective `banding` / `bands_with:<quality>` keyword (`bandCandidates`); an ordinary attack never renders it. It offers one `board-band-chip-<id>` toggle per staged attacker — including attackers with no banding keyword of their own, since a "bands with other \[quality\]" band may include a plain creature that has the quality (CR 702.22c). Each chip carries `data-banded` and `aria-pressed`; Tailwind `data-[banded=true]:` variants own the look, so JS only sets attributes. The panel disappears once the declaration is confirmed or the engine reports `attackers_declared`.
- Confirming the attack submits the toggled members as `bands: [{ members }]` on the same `declare_attackers` intent; fewer than two surviving members submits `bands: []`. Band legality is the engine's (`Game::band_is_legal`) — an illegal grouping comes back as `Reject::IllegalDeclaration` in `board-reject` like any other bad declaration, and the client holds no banding rules.
...
- `priorityBarView` derives controls from current board model and `VisibleState`; server flags such as `yielded` and `turn_yielded` are authoritative. Illegal-action feedback (`board-reject`) carries `role="alert"` so a rejected action announces. The bar anchors above the hand bar via the `--b` variable (`HAND_BAR_H + 10px`) read by a `bottom-(--b)` utility, the same pattern as the log panel.
...
- Band staging is one band per declaration (`BoardModel.combatBand: number[]`, toggled by `CombatBandToggled`), cleared alongside the other combat staging on a real step change and on confirm. Two simultaneous bands would need four banding creatures attacking at once and is deliberately not built.
...
- Chrome tests cover Next, Resolve card, Resolve stack, End Turn, Until my turn, and staged / parked play-mode cancel controls. Turn-banner Scene tests assert `data-phase-state` / `data-your-turn` drive the segment and label chrome (exactly one `now` segment, mint variant token present); the yield rocker asserts `aria-checked` plus `group/yield` rather than parallel class state.
...
- Band-panel Scene tests (`html/surfaces.test.ts`) cover the panel opening with a chip per staged attacker once one can band, `data-banded` / `aria-pressed` on a banded chip, no panel at all for an attack with no banding creature, and the panel closing on confirm. Board Scene tests assert the *outcome*: toggling two chips and clicking the primary button submits `declare_attackers` with `bands: [{ members }]`, un-banding back to one member submits `bands: []`, and a goad-only declaration still submits `bands: []`. `bandCandidates` / `stagedBands` unit tests cover the gate and the pared-back / un-staged member cases.
