...
- As a player declaring attackers or blockers, I see a combat coach strip explaining drag-to-stage, shift-drag for every copy in a cluster, click-to-cancel, and Confirm.
...
- Non-active players see the Until my turn rocker. Both toggles are the same rocker: a `role="switch"` button whose chrome keys off its own `aria-checked` (`aria-checked:` on the root, `group-aria-checked/yield:` on track/thumb inside the named `group/yield` — the variant must name the group, since bare `group-aria-checked:` compiles to `:where(.group)[aria-checked] *` and never matches), so the accessible and visual state cannot drift. Only the armed hue differs: Until my turn arms in amber earth (`yielded`), End Turn in island blue with a blue glow — neither is priority gold.
- The two rockers share a second row under the primary/companion action row (`priority-bar-rockers`). They are standing toggles rather than per-window actions, so they never reshape the action row's silhouette; exactly one shows at a time (End Turn to the active seat, Until my turn to everyone else).
- Each rocker carries its name as a span to the left of the track, collapsed to zero width and revealed on hover or keyboard focus (Arena's hover labels). The name is constant — "End turn" / "Auto-pass until my turn" — and never flips to the reversed action while armed; the rocker's checked state carries that.
...
- During local declare-attackers / declare-blockers windows, `board-combat-coach` shows drag-to-stage, shift-drag-all-copies, and click-to-cancel copy (independent of hint dismissal): attack → opponent life orb (Shift drag to send all copies; click attacker to un-stage), block → attacker creature (Shift drag to block with all copies; click blocker to un-stage). The shift copy lives on the coach rather than `HintStrip` because it only applies while a declaration is open.
...
- Chrome tests cover Next, Resolve card, Resolve stack, End Turn, Until my turn, and staged / parked play-mode cancel controls. Turn-banner Scene tests assert `data-phase-state` / `data-your-turn` drive the segment and label chrome (exactly one `now` segment, mint variant token present); the yield rocker asserts `aria-checked` plus `group/yield` rather than parallel class state. End Turn asserts the rocker shape (`role="switch"`, `aria-checked` flipping with `turn_yielded`, island-blue armed class, accessible name), that it renders inside `priority-bar-rockers` while `board-primary` does not, and that the collapsed hover label is the rocker's first child.
