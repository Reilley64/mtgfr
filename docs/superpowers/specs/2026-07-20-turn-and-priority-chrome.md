...
- Non-active players see the Until my turn rocker. The rocker is a `role="switch"` button whose chrome keys off its own `aria-checked` (`aria-checked:` on the root, `group-aria-checked:` on track/thumb inside `group/yield`), so the accessible and visual state cannot drift.
