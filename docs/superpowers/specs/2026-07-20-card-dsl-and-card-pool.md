...
- **`CardDef` is `Clone`, not `Copy`; runtime uses `CardId`.** Once a card enters a `Game` it is interned via `intern_card_def` and referenced by `CardId` through shared `Arc<CardDef>` lookups. `Effect::Sequence` / `ChooseOne` / `Conditional` payload lists now deserialize into `Arc<[Effect]>`, so runtime contextualizers and pause/resume continuations can rebuild effect tails without leaking. `CardDef`'s own slice-like DSL fields still load through `intern` / `static_slice` / `static_effect` into `'static` data for now; nested back/adventure faces are already interned as `CardId`s during load.
