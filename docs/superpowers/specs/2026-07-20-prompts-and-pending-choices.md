...
- When every `assign_combat_damage` blocker is on the battlefield, blockers highlight for on-board clicks (`pendingDamageAssignOverlay`); a click moves 1 damage onto that blocker (`clickDamageAssign` — steals from the largest other share, or adds under trample power). Chrome shows docked `pending-damage-aim` (coach copy + assigned total + Assign); Enter or Space submits when `damageAssignReady`. Blockers with amount > 0 paint Priority Gold (`pickedObjects`) and a crimson assign-amount badge (`assignAmounts`). On-board mode hides per-blocker steppers (board clicks + Assign only); off-board blockers keep steppers in the same docked HUD.
- When every `divide_spell_damage` target is a battlefield permanent, targets highlight for on-board clicks (`pendingDivideSpellOverlay`); a click moves 1 damage onto that target (index-keyed divide draft via `clickDamageAssign`). Chrome shows docked `pending-divide-aim` (coach copy + assigned total + Assign); Enter or Space submits when the assignment totals match. Targets with amount > 0 paint Priority Gold and crimson assign-amount badges. Player or off-board targets keep steppers in the same docked HUD. On-board mode hides per-target steppers.
- When every `divide_counters` permanent is on the battlefield, targets highlight via `pendingDamageAssignOverlay`; a click moves 1 counter onto that permanent. Chrome shows docked `pending-divide-counters-aim` (coach copy + assigned total + Assign); Enter or Space submits when `damageAssignReady`. Amount badges reuse combat-assign paint. On-board mode hides per-permanent steppers; off-board targets keep steppers in the same docked HUD.
- `choose_card_name` shows docked `pending-card-name-aim` with an autofocused text field (`prompt-name-input`, placeholder “Card name”); Enter submits when the trimmed name is non-empty (same gate as the Name button). Typing ≥2 characters fires `SearchCardNames`; matching results render under `prompt-name-suggestions` (click fills the draft). Catalog suggestions assist only — free-typed / nonexistent names remain submittable.
- `search_library` uses docked `pending-library-aim` (not the center modal): autofocused `pick-card-filter` (“Filter by name…”), face dedupe by label, filtered grid inside `pick-card-scroll`, with title / filter / Choose+Fail-to-find pinned above the scroll strip. Other off-board card-pick kinds use the same docked chrome as `pending-card-pick-aim` (unfiltered grid + actions; no name filter).
- `choose_creature_type` shows docked `pending-creature-type-aim` with an autofocused `prompt-type-filter` (“Filter types…”) and a scrolling option strip (`prompt-type-scroll`); only matching `pending.options` are clickable. Free-typed types outside the option list are not allowed.
...
- One-click on-board `choose_target` prompts suppress the `pending-choice` card grid and show `pending-target-aim` label chrome. When `min === 0`, the same aim chrome keeps Decline. Multi-target on-board aim shows `pending-target-count` and Confirm instead of the card grid. Off-board card items use `pending-card-pick-aim`; seat-tagged `choose_target` lists that are not fully on-canvas (mixed with off-board objects) use docked `pending-player-pick-aim` player buttons.
- On-board battlefield sacrifice / proliferate / attach / phase-out / keep-tapped / legend-rule (`choose_legendary_keep`) card-picks reuse the same `pending-target-aim` chrome (one-click or Confirm accumulate). Legend rule titles read `Legend rule — choose which {name} to keep`.
...
- `may_return_from_graveyard` carries a `mandatory` flag (from the card's `mandatory` DSL axis). When `false` (optional "you may return" — Deadly Brew, Witch of the Moors) the prompt is declinable: choosing nothing is a legal answer. When `true` (mandatory "you return" — Witherbloom Command mode 0) the engine rejects a decline while a legal card exists, so the return is not skippable; the prompt only appears when at least one matching card is in the graveyard (no legal card means the effect does nothing and no prompt is raised). Surfacing `mandatory` in the projected DTO/wire so the client hides the Decline control is client catch-up; today the server authoritatively rejects an illegal decline.
...
- `may_yes_no` / `dance_exile_more` show docked `pending-yes-no-aim` with Yes / No. Trade Secrets uses the same generic `may_yes_no` prompt shape rather than a dedicated wire kind.
