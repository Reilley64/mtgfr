...
- Engine `discard` / `may_discard` with every item in hand suppress the card grid for `pending-discard-aim` hand-bar coach; `put_land_from_hand` / `put_creature_from_hand` / `put_from_hand_on_top` use `pending-hand-aim` (Decline stays for optional put-land/put-creature).
...
- Engine GY card-picks (`exile_from_graveyard`, `may_return_from_graveyard`, `shuffle_from_graveyard`, `choose_dredge`) with a shared pile show `pending-gy-aim` and the same selectable pile overlay instead of the modal card grid.
