...
- `choose_card_name` shows centered `pending-card-name-modal` around a `@foldkit/ui` Combobox (`CardNameCombobox`, id `prompt-name`): an autofocused text field (`prompt-name-input`, placeholder “Card name”) whose suggestion list (`prompt-name-suggestions`, rows `prompt-name-suggestion-{i}`) opens as you type. Every keystroke first emits `SearchCardNames.Interrupt`, which stops the search the previous keystroke started; its `CompletedCancelSearchCardNames` result then fires `SearchCardNames` for whatever the draft holds by then, as long as that is ≥2 characters (below that the suggestions clear and no search replaces the cancelled one). The replacement is sequenced through the cancellation rather than batched with it, because Commands in one batch have no execution order. `CardNameSuggestionsFetched` still lands only when its query matches the current draft: a search that completed before its Interrupt ran still dispatches. ArrowUp/ArrowDown move the active row, Enter commits the highlighted name into the draft, Escape closes the list, and clicking a row still fills the draft. The Name button submits when the trimmed name is non-empty — Enter does not, because the combobox owns the input's keydown. Catalog suggestions assist only — free-typed / nonexistent names remain submittable. The list is anchored and portaled to `document.body`, so it carries `z-50` to clear the `z-40` prompt frame. The anchor sets `isPlacementLocked: true`: the result-count-driven height change on every keystroke would otherwise re-run Floating UI's `flip` and jump the panel between below and above the input.
...
- Unit tests cover typing into the typeahead (input value and string draft both track the keystroke; the keystroke cancels the previous search and the cancellation fires `SearchCardNames` for the current draft, or nothing under 2 characters) and picking a suggestion (draft names that card; the list closes).
- A Story test covers two keystrokes in a row: resolving the second cancellation leaves only the search for the current draft pending, so the abandoned query's suggestions can never reach the model.
