...
- **End Turn**: turn yield armed while that seat is the active player. Additional clears: (a) at Cleanup for that seat, (b) when any other seat submits a non-PassPriority intent that itself clears turn yield (so the ending player can respond — empty combat declarations do not count). While End Turn is active, `has_empty_stack_instant_play` is checked on other seats so they still get response windows.
...
- **Recurring trigger enqueue families are table-driven.** `Game::enqueue_triggers` still owns the per-event rules order, but the common watch shapes behind step-begin, life-gain/loss, ETB, and "watch every permanent" scans now route through `TriggerWatch` rows instead of hand-written per-event battlefield loops. Death look-back, cast filters, and combat-damage watches remain bespoke because they need batch scratch or per-watcher predicates.
