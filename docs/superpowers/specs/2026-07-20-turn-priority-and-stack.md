...
- **End Turn**: turn yield armed while that seat is the active player. Additional clears: (a) at Cleanup for that seat, (b) when any other seat submits a non-PassPriority intent that itself clears turn yield (so the ending player can respond — empty combat declarations do not count), (c) when the attack declarer holds priority on Declare Attackers with a non-empty `required_attacks` list (goad / must-attack — empty seal is illegal, so End Turn disarms instead of auto-passing forever). While End Turn is active, `has_empty_stack_instant_play` is checked on other seats so they still get response windows — except a seat that has armed its own turn yield, which has already declined every window until its Untap and is auto-passed straight through.
...
- **Recurring trigger enqueue families are table-driven.** `Game::enqueue_triggers` still owns the per-event rules order, but the common watch shapes behind step-begin, life-gain/loss, ETB, spell-cast, player-damage, and "watch every permanent" scans now route through `TriggerWatch` rows instead of hand-written per-event battlefield loops. Death look-back and the remaining scratch-heavy or ordering-sensitive cases stay bespoke.
