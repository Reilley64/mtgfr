...
- **Scroll.** `shellFrame` is a viewport-contained flex column (`overflow-hidden`); the stage is `flex-1 min-h-0`. The builder page fills that stage (`h-full min-h-0`, single `minmax(0,1fr)` grid row, `overflow-hidden`) and does not scroll the page. The left catalog grid and the right decklist are independent `overflow-y-auto` scrollports with `overscroll-contain` so wheel/trackpad in one pane does not move the other or the document. Both columns use `min-h-0` so their scroll hosts form real scrollports inside the grid instead of growing the page.
