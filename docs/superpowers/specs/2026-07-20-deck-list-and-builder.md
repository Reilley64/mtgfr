...
- **Print picker scroll lock.** While the picker is open (`printPicker` set), catalog and decklist scrollports use `overflow-hidden` (background frozen). The print grid keeps scrolling: `VirtualList` puts `overflow: auto` on its container as an inline style, which no class-based lock can override. Closing the picker restores independent pane scrolling. `Dialog`'s own scroll lock only freezes `documentElement`, so freezing the two inner scrollports stays the builder's concern.
