...
The parent `Message` union in `client/app/messages.ts` carries app-level tags (`UrlChanged`, `ReceivedMeGravatarHash`, …), shared shell ticks (`CardArtTick`, `DeckCardFlipTick`), and **`Got*Message` wrappers only** for child submodels — not flattened child tags. Wrappers: `GotAuthMessage`, `GotAccountMenuMessage`, `GotDeckListMessage`, `GotDeckBuilderMessage`, `GotCoverageMessage`, `GotLeaderboardMessage`, `GotLobbyMessage`, `GotBoardMessage`, `GotGameMessage`.
...
### Account chrome (`client/app/shell/account-chrome/**`)

Every signed-in shell route puts the same avatar chrome in `shell-header-trailing`: an optional `Leaderboard` link (`header-leaderboard-link`) and an avatar trigger (`account-menu-trigger`) rendering the seat face helper against `session.meGravatarHash`.

The dropdown is a `@foldkit/ui` `Menu` submodel. Its state (`accountMenu`, `Menu.init({ id: "account-menu" })`) lives on the **root** model, not per route, because the chrome is the same element everywhere; every route's `viewInputs` passes it down and lifts its messages back through `GotAccountMenuMessage`. Open/close, Escape, click-away, arrow-key navigation, typeahead, focus return to the trigger, and Floating-UI anchoring (`bottom-end`) are the primitive's. Route entry does not close the menu: `Menu` renders a full-screen backdrop while open, so nothing outside it is clickable.
...
- `client/app/shell/surfaces.test.ts` — shell Scene coverage for auth, deck, leaderboard, coverage, and lobby surfaces, including shared account chrome and the `% faithful` + `API {version}` shell badge stack; Scene asserts auth stage hero `auth-brand` outside `auth-panel`, `pool-coverage` above `app-version` when the model has complete meta, asserts `pool-coverage[href="/coverage"]`, and asserts `/coverage` renders the global percent, search field, row filtering/empty state, and retry error UI. Account-menu coverage clicks `account-menu-trigger` for real and follows each row to its outcome: `account-menu-sign-out` ends the session and redirects off the protected route, `account-menu-gravatar` reaches `OpenGravatar`. Both resolve `Menu`'s `FocusItems` command and its `PortalMenuBackdrop` / `AnchorMenu` Mounts, then `expectEnded` those Mounts and resolve `FocusButton` once the row commits.
