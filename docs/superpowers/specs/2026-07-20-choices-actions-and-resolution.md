...
A step that produces a value a *later* step reads ("remove all counters … draw a card for each counter removed **this way**") writes a named tally on `Game::resolution_frame`, and the later step reads it through the matching `Amount` variant (`counters_removed_this_way`, `permanents_destroyed_this_way`, …). Because the frame is per-resolution orchestration state on `Game`, the producing step must run on the `&mut self` `Game::run` dispatch path — `Game::execute_effect` and `Game::mint_counters` take `&self` and cannot write the frame. Each producer overwrites its own tally rather than accumulating, so a repeated step reports only its own last run. The tally name is what binds producer to consumer; effects carry no explicit producer/consumer binding field.
