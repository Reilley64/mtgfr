...
| [card-image-cdn](2026-07-30-card-image-cdn-design.md) | Design input: `edh-images.reilley.dev` Worker + R2 bucket filled on miss from Scryfall (JPEG stored as-is, no Images transformations), 302-to-Scryfall on failed fill giving `large` the fallback only `art_crop` has; `buildImageUrl` emits `.jpg`; update production-topology-and-operations §Card art CDN at implement time |
