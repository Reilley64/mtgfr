...
| [foldkit-submodels-pwa-and-play-routes](2026-07-26-foldkit-submodels-pwa-and-play-routes-design.md) | Design input: Foldkit `Got*`/`inform*`/`domain/`, play routes (`/play` picker → pregame deck+table → in-game table-only), installable-only PWA; update shell/lobby/board specs per wave |
