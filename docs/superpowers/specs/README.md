...
| [mana-only-actions](2026-07-29-mana-only-actions-design.md) | Design input: `mana_only` on `LegalAction`/`ActionView` so paid mana activates (Viridescent Bog, filter lands, Signets) drop the mint playable border; priority behavior unchanged; `mana_matters` payoff rule deferred until a card needs it; shipped behavior documented in [activation-menu](2026-07-25-activation-menu.md), [battlefield](2026-07-20-battlefield.md) and [wire-protocol-and-visibility](2026-07-20-wire-protocol-and-visibility.md) |
| [effect-and-modifier-architecture](2026-07-28-effect-and-modifier-architecture-design.md) | Design input: split resolution (data, inspectable — generic verbs + `this_way` value channel + `Condition::Compare`) from modifiers (one registered `Modifier` list absorbing 13 `Permanent` EOT fields + 6 `Game` fields); ECS rejected; refines engine-refactor-program Wave E; update card-dsl / engine-core / choices / card-inspect + `DSL_REFERENCE.md` per wave |
| [cluster-split-on-engagement](2026-07-30-cluster-split-on-engagement-design.md) | Design input: committed attackers/blockers/targets take their own row slot (`keyOf` → `id:`), so a crowded-row cluster dispenses its next free copy; abilities counted across the pile; Shift commits the whole pile; deletes dead `density.ts` fan; update battlefield / activation-menu / combat specs at implement time |
| [card-image-cdn](2026-07-30-card-image-cdn-design.md) | Design input: `edh-images.reilley.dev` Worker + R2 bucket filled on miss from Scryfall (JPEG stored as-is, no Images transformations), 302-to-Scryfall on failed fill, replacing the client-side art_crop-only fallback; `buildImageUrl` emits `.jpg`; update production-topology-and-operations §Card art CDN at implement time |
| [arena-card-frame-renderer](2026-08-01-arena-card-frame-renderer-design.md) | Design input: `domain/card-render/` draws cards from vendored frame assets + art crop (`permanent` square tile / `full` hand face without mana cost / `stack` ability face), `PrintRenderView` catalog RPC over a lazily-filled `printings` cache, per-ability `oracle` in the card TOML; update battlefield / hand-and-zone-bar / stack / camera-layout / flights / wire / catalog / card-dsl specs per slice |
