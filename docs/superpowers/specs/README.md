...
| [buf-proto-lint-breaking](2026-07-27-buf-proto-lint-breaking-design.md) | Design input: full `STANDARD` via service/RPC renames (no lint exceptions) + CI/local `buf breaking` (WIRE vs `origin/main`) with major-title / `/v2` hatch; update `ci-and-release`, wire-protocol, `WIRE_COMPAT.md` at implement time |
