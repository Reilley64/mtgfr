// Backfill `subtypes` onto the pool's card TOMLs from Scryfall.
//
// Creature subtypes exist nowhere else in the card model. Scryfall is authoritative and we already
// hold every card's Scryfall id (client/src/lib/card-ids.json, from ADR 0015), so this resolves each
// card's `type_line`, then writes it as a top-level TOML key:
//   - `subtypes = ["Goblin"]` for non-lands (a land's types already live under [kind])
//
// Printing-aware set lists are owned by tooling/backfill-sets.mjs; this script removes legacy
// singular `set = ...` metadata but never writes or rewrites `sets = [...]`.
//
// Idempotent + re-runnable: it strips any top-level singular `set`/`subtypes` it previously wrote before
...
async function fetchMeta(nameToId) {
  const ids = [...new Set(Object.values(nameToId))];
  const byId = new Map(); // scryfall id -> { typeLine }
  for (let i = 0; i < ids.length; i += 75) {
    ...
    for (const c of data) byId.set(c.id, { typeLine: c.type_line ?? "" });
...
export function subtypesOf(typeLine) {
...
export function rewriteTomlMeta(text, meta) {
  ...
  // Drop any top-level singular set/subtypes we wrote on a previous run (before the first table only,
...
function backfillFile(path, meta) {
  ...
  try {
    writeFileSync(path, rewriteTomlMeta(text, meta));
  } catch (e) {
...
async function resolveByName(name) {
  ...
  return { id: c.id, typeLine: c.type_line ?? "" };
}

async function main() {
  const nameToId = JSON.parse(readFileSync(ID_MAP, "utf8"));
  const meta = await fetchMeta(nameToId);
