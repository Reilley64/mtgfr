...
`counter_replacement`: adder → `add = 1`; doubler → `times = 2` (omit `add`); halver → `halve = true`. Applications are ordered additions → multipliers → halvings (CR 616.1's ordering choice is not offered).
