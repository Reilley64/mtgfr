...
| `"cycled"` | "When you cycle this card" (CR 702.29e): scanned off the cycled card's own abilities at `Game::cycle`'s discard, like `"when_you_cast_this"`'s self-scan — not a battlefield watcher. Queued to land *above* cycling's own draw on the stack (Krosan Tusker's "you may search your library for a basic land card … (Do this before you draw.)"); a card-with-no-cycled-ability keeps its plain immediate draw untouched. Pairs with `optional = true` and, for a paid rider, an `[abilities.cost]` carrying `x = true`/`x = N` (Decree of Justice's "you may pay {X}. If you do, create X 1/1 white Soldier creature tokens.") — answered by `Intent::PayOptionalCostX { pay, x }`, a dedicated wire shape (not `PayOptionalCost`) so the chosen `{X}` threads onto the placed ability's stack item for its own `Amount::X` to read (same mechanism an activated ability's `{X}` cost uses). |
