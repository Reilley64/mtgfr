...
| `damage/to_triggering_player` | `amount` (amount) | no (author as `type = "damage"`, `mode = "to_triggering_player"`; an `each_upkeep` payoff: damage the player whose upkeep this is, filled from the trigger's active player at placement — Copper Tablet's "At the beginning of each player's upkeep, this artifact deals 1 damage to that player." `damage/each_player` would bill every seat on every upkeep, which is once per seat per round instead of once) |
