...
| `search_library` | `filter` (card filter, §7), `to_zone` (§7), `tapped` (bool, default `false`), `count` (u8, default `1` — "up to N" finds, one shuffle), `searcher` (`"you"` default, `"target_controller"` — Path to Exile's compensation search, `"all_players"` — Veteran Explorer's "each player may search"), `overflow` (§7, default none — where every find *after the first* goes, if different from `to_zone`; Cultivate), `count_amount` (amount, §6, default none — a dynamic cap that *overrides* `count` when set, resolved once as the search begins and carried to every seat of an `all_players` fan-out; Collective Voyage's "up to X basic land cards, where X is the total amount of mana paid this way" is `count_amount = "mana_paid_this_way"`) | no (searcher picks; tutors/ramp/fetchlands) |
