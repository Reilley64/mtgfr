...
| `each_opponent_discards` | — | no (multi-player fan-out — Syphon Mind's "Each other player discards a card": each opponent, in APNAP order, discards one card of their choice, pausing on that player's own card-pick — an empty-hand opponent is skipped, contributing 0. Carries no payoff of its own: put the reflexive payoff as a following `effects` step reading `"cards_discarded_this_way"` (§6), resumed once every opponent has answered — the discard twin of `each_player_exiles_from_graveyard`) |
...
- `"cards_discarded_this_way"` — how many cards *this same resolution's own* preceding discard step
  discarded: an `each_opponent_discards` fan-out across every opponent (Syphon Mind's "You draw a
