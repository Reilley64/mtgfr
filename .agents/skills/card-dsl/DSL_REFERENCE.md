...
| `forecast` | `[forecast]` table | (none) | Forecast (CR 702.57 — Skyscribing's "Forecast — {2}{U}, Reveal this card from your hand: Each player draws a card."): `[forecast.cost]` + `[[forecast.effects]]`, the same shape as `[hand_ability]`. Unlike `hand_ability`, the card is *revealed* rather than discarded — it stays in hand — and CR 702.57a restricts activation to the controller's own upkeep, once each turn. No card needs both `hand_ability` and `forecast`. Shares `Intent::ActivateHandAbility` / the `activate_hand_ability` action (the engine reads whichever field is set and applies that mechanic's own cost-payment and gates), so no separate intent/action exists for it. |
...
| `remove_counters_kind` | string | absent | Which counter kind `remove_counters` removes: `"charge"`/`"story"`/`"study"`/`"vow"`/`"minus_one_minus_one"` (Staff of the Storyteller's "remove a story counter"; Wickerbough Elder's "Remove a -1/-1 counter"). Absent = the +1/+1 path. |
...
| `damage_each_creature` | `amount` (amount), `opponents_only` (bool, default `false` — Volcanic Torrent's "each creature ... your opponents control"), `filter` (§7 permanent filter, default: any creature — Breath of Darigaaz's "each creature *without flying*" is `filter = { without_flying = true }`) | no (damage to every creature matching `filter`) |
...
| `enters_with_counters` | `count` (amount), `kind` (`"charge"`/`"story"`/`"study"`/`"vow"`/`"minus_one_minus_one"`, default +1/+1 — Astral Cornucopia's "X charge counters"; Wickerbough Elder's "enters with a -1/-1 counter") | no (static; a -1/-1 counter subtracts 1/1 per counter from the entering creature's power/toughness, the mirror of the +1/+1 path — CR 704.5r's +1/+1 ↔ -1/-1 annihilation SBA isn't modeled, see `characteristics.rs`'s `pt_layers`) |
...
| `{ card_in_graveyard = { whose, filter } }` | a graveyard card matching a **card filter** (below); `whose` = `"yours"`/`"any"`/`"opponents"` (a living opponent's graveyard, never the chooser's own — CR "target card in an opponent's graveyard") | reanimate_to_battlefield/tuck_from_graveyard with a non-creature or MV-gated filter — Sevinne's Reclamation, Mystic Sanctuary, Excava; exile_target with `whose = "opponents"` — Nezumi Graverobber |
...
only, no instants — Anarchist's "target sorcery card from your graveyard"), `"enchantment"` (an Aura counts —
...
`tapped = true` to enter tapped), or `"graveyard"` (Buried Alive: "put them into your graveyard"
— routed through the same library-to-graveyard choke mill effects use, so the arrival never
counts as "from the battlefield" and can't fire Dies). The searcher picks up to `count` matches
(default `1`; or none — "fail to find" is always legal at any point and ends the search), each
moved to `to_zone` as it's found; the library is shuffled once, after the last pick (CR 701.19f —
Land Tax's "up to three basics" is `count = 3`). `searcher = "target_controller"` hands the search
to the ability's shared target's controller instead (Path to Exile's/Assassin's Trophy's
basic-land compensation). In this model a search reveals nothing to opponents.
