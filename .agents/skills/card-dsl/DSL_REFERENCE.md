...
| `target_becomes_treasure` | `target` | yes (target creature becomes a Treasure artifact and loses all other card types and abilities — CR 613.1d/613.1f type/ability SET, **indefinite** like `set_own_base_pt_from_amount` above, not until-end-of-turn. Mints `Event::BecameCopy { def: becomes_treasure(current_def), until_eot: false, .. }`: `becomes_treasure` keeps the target's own name, mana cost, id and legendary flag but replaces kind/subtypes/keywords/abilities with the Treasure profile, so the converted permanent is not a token and goes to the graveyard like any other creature, unlike a real "create a Treasure" token. A no-op on an illegal target (CR 608.2b). Vraska, Betrayal's Sting's −2) |
