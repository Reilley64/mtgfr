...
| `remove_counters_kind` | string | absent | Which counter kind `remove_counters` removes: `"charge"`/`"story"`/`"study"`/`"vow"`/`"minus_one_minus_one"`/`"storage"` (Staff of the Storyteller's "remove a story counter"; Wickerbough Elder's "Remove a -1/-1 counter"; Fungal Reaches' "Remove X storage counters"). Absent = the +1/+1 path. |
