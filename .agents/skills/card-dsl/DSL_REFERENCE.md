...
| `put_counters_on_player` | `kind` (player-counter kind — `"poison"` or `"rad"`; a *player*-side vocabulary, disjoint from `put_counters`' permanent `kind` axis, since no permanent counter can sit on a player and no player counter on a permanent), `count` (amount), `scope` (`"all_players"` — "each player gets a poison counter", Ichor Rats; `"each_opponent"` — "each opponent gets a poison counter", Infectious Inquiry / Vraska's Fall; the same `scope` axis `each_player_sacrifices` uses, minus `"targeted_players"`, which no pool card spells for counters) | no (counters go on the *players* in `scope`, never a permanent — CR 122.1. A player who has already lost gets nothing. Ten or more poison counters lose the game via the CR 704.5c state-based action. `"rad"` (Fallout, Feral Ghoul) has no such threshold — instead a turn-based action at the beginning of each player's precombat main phase mills that many cards, and each *nonland* card milled that way costs its controller 1 life and removes one rad counter; a short library mills only what is there, so the life loss and the removal follow the real nonland count) |
