...
| `who` | `"this"` (default), `"your_creatures"`, `"your_tokens"`, `"any_creature_damaging_your_opponent"` | `"this"` | `deals_combat_damage_to_player` | Whose combat damage counts (Leitmotif Composer / Ohran Frostfang / Curiosity Crafter). `"any_creature_damaging_your_opponent"` is the only scope that reads the *damaged* player: any creature at all, whoever controls it, but only when the damage landed on one of the ability controller's opponents (Edric, Spymaster of Trest's "whenever a creature deals combat damage to one of your opponents"). |
...
| `proliferate` | `times` (amount, usually `1` or `"x"` — Expansion Algorithm's "Proliferate X times") | no (untargeted; pauses on a select-any-number choice over every battlefield permanent with a counter, giving each one more of every kind already there — CR 701.27; players with poison are not offered yet — widen when a proliferate card needs them) |
...
| `each_opponent_gets_poison` | `count` (u32) | no (each living opponent gets that many poison counters — Infectious Inquiry) |
| `each_opponent_loses_all_counters` | — | no (each living opponent loses every counter they have — Final Act; today clears poison only) |
