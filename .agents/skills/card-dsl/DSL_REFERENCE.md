...
| `controller` | `"any"` (default), `"you"`, `"opponent"`, `"active_player"` | whose permanents qualify, relative to the effect's controller — except `"active_player"`, which is absolute: whoever is the active player when the filter runs (Siren's Call's "creatures the active player controls"). Deliberately not the same as `"opponent"`, which at a four-player table reaches three boards; every card printing this clause names exactly one player. Re-read each time the filter runs, so a sweep scheduled this turn still names the same player at that turn's end step |
