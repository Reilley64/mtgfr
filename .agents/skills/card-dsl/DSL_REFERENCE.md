...
| `destroy/all` | `filter` (§7 permanent filter), `cant_be_regenerated` (bool, default `false` — CR 701.15d "They can't be regenerated": turns off regeneration shields for this sweep, same rider as `destroy/target`'s — Reiver Demon, Winds of Rath) | no (author as `type = "destroy"`, `mode = "all"`; mass destruction, no target) |
