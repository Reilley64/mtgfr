...
| `snow` | bool | `false` | Snow supertype (CR 205.4g — Snow-Covered Forest, Ohran Frostfang). Read by snow-matters filters (`snow_land`, `PermanentFilter.snow`). |
...
| `enchant_graveyard` | bool | `false` | Animate Dead's own "enchant creature card in a graveyard" (CR 303.4a): unlike `enchant` above, the cast target is a creature *card in a graveyard*, not a battlefield permanent — `required_target` reports `TargetSpec::CreatureCardInAnyGraveyard` for the card instead of `enchant`'s battlefield-permanent spec, checked ahead of it. Kind is still `aura` (CR 704.5m's Aura-orphan SBA applies), but `resolve_spell` routes it through the generic permanent-enter path (same as a Creature/Enchantment) instead of the ordinary Aura immediate-attach arm, since there's no battlefield host yet at cast — the card's own `etb` ability does the reanimate-then-attach via `reanimate_to_battlefield` + `attach_self_to_reanimated`. The choice threads to that reanimation effect via `target = "this_auras_graveyard_target"` (§7) — a fixed reference, not a fresh choice. It's unattached for the brief window between entering and that ETB ability resolving; `check_state_based_actions`'s Aura-orphan sweep (which runs *before* the ETB ability is even placed on the stack) exempts an `enchant_graveyard` Aura for exactly that window — see `CardDef::enchant_graveyard`'s own doc. A bare bool: the pool has exactly one such card, unrestricted ("a graveyard", not "your graveyard"). The printed ETB self-rewrite ("loses 'enchant creature card in a graveyard' and gains 'enchant creature put onto the battlefield with this Aura'") is modeled: attaching records the reanimated object as the Aura's only legal host, and the CR 704.5m sweep holds it to exactly that object. |
...
| `devoid` | bool | `false` | Devoid (CR 702.114a): the card is colorless despite any colored mana-cost pips — takes priority over both the cost-pip derivation and `colors` (Smothering Abomination's `{2}{B}{B}` stays colorless). |
...
| `enters_tapped_unless_you_pay_life` | integer | (none) | A CR 614.12 as-enters replacement **choice** (shocklands — Overgrown Tomb: "As this land enters, you may pay 2 life. If you don't, it enters tapped."), *not* a `Condition` — the player decides, no board state decides for them. Raises a pay-or-decline pending choice as the land enters, offered only when the controller's life total is **greater than or equal to** the integer (CR 119.4 — a player may pay life down to and including 0); below that the land simply enters tapped, no prompt. |
| `free_cast_if` | `[free_cast_if]` table | (none) | A printed "you may cast this spell without paying its mana cost" permission (CR 118.5 — Massacre: "If an opponent controls a Plains and you control a Swamp, you may cast this spell without paying its mana cost."). A `Condition`, same shape/`type =` tags as `enters_tapped_unless` above, checked fresh at cast time; when it holds the cast is free outright (no decline pause — a rational caster always takes a free cast, same modeling `hand_has_land_with_subtype` uses). Compose multiple gates with `"all"` (§5). |
| `alternative_cost` | `[alternative_cost]` table | (none) | A printed alternative cost that isn't mana at all (CR 601.2f — Invigorate: "If you control a Forest, rather than pay this spell's mana cost, you may have an opponent gain 3 life."): `[alternative_cost.condition]` (optional — a `Condition`, same shape as `free_cast_if` above; absent = always offered) gates whether the caster may declare it, and `[alternative_cost.rider]` (a single effect, the standard §6 shape) is what they pay instead. Distinct from `free_cast_if`: this is the caster's own choice (declared via `alternative_cost = true` on the cast intent, mirroring `evoked`'s opt-in shape), not automatic, and the rider is a real cost fired at cast time — not a resolution effect. Declaring it replaces the printed `[cost]` entirely; no reduction/kicker/delve/etc. folds on top (CR 601.2f's "rather than pay this spell's mana cost"). ponytail: `Effect::OpponentGainsLife` picks the lowest-seat-index living opponent rather than offering a real choice among several — see that effect's own doc. |
| `cast_only_during_combat` | bool | `false` | "Cast this spell only during combat" (CR 601.3e — Cauldron Dance): legal only from begin-combat through end-of-combat inclusive. Layered on top of, not instead of, the ordinary instant/sorcery-speed gate — a flash/instant card with this flag is still open only during those steps, not any time it would otherwise hold priority. |
| `cast_only_during_opponents_turn` | bool | `false` | "Cast this spell only during an opponent's turn" (CR 601.3e — Siren's Call): someone other than the caster must be the active player. The cast-side twin of the activated-ability `only_during_opponents_turn` below, and independent of `cast_only_before_attackers` — Siren's Call prints both in one sentence and needs both keys. |
| `cast_only_before_attackers` | bool | `false` | "Cast this spell only before attackers are declared" (CR 601.3e — Master Warcraft): legal any time up to and including the declare-attackers step, and within that step only while attackers are still undeclared. Same layering as `cast_only_during_combat` above — a restriction on top of the ordinary timing gate, never a widening of it. |
| `cast_only_before_combat_damage` | bool | `false` | "Cast this spell only before the combat damage step" (CR 601.3e — Berserk): legal any time up to and including the declare-blockers step, and closed for the rest of that turn once combat damage is dealt. The boundary is the *first* combat damage step, so a first striker in combat closes the window one step earlier (CR 510.5). Same layering as its two siblings above. |
...
| `cycling_sacrifice` | string or table | `"none"` | A sacrifice folded into the cycling cost (CR 702.29b — Edge of Autumn's "Cycling—Sacrifice a land" carries no mana at all): same string/table shape as an activated ability's `sacrifice` (see the `sacrifice` row above — `"this"`/`"creature"`/`{ creature = {…}, count = N }`/`{ permanent = {…}, count = N }`), e.g. `cycling_sacrifice = { permanent = { types = "land" } }`. Validated and paid through the same choke an ordinary activation's sacrifice cost uses; `Intent::Cycle`'s `sacrifice: Option<ObjectId>` names the pick (ignored for a card whose cycling carries none). |
| `hand_ability` | array of `[[hand_ability]]` tables | `[]` | Zero or more hand-activated, discard-this-card abilities whose payload isn't cycling's fixed draw-1 (CR 113.6/602.5e — Magma Opus's "{U/R}{U/R}, Discard this card: Create a Treasure token."): each `[[hand_ability]]` entry has its own `[hand_ability.cost]` (same shape as `[cost]`, §2 — hybrid pips included) plus `[[hand_ability.effects]]` (the standard §5 effects-array shape). More than one entry models typecycling (CR 702.29d — Valley Rannet's mountaincycling and forestcycling are two separate abilities with different search filters, not one ability with a two-subtype filter): author one `[[hand_ability]]` block per named type, in printed order. The general sibling of `cycling` — don't overload `cycling` for an authored payload. `Intent::ActivateHandAbility`'s `index` selects which entry; the `activate_hand_ability` action is offered once per affordable entry, pays that entry's cost, discards the card, then puts the payload on the stack as a real activated ability (a single stack ability, `Sequence`-wrapped when it has more than one effect — respondable, countered by `counter_target_activated_ability`), like cycling's own draw. |
| `forecast` | `[forecast]` table | (none) | Forecast (CR 702.57 — Skyscribing's "Forecast — {2}{U}, Reveal this card from your hand: Each player draws a card."): `[forecast.cost]` + `[[forecast.effects]]`, the same per-entry shape as `[[hand_ability]]`. Unlike `hand_ability`, the card is *revealed* rather than discarded — it stays in hand — and CR 702.57a restricts activation to the controller's own upkeep, once each turn. No card needs both `hand_ability` and `forecast`; when carried it activates at `index = 0`. Shares `Intent::ActivateHandAbility` / the `activate_hand_ability` action (the engine reads whichever field is set and applies that mechanic's own cost-payment and gates), so no separate intent/action exists for it. |
...
| `cumulative_upkeep` | `[cumulative_upkeep]` table | (none) | Cumulative upkeep (CR 702.24, Jotun Grunt): "At the beginning of your upkeep, put an age counter on this permanent, then sacrifice it unless you pay its upkeep cost for each age counter on it." `[cumulative_upkeep] graveyard_cards = 2` — every controller upkeep (no "since your last upkeep" gate, unlike `echo`) places an `age` counter, then pauses on a `PendingChoice::PayCumulativeUpkeepOrSacrifice` scaled to `graveyard_cards × age counters`: paying puts that many cards from a *single* graveyard (any player's) on the bottom of that graveyard owner's library (`Intent::ChooseSacrifices`, an empty answer declines and sacrifices the permanent, reusing the "empty declines" wire shape rather than a new intent). ponytail: the upkeep cost is fixed to this one non-mana shape — CR 702.24's upkeep cost may be any cost at all; grow a mana-cost arm when a future card needs one. |
| `recover` | `[cost]` table | (none) | Recover (CR 702.59, Grim Harvest): "When a creature is put into your graveyard from the battlefield, you may pay {cost}. If you do, return this card from your graveyard to your hand. Otherwise, exile this card." `[recover]`, same shape as `[cost]` (Grim Harvest's `[recover] generic = 2, black = 1` = `{2}{B}`). Fires once per qualifying creature death (reusing the same battlefield-only gate `functions_in_graveyard`'s Dies scan uses — a discard/mill arrival never triggers it) for every recover card already sitting in that creature owner's graveyard, including the dying creature itself (CR 702.59b) — pauses on a pay-or-exile choice (`PendingChoice::PayRecoverOrExile`, `Intent::PayOptionalCost`); paying moves the card graveyard→hand, declining exiles it. ponytail: if several creatures die simultaneously, each still queues its own trigger per CR ruling (only the first to resolve has any effect), but a later one whose card already left the graveyard is silently skipped rather than re-offering a no-op pay choice. No client form yet (Phase 5 client catch-up) — answerable today over the wire. |
...
| `enter_as_copy` | `{ .. }` table | (none) | Enter-as-a-copy replacement (CR 706/707.2): a rules-keyword (no `[[abilities]]` block) — `enter_as_copy = { until_eot = false, extra_counters = "0", gains_haste = false, of = "creature" }`. As this permanent enters, its controller **may** have it become a copy of any *other* object of type `of` on the battlefield (an as-enters pause; declining is legal — it stays its printed self; no candidate ⇒ no pause). Fields (all optional): `until_eot` (bool, default `false` — the copy reverts to the printed permanent at cleanup, CR 514.2; Cursed Mirror), `extra_counters` (amount, default `0` — additional +1/+1 counters the copy enters with, routed through the CR 614 counter-replacement path; Altered Ego's `"x"`), `gains_haste` (bool, default `false` — grants the copy haste; Cursed Mirror's "except it has haste"), `of` (`"creature"` \| `"enchantment"` \| `"artifact"`, default `"creature"` — the copyable candidate's type, read off the CR 613.4 type layer so an animated land is a creature here; `"enchantment"` also matches an Aura, CR 303.2). The remaining three are copy **exceptions** (CR 707.2) — baked into the copied def as it lands, so they are themselves copiable and a Clone copying one of these inherits them: `also_enchantment` (bool, default `false` — Copy Artifact's "except it's an enchantment in addition to its other types"; written to the indefinite `Permanent::added_types`, so Disenchant still answers it), `keeps_own_color` (bool, default `false` — Vesuvan Doppelganger's "except it doesn't copy that creature's color"; the copier's *resolved* colour identity is written into the synthesized def's explicit `colors`, so it survives being re-copied), `keeps_own_abilities` (bool, default `false` — Vesuvan Doppelganger's "and it has \"At the beginning of your upkeep, …\""; only the `becomes_copy_of_target` ability itself rides along, so a second re-copy doesn't stack the abilities of the creature it was wearing). Altered Ego = `{ extra_counters = "x" }`; Cursed Mirror = `{ until_eot = true, gains_haste = true }`; Copy Enchantment = `{ of = "enchantment" }`; Clone = `{}`; Copy Artifact = `{ of = "artifact", also_enchantment = true }`; Vesuvan Doppelganger = `{ keeps_own_color = true, keeps_own_abilities = true }`. Copying an Aura this way enters unattached and then pauses again on `ChooseAttachHost` (the same deployed-Aura attach path a searched-out or reanimated Aura uses, CR 303.4f) to pick a host among legal enchant targets. The copied values are the chosen object's *current* def, so copying something already under a copy effect lands on the form it is wearing (CR 707.2). ponytail: the copied object's own ETB triggers don't fire (`def` is overwritten after the enter event). |
| `back` | `[back]` table | (none) | A full inline card table for a card's second face — its own `name`/`[back.cost]`/`[back.kind]`/`[[back.abilities]]`, the same shape as a top-level card. Two consumers: **(a)** a "prepare" DFC (soc/sos), where the front face carries a `become_prepared` ability (§6) and while prepared its controller may cast a copy of this face, which unprepares it (Kirol, History Buff); **(b)** a CR 712 Kamigawa **flip card**, where `[back]` is the *flipped* face — the front carries a `flip_source` ability (§6) and once it resolves the permanent permanently uses this face's characteristics (Nezumi Graverobber → Nighteyes the Devourer). |
...
| `cast_only_before_blockers` | bool | `false` | "Cast this spell only … before blockers are declared" (CR 601.3e — Blaze of Glory): legal any time up to and including the declare-blockers step, and within that step only while no defending player has declared yet. The declare-blockers twin of `cast_only_before_attackers` above, and composable with it and with `cast_only_during_combat` — Blaze of Glory's "only during combat before blockers are declared" is `cast_only_during_combat` **plus** this key, since this key alone would leave the pre-combat main phase open. |
| `cast_only_during_declare_attackers` | bool | `false` | "Cast this spell only during your declare attackers step" (CR 601.3e — Camouflage): the attack-side twin of the row below, plus the "your" it doesn't print — closed on every other player's turn as well as in every other step. The window in which the attack is declared but blockers are not, which is what any "instead of declaring blockers" replacement needs. |
| `cast_only_during_declare_blockers` | bool | `false` | "Cast this spell only during the declare blockers step" (CR 601.3e — False Orders): the narrowest window of the family above — one step, not everything up to one. Unlike `cast_only_before_blockers` it stays open *after* the declaration is made, which is the whole point of False Orders: it rearranges a declaration that has already happened. Don't compose it with `cast_only_during_combat` (redundant) or with `cast_only_before_blockers` (contradictory — that one closes at the declaration). |
...
| `half` | array of `[[half]]` tables | `[]` | A split card's two castable halves (CR 709, cmd): each is a full inline card table — its own `name`/`[half.cost]`/`[half.kind]`/`[[half.abilities]]`, the same shape as `[adventure]` and `[back]`. The **top-level fields are the fused card** (CR 709.4: name `"Fire // Ice"`, the combined mana cost, the combined characteristics) and are never cast — only a half is (CR 709.4a, `Intent::CastSplitHalf { half }`, 0 = left). A half on the stack resolves as its own instant/sorcery; the card is the fused card again in every other zone. Fire // Ice. |
| `sets` | array of strings | `[]` | Every Scryfall set/edition code with a printing of this card's oracle (lowercase, e.g. `["cmd", "soc"]`). Pure catalog metadata — the engine never reads it; it exists so coverage and deck-builder search can credit every printing-aware set. Backfilled from Scryfall by `tooling/backfill-sets.mjs`; don't hand-edit unless adding a card the tool won't resolve. |
| `subtypes` | array of strings | `[]` | Printed subtypes (creature types like `["Goblin", "Wizard"]`; also artifact/enchantment subtypes). Gameplay-relevant: the permanent filter's `subtypes` axis, `anthem_static`'s `subtypes`, the spell filter's `has_subtype`, and `you_control_no_subtype` all match against it. A **land's** types stay under `[kind].subtypes` (§3, rules use those); the catalog unions the two, and so does the engine's live subtype read — a `subtypes` axis matches a land's own type line without the card storing it here. Backfilled by `tooling/backfill-card-meta.mjs`. |
...
| `dredge` | int | (none) | Dredge N (CR 702.52): a rules-keyword (no `[[abilities]]` block) that works from this card's graveyard. `dredge = 3` (Life from the Loam). It **replaces** a single draw — a replacement effect, not a trigger, so no stack item: when its owner would draw one card, a `ChooseDredge` choice offers milling exactly N off the top of their library and returning this card graveyard→hand *instead* of drawing (declining draws normally). Offered only while the library holds at least N (CR 702.52a). ponytail: forks single draws only today; a "draw three" that offers dredge per individual draw is #200 slice 2. |
| `cast_x_max` | string | (none) | A non-mana cast-time ceiling on `{X}` (CR 601.2b), beyond what the caster can pay for. Only value today: `"player_count"` — Open the Way's "X can't be greater than the number of players in the game" (the count of living seats). A cast announcing a larger X is rejected (`IllegalChoice`), and the count-picker's offered ceiling is clamped to the same bound. Ignored on a card with no `{X}`. ponytail: single-variant string (only the player-count cap has a pool consumer) — grow a variant/`Amount` when a differently-bounded `{X}` spell lands. |
| `vanishing` | int | (none) | Vanishing N (CR 702.63): a rules-keyword (no `[[abilities]]` block). `vanishing = 3` (Deadwood Treefolk). The permanent enters with N time counters (CR 702.63a — answered through the same enters-with-counters choke as an `enters_with_counters` static, so `kind = "time"` counters are placed as it enters); one is removed at the beginning of each of its controller's upkeeps (CR 702.63b); when the last is removed its controller sacrifices it via a real triggered ability, so responses get a window (CR 702.63c). ponytail: the per-upkeep removal is done directly in the upkeep step rather than through a triggered ability of its own — the same shortcut `suspend`'s tick takes; nothing in the pool can respond to the removal itself. |
...
x_color = "black"   # a color restriction on the mana spent for {X} (CR 601.2g) — Drain Life's
                    # "Spend only black mana on X." The chosen value folds into that color's pips
                    # instead of into generic, so the ordinary colored-pip planner enforces it:
                    # {X}{1}{B} with x_color at X = 2 wants {B}{B}{B}{1}. Omit for every other
                    # {X} cost, where any mana pays.
...
`discard = N` (Big Score's "discard a card"), `discard_land = true` (Throes of Chaos's/Call the
Skybreaker's retrace — "discard a land card"; shares the same `Intent::Cast` `discard_cost` slot
as `discard`, with the named card required to be a land; retrace's "in addition to paying its
other costs" only applies when casting from the graveyard — casting the same card from hand pays
no discard), `reveal_creature_from_hand = true` (CR 601.2g — Disaster Radius's "reveal a creature
card from your hand"; the caster must have a matching creature card in hand or the cast is
rejected outright, CR 601.2f; which specific card is revealed is never a real decision —
revealing has no cost or downside, so the engine auto-picks the highest-mana-value creature card
in hand, recorded for `"revealed_creature_mana_value"` below to read), `pay_life = N` (a fixed life payment — Deep
...
one or more creatures"; `count` is the marker string `"one_or_more"`) or `sacrifice = { count = N,
filter }` (a mandatory fixed count — Dread Return's Flashback—Sacrifice three creatures, CR
601.2f/602.2b: the cast must name exactly N distinct permanents matching `filter`, or is rejected;
`sacrifice = { count = 3, filter = "creature" }`), `[cost.additional.kicker]` (Kicker,
...
same `[cost]`-shaped sub-table, e.g. `[cost.additional.replicate]` / `generic = 2`), or
`[cost.additional.multikicker]` (Multikicker, CR 702.33c — Lightkeeper of Emeria's "Multikicker
{W}"/Comet Storm's "Multikicker {1}"; the same `[cost]`-shaped sub-table, e.g.
`[cost.additional.multikicker]` / `white = 1`). The two
...
usual flashback/escape exile or graveyard fork. A buyback cost can also carry a **non-mana**
sacrifice rider (CR 702.27f — Constant Mists' "Buyback—Sacrifice a land") instead of/beside mana
pips: since `[cost.additional.buyback]` is itself a `[cost]`-shaped table, it takes its own nested
`[cost.additional.buyback.additional]` `sacrifice = { count, filter }` (the same shape as a plain
spell's own `[cost.additional]` `sacrifice`, e.g. `sacrifice = { count = 1, filter = "land" }`).
When the caster opts into buyback, `Game::cast_cost` folds that rider into the returned cost's
`additional.sacrifice`, so it's required and paid through the ordinary additional-sacrifice
machinery — the same `Intent::Cast` `sacrifice_cost` slot, validated by
`Game::validate_cast_cost_picks` and paid alongside any other additional-sacrifice cost.
ponytail: the fold overwrites rather than merges with a base spell's own separate
`[cost.additional]` `sacrifice` — no pool card combines a buyback-sacrifice rider with the base
spell's own separate sacrifice cost; widen to merge counts/filters if one ever does. Strive scales with a
...
Multikicker (CR 702.33c) scales with a caster-declared *payment* count, committed the same
pre-stack way as Replicate's: the caster names it up front on `Intent::Cast`'s
`multikicker_count` (0 for none), `Game::cast_cost` multiplies `[cost.additional.multikicker]` by
the count directly (each payment is a full extra instance of the cost, like Replicate's — not
Strive's "beyond the first"), and the declared count is recorded on the resolved spell (read via
`Game::spell_multikicker_count`) — see `multikicker_scaled` in the target-count table below for
Comet Storm's "choose another target for each time this spell was kicked." Unlike Strive/
Replicate's own counts, a Multikicker payoff can be an ETB trigger (Lightkeeper of Emeria's "gain
2 life for each time it was kicked"), which resolves *after* the spell that became the permanent
is already gone; `Game::spell_multikicker_count` falls back to `Permanent::entered_multikicker_count`
(set as the permanent enters, the same "read the spell's own info before it's gone" idiom
`Permanent::entered_with_x` uses for `{X}`) so an ETB effect's `Amount` can still read it.
...
| `"battle"` | `defense` (i32) | `Battle` (starting defense counters; stored in `Permanent::loyalty`). Siege protectors, attack-for-defense, and transform-on-defeat are not modeled — author a residual on the card when those matter. |
...
blocked by other Shadow creatures, checked both directions in `Game::can_block`), `fear` (CR
702.36b — can be blocked only by artifact creatures and/or black creatures, checked in
`Game::can_block` via `effective_types` and `colors_of`), `intimidate` (CR 702.13b — fear's
color-sharing sibling: can be blocked only by artifact creatures and/or creatures that share a
color with it, same `Game::can_block` check), and
...
A non-modal spell may equally carry its two clauses inside **one** `[[abilities]]` block, as two
`[[abilities.effects]]` steps with *different* `target` specs (Vengeful Rebirth's "Return target
card from your graveyard to your hand… deals damage… to any target" — a
`return_from_graveyard_to_hand` clause 0 then a `deal_damage` clause 1 — the non-modal twin of Hull
Breach's third mode). Both clauses are single-target here, unlike Magma Opus's two multi-target
ones; either shape routes through the same post-cast `ChooseSpellTargets` chain, so the cast intent
carries **no** `target` at all and the caster answers one pause per clause. Consecutive steps with
the *same* target spec still share one chosen target (Killian's `goad_target` after `tap_target`),
so only a genuinely different spec starts a new clause; an untargeted step
(`exile_self_on_resolve`) rides along either way.
...
An **activated ability** may likewise carry an independent second target clause, but as a *cost*
rather than a second stack-bound effect clause: `graveyard_exile_target_count` (above) is a
targeted "exile N cards from an opponent's graveyard" cost (CR 601.2c/602.2b), paid immediately
after every other cost — the activator pauses on a `ChooseActivationCostTargets` choice naming that
many distinct cards from one opponent's graveyard, exiled before the ability (with its own
`effects`' ordinary `target`) goes on the stack (Spurnmage Advocate's "Exile two target cards from
an opponent's graveyard: Remove target attacking or blocking creature from combat and tap it").
Unlike the spell/triggered second clause above, this one is never chosen at the same moment as the
ability's own `target` (activation costs and the stack-bound effect are always independent target
sets); no separate TOML field for the graveyard cards' target spec exists yet — the pool's one
card needs exactly "an opponent's graveyard, any card," so that shape is hardcoded rather than
authored.
...
| `"as_enters"` | **"As this permanent enters, …" (CR 614.12) — a replacement effect, not a trigger.** Watched off the same entry events as `"etb"`, but `Game::place_pending_triggers` runs the effect *inline* instead of putting it on the stack, so no player gets priority between the entry and the choice it raises. Queued ahead of the same source's `"etb"` group. Use it for every "as ~ enters, choose …" (Archangel of Strife's war/peace vote, Flickering Ward's and Voice of All's color, Patchwork Banner's creature type) — an `"etb"` there would be an approximation. Not for targeted or optional abilities: an as-enters replacement effect neither targets nor asks "you may". |
...
| `"becomes_monstrous"` | When this permanent becomes monstrous (CR 701.28b — fires off `Event::BecameMonstrous`, minted only the first time a `monstrosity` effect resolves on it; scans the now-monstrous object's own abilities, the same self-scan idiom as `"turned_face_up"` above — Alpha Deathclaw's "…or becomes monstrous…"). |
| `"attacks"` | When this creature attacks. |
| `"blocks_or_becomes_blocked"` | Whenever this creature blocks or becomes blocked (CR 509 — Goblin Cadets). Fires off `Event::BlockerDeclared` when this creature is named as either the blocker or the attacker, once per creature per combat (a multiply-blocked attacker's "becomes blocked" still fires only once — batch-scanned from `Game::declare_blockers`, not a per-event scan). |
| `"blocks_or_becomes_blocked_by"` | Whenever this creature blocks or becomes blocked by a creature matching the sibling `filter` (CR 509.1a — Cockatrice / Thicket Basilisk: "Whenever this creature blocks or becomes blocked by a **non-Wall** creature, destroy that creature at end of combat"; the filter is `{ types = "creature", exclude_subtypes = ["Wall"] }`). Unlike `"blocks_or_becomes_blocked"` above this fires once per (blocker, attacker) **pair** and is *not* deduped — its payoff names the other creature, so two creatures blocking one Basilisk each turn to stone. That creature is baked into a following `destroy/that_creature` payoff, the same filler `"deals_combat_damage_to_creature"` uses. Sibling: `filter` (permanent-filter, §7). |
| `"attacks_or_blocks"` | Whenever this creature attacks or blocks (CR 508.1a / CR 509.3a — Mana-Charged Dragon). Unlike `"blocks_or_becomes_blocked"` above, only the *blocker* half of a block declaration fires it — an attacker "becomes blocked", it doesn't "block". The attack half rides alongside `"attacks"` off `Event::AttackerDeclared`; the block half is a separate batch-scan from `Game::declare_blockers`, deduped the same way so a creature blocking multiple attackers still fires once. |
...
| `"land_put_into_graveyard"` | "Whenever a land is put into a graveyard from the battlefield" (Dingus Egg). Lands are the one permanent type the controller-scoped death watches above all exclude, and this is the one that isn't controller-scoped either: it fires on **any** player's land, under every permanent on the battlefield that watches for it. Pair with the `damage/to_triggering_player` mode (§6), which addresses the dead land's controller — the same slot Copper Tablet fills from whose upkeep it is. Owner-based controller-at-death like its sibling watches (a land stolen with Confiscate bills its owner). |
| `"nonland_permanent_you_control_dies_including_this"` | "This or another nonland permanent you control is put into a graveyard from the battlefield" (Martyr's Bond) — a permanent-type-wide (creature, artifact, enchantment, or planeswalker), self-including generalization of `creature_you_control_dies_including_this`/`enchantment_you_control_dies`: any of the four nonland types, and self-fires like the `*_including_this` creature arms. Pairs with the `shares_type_with_dying_permanent` edict filter (§7) for a dynamic "shares a card type with it" payoff. |
...
| `"each_draw_step"` | At the beginning of *each* player's draw step — the draw-step twin of `each_upkeep`/`each_end_step` (Howling Mine). Unlike those two, its payoff needs to know *whose* draw step this is — pair with the `each_draw_step_player_draws` effect (§6), which resolves its `drawer` to that active player, not necessarily this ability's controller. |
| `"draw_step"` | At the beginning of *your* draw step (Mana Vault's "at the beginning of your draw step, if this artifact is tapped, it deals 1 damage to you") — the your-turn twin of `each_draw_step` above, scoped to the active player exactly like `upkeep` is. Both ride the same watch table, so the controller-scoped and each-player forms resolve off one queue; use this one whenever the payoff addresses the ability's own controller and never needs to know whose draw step it is. |
| `"first_main_phase"` | At the beginning of your first (precombat) main phase — `Step::Main1`, scoped to the active player like `upkeep` (Advanced Reconstruction). Fires *after* the draw step. |
| `"each_player_first_main_phase"` | At the beginning of *each* player's first main phase — the first-main-phase twin of `each_upkeep`/`each_end_step` (Magus of the Vineyard). Like `each_draw_step`, its payoff needs to know *whose* first main phase this is — the ability's own `add_mana` (§6) resolves its mana credit to that active player, not necessarily this ability's controller. |
...
| `"end_of_combat"` | At the beginning of the end of combat step, in *any* player's combat (CR 511.1 — Clockwork Beast's "At end of combat, if this creature attacked or blocked this combat, remove a +1/+0 counter from it"). Battlefield-wide like `each_end_step`, not active-player-scoped like `begin_combat`, because the "or blocked" half only ever happens on somebody else's turn. Queued from the end-of-combat turn-based action itself rather than off the step-began scan, ahead of the combat clear — so an intervening-if can still read who attacked and who blocked (see `"source_attacked_or_blocked_this_combat"`, §5 Conditions). |
...
| `"opponent_gains_life"` | When an opponent of the controller gains life — the opponent-scoped twin of `you_gain_life`, fired for every player other than the gainer (Punishing Fire). Functions from the graveyard (CR 603.6e) for a card with `functions_in_graveyard = true`. |
...
| `"creature_attacks"` | Whenever a creature attacks — any controller, any defender, fired once *per attacker* declared this combat (three attackers means three separate triggers), not gated on a count and not scoped to this permanent's controller being involved (Righteous Cause: "whenever a creature attacks, you gain 1 life"). |
...
| `"you_are_dealt_damage"` | Whenever this permanent's controller is dealt damage, combat or noncombat alike (CR 120.1 — Living Artifact's "Whenever you're dealt damage, put that many vitality counters on this Aura"). The receiving-end twin of `"deals_damage_to_opponent"`: the damaged player's own permanents watch, the dealer's don't. One trigger per damage event (CR 603.3), so a creature and a spell hitting the same player fire it twice. Pair with `"triggering_damage_dealt"` (§5 Amounts) for "that many". |
...
| `"permanent_becomes_tapped"` | Whenever a permanent matching `filter` — any player's, this is not controller-scoped — becomes tapped (CR 701.21a, so a second tap of an already-tapped permanent fires nothing). `for_mana = true` narrows it to a **land** that actually produced mana (CR 106.11; Manabarbs' "whenever a player taps a land for mana"); the default `false` watches every tap there is — an attack, an Icy Manipulator, a mana ability alike (Lifetap's "whenever a Forest an opponent controls becomes tapped"). The tapped permanent's controller is available to the payoff as `to_triggering_player` damage. |
| `"enchanted_permanent_becomes_tapped"` | Whenever the permanent this Aura is attached to becomes tapped (Psychic Venom's "whenever enchanted land becomes tapped") — the tap twin of `"enchanted_creature_attacks"`, fieldless because the host is whatever the Aura is on. Fires for *every* tap, mana or not, and runs as the Aura's own controller's trigger while the host's controller is the one `to_triggering_player` names. |
| `"you_play_a_land"` | Whenever you *play* a land (Fastbond) — the CR 305.1 special action, so a fetched, reanimated, or token land never fires it (that is `permanent_enters` with `filter = "land"`). Controller-scoped and filterless; pair with the `lands_played_this_turn_at_least` condition for Fastbond's "if it wasn't the first land you played this turn." |
| `"you_discard"` | Whenever you discard a card (Containment Construct; pairs with the `exile_from_graveyard_may_play`/`exile_discarded_with_this` payoffs, §6). Fires once **per discarded card**, filterless (a discarded land fires it too). |
| `"you_discard_nonland"` | Whenever you discard **one or more nonland cards** (CR 701.8, Conspiracy Theorist). A batch-once trigger like `cards_leave_your_graveyard`: one simultaneous discard is one event (fires once, not once per card), and it fires **only if at least one discarded card was a nonland** (a land-only discard fires nothing). Pairs with the `may_exile_discarded_nonland_may_play` payoff (§6), which offers a choice among exactly the discarded nonland cards ("one of them"). |
| `"deals_combat_damage_to_player"` | Whenever a creature deals combat damage to a player, scoped by the sibling `who` field (below). |
| `"deals_combat_damage_to_creature"` | Whenever *this* permanent deals combat damage to a creature (CR 510.2, Stinkweed Imp: "Whenever this creature deals combat damage to a creature, destroy that creature"). Self-scoped, no sibling fields — never fires on combat damage to a player, or on noncombat creature damage (fight). Feeds a following `destroy/that_creature` payoff with the damaged creature's id (CR 603.10a last-known information). |
...
| `"this_is_dealt_damage"` | Whenever this permanent **is dealt** damage (CR 119.3, Fungusaur: "Whenever this creature is dealt damage, put a +1/+1 counter on it") — the receiving end, and the only one: `deals_combat_damage_to_creature`, `creature_dealt_damage_by_this_dies`, and `deals_damage_to_opponent` all watch damage this permanent *deals*. Self-scoped and fieldless, firing once per damage event off the shared `Event::DamageMarked` choke, so combat damage, fight damage, and a plain ping all count and the source is invisible. ponytail: the amount isn't on the trigger context — add a slot when a card scales with the damage taken. |
...
| `"becomes_targeted"` | Whenever this permanent becomes the target of a spell (Goldspan Dragon), or — with the sibling `targeted = "creature_you_control"` — whenever any creature its controller controls does (Venerated Rotpriest). (`"becomes_the_target"` is an accepted alias.) |
...
| `"you_proliferate"` | Whenever you proliferate (CR 701.27) — Scheming Aspirant. Fires once per proliferate *instance*, not once per resolution: "proliferate twice"/"proliferate X times" is a set of separate instances, so this fires that many times. Fires unconditionally at the end of each answered instance inside `Game::answer_proliferate` — including when the player proliferates nothing, since CR 701.27 doesn't condition the instance on a nonempty choice. |
...
| `remove_counters_kind` | string | absent | Which counter kind `remove_counters` removes: `"charge"`/`"story"`/`"study"`/`"vow"`/`"minus_one_minus_one"`/`"storage"`/`"corpse"` (Staff of the Storyteller's "remove a story counter"; Scavenging Ghoul's "Remove a corpse counter"; Wickerbough Elder's "Remove a -1/-1 counter"; Fungal Reaches' "Remove X storage counters"). Absent = the +1/+1 path. |
| `remove_counters_x` | bool | `false` | The cost removes `x` counters of `remove_counters_kind` (rather than the fixed `remove_counters` count) — the activator's own chosen X (Fungal Reaches' "Remove X storage counters from this land"). Bounds-checked against the source's actual counter count at activation time, once X is known (CR 602.2b); the existence/legality gate before X is chosen (`ability_activation_gate`) always passes trivially for this shape. Pair with `remove_counters_kind` (required alongside it) and an effect whose own amount is `"x"` (e.g. `add_mana`'s `repeat = "x"`). |
...
| `discard_cost` | u8 | `0` | "Discard a card" as part of the cost (Wild Mongrel's "Discard a card") — the activating intent names this many distinct hand cards to discard, mirroring `sacrifice`'s name-your-payment shape. Paid via the normal discard choke, so "whenever you discard a card" watchers fire. A short/duplicated/not-in-hand choice rejects the activation (CR 602.2b). |
...
| `graveyard_exile_target_count` | u8 | `0` | "Exile N target cards from an opponent's graveyard" as an additional cost (Spurnmage Advocate's "Exile two target cards from an opponent's graveyard") — the ability's own independent **second** target clause (CR 601.2c), distinct from its `target`-bearing effect. Paid after every other cost: the activator pauses on a `ChooseActivationCostTargets` choice naming exactly this many distinct cards from one opponent's graveyard, which are exiled before the ability (with its own effect target) goes on the stack. `0` (the default) is every other pool cost — no untargeted `exile_self`-style flag. |
| `loyalty` | i32 | absent | Present ⇒ planeswalker loyalty ability (e.g. `loyalty = 1`, `-4`); absent ⇒ ordinary activated ability. A **multi-target** loyalty ability (Garruk Wildspeaker's "+1: Untap two target lands", `count = { min = 2, max = 2 }`) can't name its targets on `Intent::ActivateAbility` (one `target` slot), so it pauses on the same counted `ChooseTarget` choice a multi-target triggered ability uses, answered by `Intent::ChooseTargets` — one stack ability per chosen target. |
...
| `only_during_your_turn` | bool | `false` | "Activate only during your turn" (CR 602.5b — Instill Energy's granted `{0}` untap): the mirror of the row below, and the phase-wide relative of `only_during_your_upkeep`'s single step. Not `sorcery_speed`, which also demands an empty stack and a main phase — this one only asks whose turn it is, so it still activates mid-combat. |
| `only_during_opponents_turn` | bool | `false` | "Activate only during an opponent's turn" (CR 602.5b — Nettling Imp): someone other than the activating player must be the active player. Composes with the row below, which is the other half of the same printed sentence. |
| `only_owner_may_activate` | bool | `false` | "Only this creature's owner may activate this ability" (CR 602.5c — Personal Incarnation): the pool's one activation restriction keyed to ownership rather than control. *ponytail:* this only ever **narrows** who may activate. The printed line also widens it — an owner who has lost control may still activate — but CR 602.2's controller check runs first, so a stolen permanent is activatable by nobody rather than by its thief. Pair it with an `approximates` saying so. |
| `only_before_attackers` | bool | `false` | "…before attackers are declared" (CR 602.5b — Nettling Imp): the activated-ability twin of `cast_only_before_attackers` (Master Warcraft) and gated identically — the window runs up to and including the declare-attackers step, and shuts the moment the declaration is actually made rather than at the step boundary. |
| `only_during_your_upkeep` | bool | `false` | "Activate only during your upkeep" (CR 602.5b — Cyclopean Tomb): the tightest window the pool asks for, and the only one keyed to a *step* rather than a phase. Both halves are enforced — the upkeep comes around every turn, and half of them are somebody else's — so this is not `only_before_attackers` with a different step. Sits on `[[abilities]]` itself, not inside `[abilities.activation_cost]`. |
...
[[abilities.effects]]
type = "pump_self_until_end_of_turn"
power = 1
toughness = 1
```
```toml
...
|-------|------|---------|---------|---------|
| `filter` | permanent filter (§7) | matches everything | `you_sacrifice`, `any_player_sacrifices`, `permanent_becomes_tapped`, `permanent_enters(_including_this)` | Which sacrificed/tapped/entering permanent the trigger cares about. `other = true` means "another permanent" — excludes the ability's own source (Mazirek). |
| `for_mana` | bool | `false` | `permanent_becomes_tapped` | Narrows the watch to a land tapped *for mana* (Manabarbs). `false` watches every tap. |
...
| `who` | `"this"` (default), `"your_creatures"`, `"your_creatures_batch"`, `"your_tokens"`, `"any_creature_damaging_your_opponent"` | `"this"` | `deals_combat_damage_to_player` | Whose combat damage counts (Leitmotif Composer / Ohran Frostfang / Curiosity Crafter). `"any_creature_damaging_your_opponent"` is the only scope that reads the *damaged* player: any creature at all, whoever controls it, but only when the damage landed on one of the ability controller's opponents (Edric, Spymaster of Trest's "whenever a creature deals combat damage to one of your opponents"). `"your_creatures_batch"` is the *batch* twin of `"your_creatures"` (CR 603.3b — "whenever **one or more** creatures you control deal combat damage to one or more players", Contaminant Grafter): exactly one trigger per combat damage step no matter how many of your creatures connected, where `"your_creatures"` fires once per connecting creature. |
| `targeted` | `"this"` (default), `"creature_you_control"` | `"this"` | `becomes_targeted` | Which permanent the becomes-the-target watch covers (CR 603.2c). `"this"` is the self-referential default (Goldspan Dragon); `"creature_you_control"` widens it to every creature the ability's controller controls, the source included (Venerated Rotpriest's "whenever a creature you control becomes the target of a spell"). |
...
| `"no_creatures_on_battlefield"` | — "if no creatures are on the battlefield" (Pyrohemia's self-sacrifice gate) — board-wide, every controller, not just yours (unlike `you_control_at_least_creatures` above). Usable both as `[abilities.condition]` (CR 603.4's first check, at trigger placement) and nested in a `{ type = "conditional", … }` step (the CR 603.4 *second* check, re-checked fresh at resolution — a creature created in response to the trigger suppresses the sacrifice), same pairing as `source_untapped`/Howling Mine. |
...
| `"controls_no_lands_with_subtype"` | `subtypes` (array of land-type strings) — "control **no** land whose type line has any of `subtypes`" (Sea Serpent's "when you control no Islands"). The emptiness mirror of `controls_lands_with_subtype` above, which only counts upward — `count = 0` there holds vacuously. Scoped to the ability's controller; an opponent's Island doesn't keep you afloat. |
| `"opponent_controls_lands_with_subtype"` | `subtypes` (array of land-type strings), `count` (u32) — "an opponent controls `count`+ lands whose type line has any of `subtypes`" (Massacre's "if an opponent controls a Plains"). Holds when *some* living opponent individually meets the threshold — the opponent-scoped twin of `controls_lands_with_subtype` above. |
...
| `"an_opponent_controls_lands"` | `at_least` (u32) — "an opponent controls `at_least`+ lands" (Avatar of Fury's cost reduction: "If an opponent controls seven or more lands, this spell costs {6} less to cast"). Holds when *some* living opponent individually meets the threshold — the plain-land-count twin of `opponent_controls_lands_with_subtype` above, not summed across opponents like `opponents_control_lands`. |
| `"you_have_opponents"` | `at_least` (u32) — "you have `at_least`+ opponents" (Undergrowth Stadium's "This land enters tapped unless you have two or more opponents"). Every other player at the table is an opponent (CR 102.3), but an eliminated seat no longer counts (CR 800.4a) — counts living seats, not the table's starting size, so a four-player game gone to heads-up reads *one* opponent. |
...
| `"you_control_lands_at_most"` | `at_most` (u32) — "you control N or fewer lands" (Edge of Autumn's "If you control four or fewer lands, search…"), the `at_most` twin of `you_control_lands` above. Only reachable inside `{ type = "conditional", … }` (a spell's own resolve-time check) — no pool card needs it as an intervening-if or activation restriction yet. |
...
| `"source_attacked_this_turn"` | — "has indestructible as long as it attacked this turn" (CR 508.1; Agent Frank Horrigan's `conditional_keywords` grant). Reads the source's turn-scoped `Permanent::attacked_this_turn` flag, set when it's declared an attacker and cleared for every battlefield permanent at the next Untap step — distinct from a `filter = "attacking"` read, which lapses the instant the creature leaves combat or the turn's combat ends; this holds through the rest of the turn. Only wired into the `conditional_keywords` static-keyword scanner (`characteristics.rs`), not the general `[abilities.condition]`/`{ type = "conditional", … }` evaluator — no pool card needs it there yet. |
| `"source_has_no_counters_of_kind"` | `kind` (`"charge"`/`"story"`/`"study"`/`"vow"`) — "has no charge counters on it" (Mana Bloom) |
| `"source_attacked_or_blocked_this_combat"` | — "if this creature attacked or blocked this combat" (Clockwork Beast's end-of-combat wind-down). Reads the live combat declarations, not a turn-scoped flag like `"source_attacked_this_turn"` above: *this* combat, and it counts blocking too. Only usable from an `"end_of_combat"` trigger (§5 Timings), which is queued ahead of the combat clear so the declarations are still there; anywhere else it is simply false. |
| `"source_has_counters_of_kind"` | `kind` (any counter kind), `at_least` (u32) — the positive sibling of `source_has_no_counters_of_kind` above, and the named-kind sibling of `source_has_counters`. Living Artifact's upkeep "you may remove a vitality counter from this Aura. If you do, you gain 1 life": as an intervening-if (CR 603.4) the trigger never reaches the stack with nothing to remove, which is what "if you do" comes to when the removal can't fail once a counter is there. |
...
| `"source_untapped"` | — "if [this permanent] is untapped" (Howling Mine's `each_draw_step` intervening-if). Unlike `this_permanent_entered_untapped` (a one-time snapshot of how the permanent entered), this reads the source's *current* tapped state live — usable both as `[abilities.condition]` (CR 603.4's first check, at trigger placement) and nested in a `{ type = "conditional", … }` step (the CR 603.4 *second* check, re-checked fresh at resolution — Howling Mine pairs both so a response that taps it after it triggers still suppresses the draw). |
| `"source_tapped"` | — "if [this permanent] is tapped" (Mana Vault's draw-step ping). The exact negation of `source_untapped` above, spelled as its own condition because `[abilities.condition]` has no negation axis (only a nested `{ type = "conditional", … }` step carries `negate`). Usable in both places its twin is, and pair both the same way — the CR 603.4 first check on the ability, the second nested inside — so a Vault untapped in response to the trigger takes no damage. |
| `"enchanted_permanents_controllers_upkeep"` | — "at the beginning of the upkeep of enchanted <permanent>'s controller" (the 2ed upkeep-tax Aura cycle: Cursed Land, Feedback, Wanderlust, Warp Artifact). Goes on a `timing = "each_upkeep"` ability — the watch fires on every player's upkeep and this narrows it to the one belonging to the Aura's **host's** controller, which is not necessarily the Aura's own (enchant an opponent's land and the tax lands on them). Source-object-based like `source_untapped`: reads the source's attachment, so an Aura that fell off never fires. Only usable as `[abilities.condition]`. Pair with the `damage/to_triggering_player` mode (§6), which addresses the same player. |
| `"chosen_players_upkeep"` | — "at the beginning of the chosen player's upkeep" (Black Vise). Goes on a `timing = "each_upkeep"` ability and narrows the watch to the one upkeep belonging to the opponent this permanent named as it entered — pair it with a `timing = "as_enters"` `choice`/`choose_opponent` step, which is what fills `Permanent::chosen_opponent`. Never holds before that choice is answered. Source-object-based like `enchanted_permanents_controllers_upkeep` above. Only usable as `[abilities.condition]`. Pair with the `damage/to_triggering_player` mode (§6), which addresses the same player. |
| `"enchanted_creature_has_keyword"` | `keyword` (keyword name) — "if enchanted creature has flying" (Earthbind's `timing = "etb"` intervening-if). Source-object-based like `enchanted_permanents_controllers_upkeep` above: reads the Aura's own host and asks after its *effective* keyword set, so a granted flying counts too. An Aura with no host never holds. Only usable as `[abilities.condition]`. Pair with the `pump/enchanted_creature_loses_keywords` mode (§6), which grounds the same host. |
...
| `"cards_in_your_graveyard_at_least"` | `count` (u32) — "`count`+ cards in your graveyard" (Werebear's Threshold: "gets +3/+3 as long as there are seven or more cards in your graveyard"). Every card in the controller's graveyard, any kind. |
| `"creature_cards_above_this_in_graveyard_at_least"` | `count` (u32) — "if this card is in your graveyard with `count` or more creature cards above it" (Nether Shadow). *Positional*: only the cards buried on top of this one count, so a creature card underneath never digs it out. A graveyard's order is arrival order. Source-object-based like `source_has_counters`, so it is only evaluated at trigger placement — pair it with `functions_in_graveyard = true` and a `timing = "upkeep"` ability. |
| `"creature_cards_in_all_graveyards_at_least"` | `count` (u32) — "`count`+ creature cards total in all graveyards" (Avatar of Woe's self cost reduction). Sums creature cards over *every* living player's graveyard, not just the controller's. |
...
| `"source_power_at_most"` | `at_most` (u32) — "if its power is `at_most` or less" (Lily Bowen, Raging Grandma's upkeep gate: "double the number of +1/+1 counters on Lily Bowen if its power is 16 or less"). Reads the source's own live power, not `TriggerContext` — same source-object shape as `source_has_counters`, only reachable inside `{ type = "conditional", … }` (the resolve site special-cases it against its own `source`, not `[abilities.condition]`). |
| `"target_card_owner_graveyard_empty"` | — "if no cards are in that graveyard" (Nezumi Graverobber's flip gate: "{1}{B}: Exile target card from an opponent's graveyard. If no cards are in that graveyard, flip this creature."). Reads the owner of the *resolving effect's own chosen graveyard-card target* (whose owner survives the exile — the moved object still records it) and holds iff that owner's graveyard is now empty. Target-based like `target_power_at_least` — only reachable inside `{ type = "conditional", … }` (an intervening-if has no chosen target yet), and gates a following `flip_source` step in the same `effects` sequence. |
...
| `"an_opponent_has_poison_at_least"` | `at_least` (u32) — the printed **Corrupted** ability word (CR 702.165): "if an opponent has `at_least` or more poison counters". An existential over the ability controller's living opponents, the same shape as `"an_opponent_has_life_at_most"` above; a player who has already lost contributes nothing. Serves both as an intervening-if (Contaminant Grafter's "Corrupted — At the beginning of your end step, if an opponent has three or more poison counters, …") and, on `timing = "activated"`, as an activation restriction (CR 602.5b — Glistening Sphere's "Activate only if an opponent has three or more poison counters"), since `[abilities.condition]` gates both. |
...
| `"lands_played_this_turn_at_least"` | `at_least` (u32) — "it wasn't the first land you played this turn" (Fastbond, `at_least = 2`): the controller has *played* at least that many lands this turn. The play being checked is already counted by the time CR 603.4 runs, so "not the first" is two. Narrower than `land_entered_under_your_control_this_turn` below — a fetched or token land is never *played*. |
...
| `"during_your_turn"` | — "during your turn" (Restless Spire's animated form: "During your turn, this creature has first strike"). Holds iff the controller is the active player right now; re-evaluated live, not cached at animation time. Pair with `anthem_static`'s `self_only = true` to gate a manland's own conditional keyword. Also the DSL's spelling of an activated ability's "Activate only during your turn" restriction (Disrupting Scepter) — a plain `[abilities.condition]` on an `activated` ability is an activation restriction (CR 602.5b), and this is the *turn* axis, deliberately not `sorcery_speed`, which would also forbid combat, the end step, and a non-empty stack. |
| `"during_your_upkeep"` | — "Activate only during your upkeep" (Rock Hydra). The narrower sibling of `"during_your_turn"`: the controller must be the active player *and* the game must be in the upkeep step. Nothing else in the pool restricts an activation to a single step of its controller's turn, so this is one condition rather than a step axis crossed with `"during_your_turn"`. |
| `"during_combat"` | — "Activate only during combat" (Jade Statue). Holds iff the current step is a combat step (CR 506.1 — Beginning of Combat through End of Combat), whoever the active player is: the printed text names no seat, so the Statue animates on an opponent's turn to block. The activated-ability twin of a spell's top-level `cast_only_during_combat` field, and an independent axis from `"during_your_turn"` — combine both in one card only when the text says both. |
| `"won_clash"` | — "if you win the clash" (CR 701.22d; Lash Out's win rider). Reads the resolution-scoped won-the-clash flag a preceding `clash` step (§4) in the same `effects` sequence set — only reachable inside `{ type = "conditional", … }` following a `clash` step, never `[abilities.condition]` (nothing has clashed at trigger-placement time). |
| `"color_was_spent_to_cast_this"` | `color` (a color) — "if `color` was spent to cast this" (CR 106.9; Court Hussar's "unless {W} was spent to cast it" — pair with `negate = true` on the enclosing `conditional`, §6; Firespout's "if {R} was spent to cast this spell" — plain, no `negate`). Reads `Permanent::spent_colors` if the source has already resolved to a permanent, else the still-on-the-stack `Spell::spent_colors` (Firespout is a sorcery, so its own ability resolves while `source` is still a spell) — either way snapshotted from the casting spell's actual mana payment (which colors funded which pips, including the generic and any hybrid pips) — only reachable inside `{ type = "conditional", … }`, not `[abilities.condition]` (same shape as `source_entered_with_x_at_least` above). ponytail: only a literally-colored mana source counts — a dual/filter/"any" credit spent toward a pip isn't attributed to either of its colors (see `ManaPool::colors_spent`'s doc); no pool card's mana base exercises that gap yet. |
| `"cast_from_hand"` | — "if you cast it from your hand" (CR 603.4; Dread Cacodemon's/Reiver Demon's ETB intervening-if). Source-object-based like `source_untapped`: reads `Permanent::cast_from_hand`, copied from `Spell::cast_from_hand` as the spell resolves into a permanent — `false` for anything that entered by any other means (reanimation, flicker, search-to-battlefield, a token, …). Only usable as `[abilities.condition]` — no pool card needs the CR 603.4 *second* check for this one. |
| `"source_activated_this_turn_at_least"` | `at_least` (u32) — "if this ability has been activated `at_least` or more times this turn" (CR 602.2b; Dragon Whelp's "{R}: This creature gets +1/+0 until end of turn. If this ability has been activated four or more times this turn, sacrifice this creature at the beginning of the next end step."). Source-object-based like `source_untapped`: counts this turn's `Event::AbilityActivatedThisTurn` entries for `source` (every activated-ability activation records one — not just an `activate_only_once_each_turn`-capped one — cleared at every untap), so an earlier activation this turn is already counted by the time a later one resolves. Only reachable inside `{ type = "conditional", … }`, not `[abilities.condition]` (same shape as `source_entered_with_x_at_least` above). ponytail: counts every activation of the source *object*, not of the specific ability the condition hangs on (the resolve site has no ability index) — exact for a card whose only activated ability is this one, so don't reach for it on a card with two until the index is threaded through. |
...
|--------|--------|--------|
| `deal_damage` | `amount` (amount), `target`, `count` (target-count, §7 — default one mandatory target; "up to two target" is `{ max = 2 }`, each taking `amount`), `divided` (`false` (default) / `true` / `"evenly"` — one `amount` total split across the chosen targets. `true` is "divided as you choose", ≥1 each: Magma Opus's "divided as you choose among any number of targets" is `target = "any"`, `count = { max = 4 }`, `divided = true` — the targets are whatever `target` admits, so `"any"` lets the split fall on **players and planeswalkers** as well as creatures. `"evenly"` is Fireball's "divided evenly, rounded down", a *computed* split with no choice at all — each target takes `total / target count`, so the remainder is never dealt and 2 among 3 deals nothing), `cant_be_regenerated` (bool, default `false` — CR 701.15d, the damage-side twin of `destroy/target`'s field of the same name: it marks the *damaged creature* "can't be regenerated this turn", so every regeneration shield on it is dead for the rest of the turn, including against a later unrelated destroy), `exile_instead_of_dying` (bool, default `false` — "if it would die this turn, exile it instead": the same dies replacement a finality counter applies, CR 614.12, but nameless and expiring at the next untap step). Disintegrate's "Disintegrate deals X damage to any target. If it's a creature, it can't be regenerated this turn, and if it would die this turn, exile it instead." is `amount = "x"`, `target = "any"`, both riders `true`. ponytail: the riders travel with the damage event, so an infect source or fully prevented damage marks nothing — no pool card creates either case), `gain_life_equal_to_damage` (bool, default `false` — Drain Life's "You gain life equal to the damage dealt, but not more life than the player's life total before the damage was dealt, the planeswalker's loyalty before the damage was dealt, or the creature's toughness": the controller gains the damage this effect *actually* landed, so a prevented, shielded, or protected-from hit feeds nothing, capped at the target's own capacity to absorb it — life total, loyalty, or toughness, read before the damage) | yes |
| `draw_cards` | `count` (amount) | no |
| `may_draw_up_to` | `count` (amount) | no (untargeted; the effect's own controller). The declinable twin of `draw_cards` — a CR 120.4 / 601.2c "may draw up to N cards" (Arcane Denial's "Its controller may draw up to two cards"). Resolution pauses that controller on a `PendingChoice::MayDrawUpTo`/`Intent::ChooseDrawCount` count choice clamped to `0..=count`, then draws exactly the chosen number. Fires at resolution time — usually the `then` of a `schedule_at_next_upkeep` (§delayed) so the choice is offered at the scheduled upkeep. |
| `may_draw_up_to_then_opponent_may_repeat` | `count` (amount) | uses the Sequence's shared target (`TargetSpec::OpponentPlayer` — pair with a preceding `target_player_draws { opponent = true }` step, Trade Secrets: "Target opponent draws two cards, then you draw up to four cards. That opponent may repeat this process as many times as they choose."). Pauses the resolving controller (caster) on a `PendingChoice::TradeSecretsCasterDraw`/`Intent::ChooseDrawCount` count choice clamped to `0..=count`; once answered, pauses the *same target opponent* on a `PendingChoice::TradeSecretsRepeat`/`Intent::AnswerMay` (`yes` re-runs the mandatory two-card draw then this count choice again — self-rescheduling like `dance_with_calamity`'s exile loop; `no` stops). |
| `discard` | `count` (§6), `target_player` (bool, default `false`), `damaged_player` (bool, default `false`), `random` (bool, default `false`), `or_one_matching` (card filter, §7, optional) | no by default (the controller discards — fewer in hand discards the whole hand); `target_player = true` targets a player instead (Prismari Command); `damaged_player = true` hands the discard to the player this ability's source just damaged, for a `deals_damage_to_opponent` watch whose payoff says "**that player** discards" (Hypnotic Specter) rather than the controller (Looter il-Kor, which leaves it unset); `random = true` is "at random" (Hypnotic Specter, Mind Twist's `count = "x"`) — nobody chooses, so the discard raises no pause at all and the cards come off the engine's injected per-op RNG; `or_one_matching` is an escape valve — a single card matching the filter satisfies the whole discard in place of the `count`-card answer (Compulsive Research's "discards two cards unless they discard a land card", `or_one_matching = "land"`); no matching card in hand collapses back to the plain `count`-card discard |
| `put_from_hand_on_top` | `count` (u32) | no (the controller puts `count` cards from their own hand on top of their own library, in an order they choose — Brainstorm's "put two cards from your hand on top of your library in any order"; pauses on a card-pick choice over the whole hand, like `discard`, but the answer is *ordered*: the first-named card ends up on top. Fewer than `count` in hand names the whole hand — same clamp as `discard`; an empty hand is a no-op) |
...
| `deal_damage_to_self` | `amount` (amount) | no (the controller takes real damage — Ashes to Ashes' "Ashes to Ashes deals 5 damage to you"; the fieldless self-targeted twin of `deal_damage`'s `Target::Player` arm, distinct from `lose_life` — CR 120.1 damage, not a life-loss effect, so it emits a damage-dealt marker for damage-triggered watchers/future prevention) |
...
| `deal_damage_to_target_controller` | `amount` (amount) | no (the *shared target creature's* controller takes real damage — Lash Out's win rider, "deals 2 damage to that creature's controller"; the player twin of `gain_life_target_controller`, CR 120.1 damage like `deal_damage_to_self`. Pair with a preceding `deal_damage { target = "creature" }` step in an `effects` sequence — reads that creature's controller via `Object::Moved`, so it still resolves even if the earlier damage killed the creature) |
| `clash` | — | no (Clash with an opponent, CR 701.22 — Lash Out. The ability's controller picks an opponent (the shared opponent chooser, collapsed when there's only one), both reveal the top card of their library (public; an empty library reveals nothing), and each takes a one-card keep-on-top-or-bottom `scry` (an `ArrangeTop` pause — the controller decides first, then the opponent). Sets a resolution-scoped won-the-clash flag — the controller wins iff their revealed card's mana value is strictly greater — read by a following `conditional { condition = { type = "won_clash" } }` step, §5) |
| `schedule_colorless_mana_for_countered_spell_next_main_phase` | — | no (Scattering Stroke's win rider, CR 603.7: schedules a delayed one-shot for the controller's *own* next first main phase (`Main1`, controller-scoped) that adds {C} equal to the shared target spell's printed mana value — "at the beginning of your next main phase, add {C} for each 1 mana in that spell's mana cost." Reads the enclosing sequence's shared target (the just-countered spell) and captures its mana value now as last-known information; place inside a `conditional { condition = { type = "won_clash" } }` step after a `clash` + `counter_target_spell`. ponytail: uses the printed mana value, so an {X} spell's chosen X counts as 0; "your next main phase" is approximated as your next *precombat* main.) |
| `skip_next_untap_opponent_creatures` | — | no (Pollen Lullaby's win rider: "creatures your opponents control don't untap during their controllers' next untap steps." Marks every creature an opponent of the controller controls right now; each mark is consumed the next time that permanent's controller reaches their untap step, whether or not it was tapped. No target; place inside a `conditional { condition = { type = "won_clash" } }` step after a `clash`.) |
| `take_extra_turn` | — | no ("Take an extra turn after this one" — Time Walk, CR 505.6a. Queues one extra turn for the ability's controller, taken as the current turn ends and before the rotation moves on; nothing about the *current* turn changes. Several extra turns created in one turn stack, most recent taken first (CR 500.7), and a queued turn belonging to a player who has since lost is skipped.) |
| `you_lose_the_game` | — | no ("you lose the game" — Lich's dies trigger, and the shortfall arm of its damage tax, CR 104.3b. Eliminates the effect's controller outright rather than taking them to 0 life: nothing about the loss is a life total, so no prevention, protection or life-total static holds it off — including Lich's own `you_dont_lose_at_zero_life`.) |
...
| `add_mana` | `mana` (array of mana symbols, §8), `repeat` (amount, default 1 — adds the batch `repeat` times, e.g. Mana Geyser's `{R}` per tapped opponent land), `identity` (u8, default 0 — commander-color-identity credits, Arcane Signet), `opponent_colors` (u8, default 0 — "any color an opponent's land could produce" credits, Fellwar Stone), `restriction` (§8, default none — "spend this mana only to…", wraps the whole batch, Troyan Gutsy Explorer), `single_color` (bool, default `false` — CR 106.4 "add N mana of any one color": locks every `"any"` credit in `mana` × `repeat` to one color the controller chooses when the ability resolves, pausing on a `ChooseManaColor` choice instead of resolving straight to mana; Lotus Field's three mana, Kami of Whispered Hopes's power-many mana. Only the `mana` field's `"any"` credits are locked — no pool card combines it with `identity`/`opponent_colors`/colored/colorless credits on the same ability), `track_provenance` (bool, default `false` — record each credit this ability produces against its own source, so a later spell-cast payment can fire the source's `spend_mana_to_cast` trigger, "When you spend this mana to cast …"; Study Hall / Path of Ancestry. Cleared with the pool at every step/phase boundary), `target` (§7 target spec, default none — a player this mana ability targets, whose hand size a `"cards_in_target_player_hand"` `repeat` reads; Rousing Refrain's `target = "opponent"`. Only ever set on a `timing = "spell"` ability — an activated mana ability can't target, CR 605.1a), `persist_until_end_of_turn` (bool, default `false` — CR 500.4's "until end of turn, you don't lose this mana as steps and phases end" exception; Rousing Refrain, the only pool card that prints it. The batch still empties at the turn's actual end, CR 514.2 cleanup). Not a TOML field: `recipient` — under an `each_player_first_main_phase` ability, context-filled to the active player whose first main phase this is (Magus of the Vineyard's "**that player** adds {G}{G}"), the same "reads `TriggerContext::active_player`, not this ability's controller" shape as `each_draw_step_player_draws`'s `drawer`; every other `add_mana` ability leaves it `None` and the mana goes to the controller, unchanged | no |
| `lose_all_unspent` | `to_you` (bool, default `false`) | no — reads the enclosing `Sequence`'s shared target player. "That player loses all unspent mana" (Mana Short, Drain Power, and Power Sink's decline penalty): empties that player's pool outright, `persist_until_end_of_turn` credits included — CR 500.4's exception is about *step boundaries*, and a card that says "all" means all. Always the tail of a `Sequence` whose earlier step already named the player (`tap_all_target_player_controls` or `target_player_taps_lands_for_mana`); it has no target of its own and no `target` field. `to_you = true` is Drain Power's "and you add the mana lost this way" — every credit moves to this ability's controller's pool kind for kind, so a dual land's either-credit arrives as flexible as it left |
| `target_player_taps_lands_for_mana` | — (no fields) | intrinsic (target player). "Target player activates a mana ability of each land they control" (Drain Power): walks that player's untapped lands and taps each on their behalf down the same tap-for-mana path their own click takes, so every land-tap watch (Mana Flare, Manabarbs) fires as it would have. A land with no mana ability, or one already tapped, is skipped. ponytail: takes each land's default credit rather than offering a per-land pick — raise a pending choice if a card ever makes the pick matter |
...
| `pump_each_creature_until_end_of_turn` | `power`, `toughness` (amounts), `keywords` (array, default `[]`), `filter` (§7 permanent filter, default: any) | no (board-wide twin of `pump_creatures_you_control_until_end_of_turn` — every creature on the battlefield matching `filter`, regardless of controller; Bladewing the Risen's "{B}{R}: Dragon creatures get +1/+1 until end of turn" is `filter = { subtypes = ["Dragon"] }`) |
...
| `keyword_anthem_static` | `keywords` (array, default `[]`), `filter` (§7 permanent filter, default: any), `all_players` (bool, default `false` — drops the "source's controller controls the candidate" gate, same axis as `anthem_static`'s own `all_players`; Avatar of Slaughter's "**All** creatures have double strike" is `keywords = ["double_strike"]`, `all_players = true`) | no (static twin of `grant_keywords_to_permanents_you_control_until_end_of_turn` — same "you control" scan + filter shape, but read fresh on every keyword recompute instead of resolved once; Sterling Grove's "Other enchantments you control have shroud" is `keywords = ["shroud"]`, `filter = { types = ["enchantment"], other = true }`) |
...
| `set_own_base_pt_from_amount` | `amount` (amount, §6) | no (an **indefinite** — never cleared at cleanup, CR 611.2c — self base-P/T SET (CR 613.3(7b)) sourced from a resolved `amount` rather than a literal `power`/`toughness` pair; both power and toughness are set to the same resolved value. A no-op if the source has already left the battlefield by the time this resolves (CR 608.2c). Trench Gorger's "this creature has base power and base toughness each equal to the number of cards exiled this way" is `amount = "cards_exiled_by_search_this_way"`, paired with a preceding `search_library` step in the same `effects = [...]` sequence — the indefinite, amount-sourced twin of `set_base_pt_target_until_end_of_turn`) |
| `target_becomes_subtypes_while_source_remains` | `target`, `set_subtypes` (array of subtype names) | yes (Gaea's Liege's "target land becomes a Forest until this creature leaves the battlefield" — a CR 613.4 type change whose duration is the *source's* stay on the battlefield, the only one the pool prints. `set_subtypes` replaces the whole land-type line per CR 305.7, so a converted Mountain taps for `{G}` and not `{R}`. Written to the target's `subtypes_set_while_source_remains` and read back only while the source is still a permanent — nothing schedules a cleanup, the read just stops finding a live source, so a bounced or destroyed source frees the land instantly. A no-op on an illegal target (CR 608.2b). Contrast `set_attached_types`, which is an Aura static, and `all_lands_of_type_become`, which is a board-wide sweep) |
| `target_becomes_color` | `target`, `color` (one of `"white"`, `"blue"`, `"black"`, `"red"`, `"green"`) | yes (the lace cycle's "target spell or permanent becomes black" — a CR 613.3c layer-5 colour SET that *replaces* the object's colours rather than adding to them, with **no duration printed at all**, so it lasts as long as the object does. Pair with `target = "spell_or_permanent"`. Writes `Permanent::set_color` or, for a spell still on the stack, `Spell::set_color`, through `Event::ColorSet { until_end_of_turn: false }` — the same slot Wild Mongrel's `set_own_color_until_end_of_turn` writes with `true`, so a second set clobbers the first, which is CR 613.7 timestamp order. The reminder text "its mana symbols remain unchanged" needs no modelling: colours are read off the object, never re-derived from the pips. A no-op if the target already left the stack or the battlefield (CR 608.2b). Deathlace, Chaoslace, Lifelace, Purelace, Thoughtlace) |
| `becomes_copy_of_target` | `target`, `keeps_own_color` (bool, default `false`), `keeps_own_abilities` (bool, default `false`) | yes (Vesuvan Doppelganger's "At the beginning of your upkeep, you may have this creature become a copy of target creature, except it doesn't copy that creature's color and it has this ability", CR 707.2). The **source** permanent is what gets rewritten — `target` names the creature being copied. The two flags mirror the `enter_as_copy` fields of the same name and run through the same synthesizer, so entering as a copy and re-copying later land on exactly the same def; `keeps_own_abilities` re-grants *this* ability (matched by its own effect shape), which is what lets the upkeep repeat forever. Indefinite (`until_eot: false`) — the printed shapeshifter never comes back. A no-op if either permanent has left the battlefield (CR 608.2b). Pair with `optional = true` on the ability for the "you may". |
| `target_becomes_treasure` | `target` | yes (target creature becomes a Treasure artifact and loses all other card types and abilities — CR 613.1d/613.1f type/ability SET, **indefinite** like `set_own_base_pt_from_amount` above, not until-end-of-turn. Mints `Event::BecameCopy { def: becomes_treasure(current_def), until_eot: false, .. }`: `becomes_treasure` keeps the target's own name, mana cost, id, legendary flag and colour (`colors`/`devoid` — a type/ability set never touches colour, CR 613 layer 5) but replaces kind/subtypes/keywords/abilities with the Treasure profile, so the converted permanent is not a token and goes to the graveyard like any other creature, unlike a real "create a Treasure" token. A no-op on an illegal target (CR 608.2b). Vraska, Betrayal's Sting's −2) |
| `animate_self_until_end_of_turn` | `add_types` (type set, default none), `add_subtypes` (array, default `[]`), `base_power`, `base_toughness` (plain `i32`), `keywords` (array, default `[]`), `add_colors` (array of colors, default `[]` — unioned onto `Game::colors_of` while the animation is live, CR 105.2a), `ends_at_end_of_combat` (bool, default `false` — Jade Statue's "becomes a 3/6 Golem artifact creature **until end of combat**": the animation is swept at the End of Combat step instead of surviving to cleanup. Strictly shorter than the default until-end-of-turn window, so a card that prints it can't be authored without this flag — the Statue would otherwise still be a creature in the postcombat main phase. Pair with `condition = { type = "during_combat" }` when the card also restricts *when* it may be activated) | no (manland self-animation: the ability's own source gains `add_types`/`add_subtypes` (CR 613.4), has its base P/T SET to `base_power`/`base_toughness` (613.3(7b)), and gains `keywords`/`add_colors`, all until end of turn — Restless Spire → "becomes a 2/1 blue and red … Elemental creature. It's still a land"; a noncreature land animating into a creature. A *conditional* grant like Restless Spire's "During your turn, this creature has first strike" doesn't belong in `keywords` here — pair a `self_only` `anthem_static` with `condition = { type = "during_your_turn" }` instead, since this effect's own `keywords` are unconditional for the whole until-EOT window) |
...
| `grant_chosen_color_protection_until_end_of_turn` | `target` (§7 target spec) | yes (Mother of Runes's "{T}: Target creature you control gains protection from the color of your choice until end of turn." Sequence it after a `choose_color` step in the same ability — that step stores the color on the ability's own source (`Permanent::chosen_color`), and this step mints a keyword-only `TempBoost` of `protection from <that color>` on the target. The dynamic-scope, targeted twin of `pump_until_end_of_turn`'s fixed `keywords` — the scope isn't known until resolution) |
| `radiance_chosen_color_protection_until_end_of_turn` | `target` (§7 target spec, single creature) | yes (Bathe in Light's old "Radiance" keyword action, CR 105.2/702-era: "Choose a color. Target creature and each other creature that shares a color with it gain protection from the chosen color until end of turn." Sequence it after a `choose_color` step; the batch-capable twin of `grant_chosen_color_protection_until_end_of_turn` right above — same targeting and choice-read shape, but the grant lands on every creature in `Game::radiance_batch(target)` (the target plus every other battlefield creature sharing ≥1 color with it, `Game::colors_of`; a colorless target's batch is itself alone) instead of just the target. The source here is the spell itself, not a permanent, so the chosen color reads back via `Game::chosen_color_of` (`Spell::chosen_color`) rather than `Permanent::chosen_color` directly) |
| `anthem_static` | `power`, `toughness` (amounts, default 0 — may scale off a live count: Storm-Kiln Artist's "+1/+0 for each artifact"), `keywords` (array, default `[]` — Ohran Frostfang's deathtouch), `subtypes` (array, default `[]` — "Spirits you control", Quintorius), `colors` (array of colors, default `[]` — restricts to creatures whose color set intersects it (CR 105.2, `Game::colors_of`); `[]` matches every color; Balefire Liege's "Other **red** creatures you control get +1/+1" is two separate `anthem_static` effects, `colors = ["red"]` and `colors = ["white"]`), `chosen_subtype` (bool, default `false` — restricts to creatures of the source's own as-enters chosen creature type, `Permanent::chosen_subtype`; `None` matches nothing; Patchwork Banner. ANDs with `subtypes` if both set — no pool card combines them), `self_only` (bool — pumps *only* its own source), `exclude_source` (bool, default `false` — CR "**other** … creatures you control": excludes the source from the creatures it buffs, still buffing the rest of the team — the opposite restriction from `self_only`; Balefire Liege/Creakwood Liege), `tokens_only` (bool, default `false` — restricts to token creatures the controller controls, checked against `Permanent::token`; Brudiclad, Telchor Engineer's "Creature tokens you control have haste"), `attacking_only` (bool — "attacking creatures you control"), `blocking_only` (bool — the blocking sibling of `attacking_only`; "blocking creatures you control" — Crescendo of War; checked against [`CombatState::blocks`], any candidate currently blocking some attacker), `untapped_only` (bool, default `false` — restricts to candidates that aren't tapped right now; Castle's "Untapped creatures you control get +0/+2". Live like every other axis here (CR 613.4): the buff falls off the instant a creature taps, and comes back when it untaps), `commander_only` (bool — "commander creatures you control", Guardian Augmenter), `has_counters` (bool, default `false` — "creatures you control with counters on them", Nev the Practical Dean's trample grant; read live off [`Game::has_any_counter`] — +1/+1, every named kind, and the finality counter, CR 122.1's unqualified "counter"), `condition` (optional §5 `Condition`, default none — gates the whole anthem, e.g. "as long as you have the city's blessing", Tendershoot Dryad's Saproling anthem; evaluated against the anthem source's own controller), `from_graveyard` (bool, default `false` — the anthem runs from the source card's **graveyard**, not the battlefield: Anger's "as long as this card is in your graveyard … creatures you control have haste"; also set `functions_in_graveyard = true` on the card, §1. A `false` anthem applies only while the source is a battlefield permanent, a `true` one only while it's a graveyard card, so a card with both a battlefield anthem and a graveyard-functional ability — Vanguard of the Restless — never leaks either across zones), `all_players` (bool, default `false` — drops the "source's controller controls the candidate" gate entirely, so the anthem reaches every creature on the battlefield instead of just its controller's; CR "**all** creatures" — Concordant Crossroads's "All creatures have haste." Every other axis above still applies), `war_choice` (optional bool, default none — restricts to creatures controlled by a player whose own answer for this anthem's own source (`Player::war_choices`, §7 `each_player_chooses_war_or_peace`) matches — CR 614.12 makes the choice per permanent, so a second Archangel's answers never speak for the first's; Archangel of Strife's "Creatures controlled by players who chose war/peace get +3/+0"/"+0/+3" is two `anthem_static` effects, `war_choice = true` and `war_choice = false`, both `all_players = true` since either side's creatures can belong to any player) | no (static; buffs your creatures, or every creature's with `all_players`) |
| `protection_from_chosen_color` | — | no (self-static, `type = "static"` / `mode = "protection_from_chosen_color"`: "This creature has protection from the chosen color" — Voice of All / Chromatic Armor-style. The source's own as-enters chosen color (`Permanent::chosen_color`, set by a paired `timing = "as_enters"` `choose_color`) becomes protection on the source itself, read live at keyword recompute; grants nothing until a color is chosen. The self-static twin of `grant_to_attached`'s `protection_from_chosen_color` Aura axis — same live-off-`chosen_color` scope, but on the source, not an enchanted host) |
| `tapped_for_mana_bonus` | `scope` (`"enchanted_host"` (default) — only the land this Aura is attached to, Fertile Ground's "enchanted land"; or `"controller"` — every land the watcher's controller taps for mana, Mirari's Wake's "whenever you tap a land for mana"; or `{ any_land = { <permanent filter, §7> } }` — every land on the battlefield matching the filter whoever taps it, Mana Flare's "whenever a player taps a land for mana" (`{ types = "land" }`) and Gauntlet of Might's "whenever a Mountain is tapped for mana" (`subtypes = ["Mountain"]`, matched against the effective type line so a dual counts, CR 305.6); the bonus still lands in the **tapping** player's pool, not the watcher's), `bonus_color` (`"any_color"` (default) — "one mana of any color", pauses on a `ChooseManaColor` choice, Fertile Ground; or `"produced"` — "one mana of any type that land produced", copies the type the tap just made, no pause; or `{ fixed = "<color>" }` — one named color whatever the land produced, Wild Growth's "an additional {G}", credited inline since there is nothing to choose) | no (static watch, CR 605.3 — read at the land-tap-for-mana choke, `Game::land_tapped_for_mana`, never resolved off the stack. Fires the instant a **land** is tapped for mana, at both chokes — the `produces` sugar tap and an `add_mana` land activation; a non-land mana source (Sol Ring) fires nothing. Mana abilities don't stack, so the bonus resolves inline into the tap's own pool batch. ponytail: only the first `enchanted_host` Aura on a land raises its "any color" pause — a second is dropped; and a `single_color` land, e.g. Lotus Field, taps before the watch, so a Fertile Ground on one fires nothing. No pool land needs either.) |
| `grant_activated_ability` | `filter` (§7), `granted_ability` (a `{ cost = <activation-cost table>, effects = [...] }` sub-table, the same shape `grant_to_attached`'s `granted_ability` uses) | no (static: every permanent matching `filter`, anywhere on the battlefield and under any controller unless `filter.controller` says otherwise, gains the activated ability — Zombie Master's "Other Zombies have '{B}: Regenerate this permanent.'". The filter-scoped twin of `grant_to_attached`'s `granted_ability` (which reaches only a host the source is attached to) and the non-mana twin of `grant_mana_ability`; all three surface on the host past its own abilities, so a granted `target = "this"` names the *host*, not the granting source. `filter.other` is how "**other** Zombies" excludes the lord itself, and `filter`'s `controller = "you"` is read relative to the **granting** permanent's controller. Only activated grants are read here — spell a granted *trigger* with `grant_to_attached`) |
| `grant_mana_ability` | `filter` (§7), `cost` (an activation-cost table: `taps_self`, `sacrifice`, …), `mana` (array of mana symbols), `restriction` (§8, default none — Galazeth Prismari), `single_color` (bool, default `false` — CR 106.4 "add N mana of any one color": the granted ability locks every `"any"` credit in `mana` to one color the controller chooses on activation, pausing on `ChooseManaColor`; Goldspan Dragon's Treasures' "two mana of any one color". The granted twin of `add_mana`'s own `single_color`) | no (static: matching permanents you control gain the activated mana ability — Goldspan Dragon's Treasures) |
...
| `you_dont_lose_at_zero_life` | — | no (static — Lich's "You don't lose the game for having 0 or less life": CR 704.5a's exemption, read off every permanent its controller controls, so it lasts exactly as long as the permanent printing it does. Waives *only* the life-total clause — an empty-library draw, ten poison and lethal commander damage all still eliminate its controller, as does a plain `you_lose_the_game`.) |
| `life_gain_becomes_draw` | — | no (static — Lich's "If you would gain life, draw that many cards instead": a CR 614 replacement, so the life never arrives at all and nothing watching life gain — `"life_gained_this_turn"`, a `you_gained_life_this_turn` condition, a lifegain trigger — sees anything. Applied at the single funnel every life gain passes through, which is what makes it cover lifelink and drains as well as a printed "you gain N life". The replacement cards are drawn as an ordinary draw, dredge and all.) |
| `discard_to_library_top_instead` | — | no (static, Library of Leng's "If an effect causes you to discard a card, discard it, but you may put it on top of your library instead of into your graveyard" (CR 701.8c) — a destination replacement `Game::discard_ids` consults for the discarding player, so the card is still *discarded* and every "whenever you discard" watcher still fires. Cleanup-step hand-size trims and discard *costs* aren't effect discards and are not replaced; neither routes through `discard_ids`. ponytail: the printed "you *may*" is taken as always-yes — no pause can be raised mid-discard until the discard path is resumable) |
| `opponents_cant_search_libraries` | — | no (static — Stranglehold's "Your opponents can't search libraries" (CR 701.19). Checked at the single choke every library search raises through, `Game::opponent_search_denied` scanning for a live opposing `Timing::Static` instance of this effect. A denied search never raises a `PendingChoice` at all — no shuffle either, since the search and its tied shuffle (CR 701.19f) are one instruction. An `AllPlayers` fan-out (Veteran Explorer) that hits a denied seat walks straight to the next queued player instead of dropping the rest of the fan-out, via `Game::continue_search_fanout`) |
...
| `prevent_damage_to_self_removing_counter` | — | no (static, CR 615 — Phantom Centaur: "If damage would be dealt to Phantom Centaur, prevent that damage. Remove a +1/+1 counter from Phantom Centaur." Self-only (unlike `prevent_noncombat_damage_to_other_creatures_you_control`'s "other creatures"), and prevents ALL damage including combat (unlike that static's noncombat-only scope). Scanned by `Game::phantom_shield_active` at every creature-damage choke (combat, effect damage, fight damage); each prevented damage-dealing event removes one +1/+1 counter from the source via `Game::phantom_shield_counter_removal` — a no-op guard, not an error, when none remain) |
| `prevent_damage_to_self_removing_counter_per_point` | — | no (static, CR 615 — Rock Hydra: "For each 1 damage that would be dealt to this creature, if it has a +1/+1 counter on it, remove a +1/+1 counter from it and prevent that 1 damage." Worded **per point** rather than per event, which is the whole difference from its two siblings above: it covers `min(damage, counters present)` points, removes that many +1/+1 counters, and lets whatever it can't pay for be dealt for real — a 5-counter Hydra takes nothing from a Lightning Bolt and loses three counters, a 1-counter Hydra loses its counter and takes 2. Because a leftover has to survive the shield, it is deliberately *not* part of `Game::phantom_shield_active` (which short-circuits a whole damage event at each choke); it is spent inside `Game::per_point_counter_shield`, ahead of the ordinary `prevent_next_damage` shields in the shared creature-damage path. Consequence: unlike its siblings it is skipped by the hand-rolled Radiance sweep, which bypasses every prevention shield) |
| `prevent_damage_to_self_removing_counters_giving_rad` | — | no (static, CR 615 — Bloatfly Swarm: "If damage would be dealt to this creature while it has a +1/+1 counter on it, prevent that damage, remove that many +1/+1 counters from it, then give each player a rad counter for each +1/+1 counter removed this way." The scaling sibling of `prevent_damage_to_self_removing_counter`: also self-only and covers combat and noncombat damage alike, scanned by the same `Game::phantom_shield_active`, but `Game::phantom_shield_counter_removal` removes `min(damage, counters present)` instead of a flat one, then gives every player — CR 102.1's "each," the controller included — a rad counter (`PlayerCounterKind::Rad`) per counter actually removed. No counters on it and the predicate is false — the damage lands normally) |
| `play_any_number_of_lands` | — | no (static, CR 305.2 — Fastbond's "You may play any number of lands on each of your turns": lifts the one-land-per-turn cap for its controller. Read by `Game::land_drop_available`, the single gate both the `Intent::PlayLand` legality check and the playability hint route through. ponytail: fieldless — unlimited is the only permission the pool prints; add a `count` when an Exploration-style "play an additional land" lands) |
| `play_from_graveyard_once_per_turn` | — | no (static, CR 118.9 — Serra Paragon: once during each of the controller's turns, they may play a land or cast a permanent spell with mana value 3 or less from their own graveyard, capped by the per-turn `graveyard_play_used_this_turn` flag; a permanent played/cast this way gains "when it's put into a graveyard from the battlefield, exile it and its controller gains 2 life" — a real CR 603.6 placed trigger, fabricated by `Game::enqueue_triggers` off the genuine `Event::MovedToGraveyard` once the tagged permanent actually dies; see `exile_graveyard_object_gain_life`) |
| `redirect_unblocked_damage_to_self` | — | no (Veteran Bodyguard, CR 615.10 — "as long as this creature is untapped, all damage that would be dealt to you by unblocked creatures is dealt to this creature instead." A *static* redirection, not a shield: nothing is armed and nothing is spent, and the untapped condition is read live at damage time, so a bodyguard tapped after blockers were declared protects nothing. `Game::unblocked_damage_bodyguard` finds the first untapped permanent its controller has with this static; the redirect itself sits in `Game::combat_damage_substep`, the one place that knows an attacker went unblocked — a blocked attacker's trample leftover is not moved. *ponytail:* only combat damage is moved; the printed line also covers noncombat damage an unblocked creature deals, which no pool card creates, and the upgrade path is a per-turn "went unblocked" set the general damage choke could read) |
...
| `spend_mana_as_though_another_color` | `from` (color), `to` (color) | no (static, CR 609.4b — Sunglasses of Urza's "You may spend white mana as though it were red mana": while this permanent's controller pays any cost, each of their `from` credits may pay a `to` pip *as well as* its own. `Game::mana_substitutions` gathers every one the payer controls and `ManaPool::substituted` widens each mono `from` credit into an `of_colors` credit over `from` plus every colour it may be spent as — widening, not recolouring, is the "may". Applied at all three payment chokes: `Game::plan_payment` (which maps the plan back onto real credits, so `Event::ManaSpent` never names mana the pool lacks), `Game::plan_auto_taps` (so a Plains is auto-tapped for `{R}`), and `Game::available_mana` (so playability and the `{X}` ceiling agree). ponytail: a `restricted` credit's base colour isn't widened — no card prints both) |
| `tax_spell_cost` | `amount` (amount), `filter` (spell filter, §7) | no (static, CR 601.2f — Gloom's "White spells cost {3} more to cast": `filter = { color = "white" }`. The sign-flipped twin of `reduce_spell_cost` above and deliberately a *separate* mode rather than a negative amount on that one, because the two differ in **scope** — a reducer only ever discounts its own controller's spells (`Game::cost_reduction` skips every other seat's permanents), while a taxer reaches the whole table (`Game::cost_increase` scans every battlefield). Folded into `[cost].generic` at the cast choke ahead of the reduction, per CR 601.2f's increases-then-reductions order, so a reducer can't shave the tax off a cost it would otherwise have floored at 0 first) |
| `tax_activated_ability` | `amount` (amount), `filter` (§7 permanent filter) | no (static, CR 602.2b — Gloom's "Activated abilities of white enchantments cost {3} more to activate": `filter = { types = "enchantment", color = "white" }`. The activation-choke twin of `tax_spell_cost`, table-wide for the same reason, with `filter` matched against the ability's **source permanent** rather than a spell — leave its `controller` axis at the default `"any"` or the tax would only bite its own controller's enchantments. `Game::activation_tax` is folded into `ActivationCost::mana.generic` inside `Game::ability_activation_gate`, the single gate the activation itself, the priority scans, and the playability previews all route through, so no caller quotes the printed cost back untaxed) |
...
| `cant_attack_unless_defender_controls` | `filter` (§7 permanent filter) | no (static attack restriction, CR 509.1a — Sea Serpent's "This creature can't attack unless defending player controls an Island" is `filter = { types = "land", subtypes = ["Island"] }`. The mirror of `cant_be_attacked_by` below: this one rides the **attacker**, and is satisfied when the defending player controls at least one matching permanent. Give the filter no `controller` axis — `Game::may_attack_defender` already scans that player's battlefield and nobody else's, so a `controller = "you"` here would read the wrong seat. Checked per declared (attacker, defender) pair, which at a pod makes the same creature legal against one seat and illegal against the next, and again in `Game::can_attack` so a creature with no open seat isn't "able" and goad can't demand the attack) |
...
| `may_skip_draw_for_cant_be_attacked_by` | `filter` (§7 permanent filter) | no (Island Sanctuary's "If you would draw a card during your draw step, instead you may skip that draw. If you do, until your next turn, you can't be attacked except by creatures with flying and/or islandwalk." An optional replacement on the draw-step draw (CR 614) that buys a temporary copy of `cant_be_attacked_by` above. Author `filter` as the **banned** set, the same polarity that static uses — invert the printed "except by" yourself, so the exempt keywords become `without_flying = true, without_keyword = { landwalk = "island" }`. The offer is raised at the draw step before dredge's own replacement and declining falls through to it; accepting arms a per-player shield that lapses at that player's *own* next untap, which is what "until your next turn" means) |
| `cant_block_filter` | `filter` (§7 permanent filter) | no (static block restriction, CR 509.1a — Razorjaw Oni's "Black creatures can't block": `filter = { color = "black" }`. Unlike `cant_be_attacked_by`, which is scoped to the ability's own controller (only their permanents are protected), this reaches every matching creature battlefield-wide regardless of who controls the source or the would-be blocker — `Game::cant_block_filter` scans every permanent's `functional_abilities` and is consulted directly in `Game::can_block`) |
| `doesnt_untap` | `self_only` (bool, default `false`), `filter` (§7 permanent filter, default matches everything) | no (static untap-step exception, CR 502.2 — read by `Game::doesnt_untap`, which the untap step's turn-based action consults before it untaps anything, and by nothing else: a permanent held down here still untaps from an ordinary untap *effect*, which is exactly how Basalt Monolith's "{3}: Untap this artifact" frees it. `self_only = true` is the printed-on-itself form (Mana Vault, Basalt Monolith) and ignores `filter` entirely. Otherwise `filter` is matched against every permanent about to untap — leave its `controller` axis alone, because the default `"any"` is what makes Meekstone's "their controllers' untap steps" reach every seat at the table) |
| `players_skip_untap_steps` | — | no (static, CR 703.4a — Stasis's "Players skip their untap steps". Read by `Game::players_skip_untap_steps`, which gates the untap step's two turn-based actions (phasing in, CR 502.1; untapping, CR 502.2) and nothing else. Fieldless and unscoped: "players" is everyone including its own controller, so there is no `all_players` flag and no filter. Reach for this over a table-wide `doesnt_untap` above whenever the card *skips the step* rather than holding permanents down inside one — the difference is visible, because a skipped step also declines to phase anything in and declines to consume a "skip your next untap step" mark. Losing summoning sickness and refreshing loyalty are per-turn durations, not untap-step actions, so they still happen under this) |
| `untap_at_most_one` | `filter` (§7 permanent filter, default matches everything) | no (static untap-step cap, CR 502.2 — Smoke's "Players can't untap more than one creature during their untap steps", Winter Orb's land twin. Read by `Game::untap_at_most_one_filters`, which the untap step consults while building its untap set: the permanents matching `filter` that were about to untap are offered through the same `decline_untap` pause `may_choose_not_to_untap` raises, and an answer letting two of one group up is rejected — a ceiling, not a quota, so keeping every one of them tapped is legal. Distinct from `doesnt_untap` above, which holds a named class down entirely rather than making the player pick one; and from `players_skip_untap_steps`, which deletes the step. Leave the filter's `controller` axis alone: the default `"any"` is what makes both cards symmetrical ("players"). An "as long as …" gate belongs on the ability, not the filter — Winter Orb is `condition = { type = "source_untapped" }`, read once as the untap step starts, which is why an Orb tapped down in response untaps alongside the lands without stopping any of them) |
| `may_skip_turn_while_tapped` | *(none)* | no (static turn replacement, CR 614 — Time Vault's "If you would begin your turn while this artifact is tapped, you may skip that turn instead. If you do, untap this artifact." Read by `Game::may_skip_turn_offer`, which `advance_step` consults as a new turn's untap step begins and *before* any turn-based action runs: a "yes" untaps the source and hands the turn straight on to the next player, a "no" runs that untap step and takes the turn with the source still tapped. Fieldless on purpose — "your turn" is the source's controller's and "this artifact" is the source, so there is nothing for a filter to say. The only thing in the pool that undoes a `doesnt_untap` on its own card; pair the two on one card and keep them separate abilities, one per printed sentence) |
| `cant_be_blocked_by` | `filter` (§7 permanent filter) | no (static block restriction, CR 509.1b — Juggernaut's "This creature can't be blocked by Walls": `filter = { types = "creature", subtypes = ["Wall"] }`. The filter always names the blockers **turned away**, so an "except by X" clause is authored *inverted* — Invisibility's "can't be blocked except by Walls" is `exclude_subtypes = ["Wall"]`, not a positive Wall filter. Printed on the attacker here; the Aura-granted form is `grant_to_attached`'s `cant_be_blocked_by` (§9). Read in `Game::can_block`) |
| `cant_block_attackers` | `filter` (§7 permanent filter) | no (static block restriction, CR 509.1b — Ironclaw Orcs' "This creature can't block creatures with power 2 or greater": `filter = { types = "creature", power_min = 2 }`. Printed on the *blocker* but describing the **attacker** — the mirror of `cant_block_filter` below, which matches the would-be blocker and reaches the whole battlefield. This one restrains only the permanent printing it) |
| `can_block_additional` | `count` (u8) | no (static, CR 509.1b — Two-Headed Giant of Foriys' "This creature can block an additional creature each combat": raises this permanent's own block ceiling from the default one attacker to `1 + count`. Every creature is capped at one block without this; the ceiling is decided in `Game::max_blocks` and enforced across the whole declaration in `Game::declare_blockers`) |
| `cant_cast_during_combat` (`type = "static"`) | — | no (static, Basandra, Battle Seraph's "Players can't cast spells during combat": a plain live existence check (`Game::cant_cast_during_combat`, mirroring `must_attack_each_combat`'s no-source-controller-gate shape) consulted at the top of `Game::cast_timing_ok` — global and absolute, reaching every player including the ability's own controller, and checked ahead of the ordinary instant-speed/flash bypass so it can't be routed around) |
| `cant_attack_if_cast_this_turn` (`type = "static"`) | — | no (static, Angelic Arbiter's "Each opponent who cast a spell this turn can't attack with creatures": a blanket per-player attack ban, unlike `cant_be_attacked_by`'s defender-scoped filter — the gated player can't declare *any* attacker at all. Checked in `Game::cant_attack_if_cast_this_turn` (`combat.rs`) against `Player::spells_cast_this_turn`, only against a static controlled by someone other than the declaring player — CR "opponent") |
| `cant_cast_if_attacked_this_turn` (`type = "static"`) | — | no (static, Angelic Arbiter's "Each opponent who attacked with a creature this turn can't cast spells": the mirror of `cant_attack_if_cast_this_turn` — a blanket per-player cast ban. Checked in `Game::cant_cast_if_attacked_this_turn` (`playable.rs`) against the turn-scoped `Player::attacked_this_turn` flag (set by `Event::AttackerDeclared`, reset each turn at untap), only against a static controlled by someone other than the casting player, ahead of the ordinary instant-speed/flash bypass so it can't be routed around) |
| `blocks_each_attacker_if_able` (`type = "misc"`) | `target` (§7, default `TargetSpec::Creature`; Blaze of Glory narrows it to `{ permanent = { types = "creature", controller = "defending_player" } }`) | yes (Blaze of Glory, CR 509.1b + 509.1c — "Target creature defending player controls can block any number of creatures this turn. It blocks each attacking creature this turn if able." Both halves of one printed sentence, so one effect: the block ceiling comes off (`CombatExtras::may_block_any_number`, which makes `Game::max_blocks` return no ceiling for that creature) and the requirement goes on (`CombatExtras::must_block_all`, gathered by `Game::required_blocks` beside Lure's static). Nothing is minted — this is turn-scoped combat bookkeeping written straight, the same as `prevent_all_combat_damage_this_turn` below, and both halves expire at the next untap. Pair it with the card-level `cast_only_during_combat` + `cast_only_before_blockers` window (§1), which is the rest of Blaze of Glory's text) |
| `must_attack_target` (`type = "misc"`) | `target` (§7, default `TargetSpec::Creature` — Basandra, Battle Seraph's unqualified "target creature", so most cards need no TOML field; author it to narrow the clause, as Nettling Imp's "target non-Wall creature the active player has controlled continuously since the beginning of the turn" does) | yes (Basandra, Battle Seraph's `{R}:` ability, CR 508.1a "if able" — "Target creature attacks this turn if able." Reuses the same `Event::MustAttackDeclared` / `Game::declare_attackers` `must_attack` loop as `must_attack_random_opponent`, but records the target's own controller as the `defender` — a sentinel `declare_attackers` already reads as "must attack, any legal defender" (its `required_legal` gate short-circuits whenever the recorded defender equals the attacking creature's own controller), so no specific required opponent is ever named) |
| `must_attack_all` (`type = "misc"`) | `filter` (§7 permanent filter, required) | no (Siren's Call, CR 508.1a "if able" — "Creatures the active player controls attack this turn if able." `must_attack_target`'s clause over a board scan instead of one chosen creature, minting the same `Event::MustAttackDeclared` per match with the same own-controller "any legal defender" sentinel. The set is fixed as this resolves — a creature that arrives afterwards was never named) |
| `must_attack_random_opponent` | — | no (Ruhan of the Fomori, CR 508.1a "if able": "Choose an opponent at random. Ruhan attacks that player this combat if able." Picks a living opponent of the ability's controller uniformly via the injected RNG, then records a this-turn `Event::MustAttackDeclared` naming the ability's own source — read back by `Game::declare_attackers`'s `must_attack` loop, the same requirement `create_token`'s `must_attack_defender` populates for a token. A no-op with no living opponents) |
| `must_attack_each_combat` (`type = "static"`) | `self_only` (bool, default `false` — Juggernaut's "**This creature** attacks each combat if able" compels only the permanent printing it; the default board-wide reading is Avatar of Slaughter's. The same split `doesnt_untap` makes) | no (static, CR 508.1a "if able" — Avatar of Slaughter's "attack each combat if able": unlike `must_attack_random_opponent`'s recorded per-combat `Event::MustAttackDeclared`, this is a plain live existence check (`Game::must_attack_each_combat`, no per-source controller gate — reaches every creature by default, mirroring `keyword_anthem_static`'s `all_players`; `self_only = true` narrows it to the source) consulted in a dedicated loop in `Game::declare_attackers`: every creature the active player controls that could attack must be declared, against **any** legal defender — no specific opponent is named, so it never conflicts with a recorded `must_attack` requirement the way two *different* required opponents would) |
| `you_choose_which_creatures_attack` | — | no (Master Warcraft, CR 508.1a: "You choose which creatures attack this turn." Records this spell's controller as the seat that makes the attack declaration in the active player's place, read through `Game::attack_declarer` — the creatures on offer are still the *active player's*, and every attack legality check is unchanged; only the seat submitting `Intent::DeclareAttackers` moves. Falls back to the active player if the chosen seat has lost (CR 104.3a). Expires at the next untap step, the same "this turn" boundary the combat-damage shields use.) |
| `you_choose_which_creatures_block` | — | no (Master Warcraft, CR 509.1a: "You choose which creatures block this turn and how those creatures block." The block twin of `you_choose_which_creatures_attack`, read through `Game::block_declarer` — but there is one declaration per attacked player to displace, so the overridden declaration is a *single* `Intent::DeclareBlockers` covering every attacked seat at once, and each blocker is still checked against its own controller's restrictions (`Game::can_block`). Same loss fallback and same turn boundary.) |
...
| `offer_prevention_top_up` | `cost` (§5 cost table), `amount` (int) | yes (Guardian Angel, CR 615 — the *offer* counterpart of `prevent_next_damage`: "Until end of turn, you may pay {1} any time you could cast an instant. If you do, prevent the next 1 damage that would be dealt to that permanent or player this turn." Nothing is prevented on resolution; it records a standing offer on `Game::standing_preventions` that is enumerated for its controller as a `MeaningfulAction::PayStandingPrevention` wherever the priority path already lists activatable abilities, so "any time you could cast an instant", affordability and auto-tap all come from the existing action plumbing. Each payment mints an ordinary `amount`-point `Game::damage_prevention_shields` entry, and the offer survives being taken — it can be bought as often as the controller can pay, until "until end of turn" clears it at the same next-untap boundary an unspent shield expires at. Takes **no** `target`: "that permanent or player" is the target the enclosing effects array already chose for the preceding `prevent_next_damage` step, which a `Sequence` shares across its steps — so this mode is only ever authored beside one) |
| `prevent_next_damage` | `amount` (§6, **optional**), `target` (§7, default none), `from_color` (§7 color, default `"any"`), `gain_life` (bool, default `false`), `redirect_to_controller` (bool, default `false`), `shield_source` (bool, default `false`), `all_but` (§6, **optional**), `target_is_source` (bool, default `false`), `combat_only` (bool, default `false`) | yes (Healing Salve / Samite Healer / Conservator / the Circle of Protection cycle / Reverse Damage, CR 615 — the *consumable* counterpart of the all-or-nothing shields above: "Prevent the next N damage that would be dealt to any target this turn" pushes an `amount`-point entry onto `Game::damage_prevention_shields` keyed to the chosen target, spent point-for-point at the two damage chokes `Game::creature_damage_events` and `Game::player_damage_events` — so combat *and* noncombat damage alike. Damage a shield covers entirely was never dealt: no `DamageMarked`, no life loss, no lifelink, no damage watch. Each spend emits `Event::DamagePrevented` and decrements the entry in `apply`. Omit `target` for "dealt to you" (Conservator), which shields the ability's controller and takes no target at all. **Omit `amount`** for "prevent *that* damage" (the Circles, Reverse Damage): the shield has no point total and is spent outright on the whole of the next qualifying hit, however big. `from_color` gates which sources it stops (CR 105.2a — "a **black** source of your choice"); the shield rechecks the source's live colors at the choke, so a source that changed color between arming and striking is judged as it strikes. `gain_life` is Reverse Damage's "you gain life equal to the damage prevented this way" — the shield's own controller gains exactly what the shield ate, minted alongside the `DamagePrevented`. *ponytail:* `from_color` models the color of "a black source of your choice" but not the *choice* — a Circle arms against whichever black source strikes first rather than one picked as the ability resolves, so it cannot be held back for the second of two black hits in a turn; upgrade path is a `PendingChoice::ChooseDamageSource`. `redirect_to_controller` turns the shield into a *redirection* (Jade Monolith, CR 615.10): the bite is not eaten but dealt again to the ability's controller, through their own shields, and cannot be moved a second time (CR 616.1). ponytail: a redirected bite is logged as `Event::DamagePrevented` at the original target plus ordinary damage at the new one, so the log reads "prevented … then dealt" where the card reads "dealt … instead"; upgrade path is an `Event::DamageRedirected { from, to, .. }`. `shield_source` puts the shield on the permanent that armed it — "damage that would be dealt to **this creature**" (Personal Incarnation), which is not a target and which no §7 `target` spec can name; it overrides `target` outright, and pairs with `redirect_to_controller` for the full "…is dealt to its owner instead". `all_but` is Forcefield's "prevent **all but 1** of that damage" (CR 615.4) — the points that get *through* rather than the points stopped, so it is set **instead of** `amount`, and like an `amount`-less shield it is spent outright by the one hit it stood in front of, however big. `target_is_source` swaps what the chosen `target` means: the named permanent is the damage's *source* and the shield stands in front of this ability's own controller ("an unblocked creature of your choice would deal combat damage **to you**"); with it set, a source-shield keyed to one named creature ignores every other source, and an ability that resolves with no `Target::Object` in hand does nothing. `combat_only` narrows the shield to damage dealt in a combat damage step ("would deal **combat** damage"), read off the step rather than the event, since those two steps deal nothing else, CR 510.2) |
...
| `destroy/target` | `target`, `count` (target-count, §7, default `{1, 1}` — Pest Infestation's "up to X target artifacts and/or enchantments" is `count = { min = 0, max = 0, x_scaled = true }`), `cant_be_regenerated` (bool, default `false` — CR 701.15d "It can't be regenerated": turns off any regeneration shield on the target so a shielded creature dies anyway — Rapid Hybridization), `at` (step, default absent — `at = "end"` postpones the destruction to a CR 603.7 delayed triggered ability at the beginning of the next end step, over the object this ability *already* chose: Stone Giant's "Destroy that creature at the beginning of the next end step". Nothing is re-targeted when it fires; the chosen id is baked into a `destroy/that_creature` payload at resolution, and that payload no-ops if the creature has since left the battlefield), `attack_rider` (`"ignore"` (default) / `"only_if_it_attacked"` / `"only_if_it_didnt"` — whether the destruction asks about the creature's attack this turn: Berserk's "destroy that creature **if it attacked this turn**" and Nettling Imp's "**if it didn't attack this turn**". Rides along with `at` into the scheduled payload and is checked when *that* fires, so a creature Berserked in the first main phase and then declared an attacker still dies. Meaningless without `at`) | yes (author as `type = "destroy"`, `mode = "target"`; creature *or* noncreature — see §7) |
...
| `destroy/all` | `filter` (§7 permanent filter), `cant_be_regenerated` (bool, default `false` — CR 701.15d "They can't be regenerated": turns off regeneration shields for this sweep, same rider as `destroy/target`'s — Reiver Demon, Winds of Rath), `at` (step, default absent — `at = "end"` postpones the sweep to a CR 603.7 delayed triggered ability at the beginning of the next end step: Siren's Call's "at the beginning of the next end step, destroy all …". Unlike `destroy/target`'s `at`, **nothing is baked in** when it is scheduled — the same `filter` re-runs over the battlefield when the delayed ability fires, which is the only way an axis like `did_not_attack_this_turn` can read an attack declared after the scheduling) | no (author as `type = "destroy"`, `mode = "all"`; mass destruction, no target) |
...
| `radiance` (author as `type = "damage"`, `mode = "radiance"`) | `amount` (amount), `target` (§7 target spec, single creature) | yes (Cleansing Beam's old "Radiance" keyword action, CR 105.2/702-era: "Radiance — Cleansing Beam deals 2 damage to target creature and each other creature that shares a color with it." `amount` is resolved once, then dealt to every creature in `Game::radiance_batch(target)` (the target plus every other battlefield creature sharing ≥1 color with it, `Game::colors_of`; a colorless target's batch is itself alone) — each creature's own share still independently checks protection/prevention the same way `damage_each_creature`'s sweep does. Shares its batch-expansion helper with `radiance_chosen_color_protection_until_end_of_turn` below — the one place both radiance cards compute the batch) |
...
| `weaken_each_creature` | `power`, `toughness` (amounts), `opponents_only` (bool, default `false` — Doomwake Giant's "creatures your opponents control") | no (each creature gets -power/-toughness EOT; both amounts are resolved once per affected creature relative to *that creature's* controller, so `"controllers_poison_counters"` gives each opponent's creatures a different `-N/-N` — Phyresis Outbreak) |
| `enchanted_creature_loses_keywords` | `keywords` (array) | no (Earthbind's "this Aura gains 'Enchanted creature loses flying'": the Aura's host loses `keywords` for as long as this Aura stays attached to it, with no end-of-turn expiry — the indefinite, Aura-bound sibling of `strip_keywords_from_opponents_creatures` below. The loss is recorded on the **Aura**, not the host, and read through the Aura's live attachment, so it ends by itself when the Aura leaves and follows the Aura to a new host; it is applied to the fully unioned keyword set last, so it beats every grant regardless of source. Nothing happens if the Aura is no longer attached. There is no separate "gains an ability" surface — the gained ability *is* this effect, so gate it with an `[abilities.condition]` such as `enchanted_creature_has_keyword` when the card only grants it conditionally) |
...
| `put_counters` | `count` (amount), `target`, `targets` (target-count, §7 — "on each of up to X target creatures", Silkguard), `kind` (`"charge"`/`"story"`/`"study"`/`"vow"`/`"strife"`/`"age"`/`"corpse"`/`"mire"`/`"vitality"`/`"plus_one_plus_zero"`, default +1/+1 — Crescendo of War's own upkeep counter is `target = "this"`; `age` is placed internally by `cumulative_upkeep`, not authored via `put_counters`; `vitality` is Living Artifact's banked damage — inert bookkeeping, one counter per point dealt to the Aura's controller, spent one per upkeep for 1 life; `mire` is Cyclopean Tomb's "That land is a Swamp for as long as it has a mire counter on it" — the counter *is* the type change, read straight off the land by `Game::effective_subtypes` so it outlives the Tomb, and it replaces the whole land-type line per CR 305.7; `plus_one_plus_zero` is Clockwork Beast's wind-up — a *real* P/T counter, not bookkeeping, read out in layer 7d like `minus_one_minus_one` but power only, which is why it can't ride the +1/+1 path), `divided` (bool, default `false` — `deal_damage`'s `divided` twin: `count` becomes one total split across the chosen `targets`, ≥1 each; Grove's Bounty's "distribute X +1/+1 counters among any number of target creatures you control" is `count = "x"`, `targets = { min = 0, x_scaled = true }`, `divided = true`. Only meaningful with `kind` unset — no pool card divides a named counter kind), `max_total` (u8, default unset — Clockwork Beast's "This ability can't cause the total number of +1/+0 counters on this creature to be greater than seven": a ceiling on the recipient's *total* of this `kind`, so what lands is the room left, not the announced `count`. Named-`kind` arm only; nothing in the pool caps a +1/+1 total) | yes |
...
| `put_counters_each` | `filter` (§7), `count` (amount), `target_player` (bool, default `false`), `kind` (same vocabulary as `put_counters`'s own `kind`, default +1/+1 — Contagion Engine's "-1/-1 counter on each creature target player controls" is `kind = "minus_one_minus_one"`) | no by default (a counter on each permanent matching `filter` from the *controller's* perspective — Mazirek); `target_player = true` reads `filter`'s `you`/`opponent` axis from a chosen Player target instead (Shadrix Silverquill's begin-combat "target player puts a +1/+1 counter on each creature they control") — intrinsic (a player) in that case. A named `kind` bypasses the +1/+1 counter-replacement pipeline entirely (Hardened Scales/Doubling Season don't amplify it — fidelity increment #19 widens that pipeline to other kinds) |
| `put_loyalty_counter_each` | `filter` (§7, permanent filter) | no (the Wolf token's own dies trigger — Garruk, Cursed Huntsman's `0`: "When this token dies, put a loyalty counter on each Garruk you control." Loyalty is the scalar `Permanent::loyalty`, not a `CounterKind`, so this walks `filter` and emits `Event::LoyaltyChanged { amount: 1 }` per match rather than reusing `put_counters_each`'s counter-replacement pipeline; `filter = { types = "planeswalker", subtypes = ["Garruk"], controller = "you" }` names "each Garruk you control" as a permanent-type filter, not the creating walker, so it survives that walker's own death) |
| `put_counters_on_player` | `kind` (player-counter kind — `"poison"` or `"rad"`; a *player*-side vocabulary, disjoint from `put_counters`' permanent `kind` axis, since no permanent counter can sit on a player and no player counter on a permanent), `count` (amount), `scope` (`"all_players"` — "each player gets a poison counter", Ichor Rats; `"each_opponent"` — "each opponent gets a poison counter", Infectious Inquiry / Vraska's Fall; `"targeted_opponent"` — "target opponent gets a poison counter", Venerated Rotpriest; the same `scope` axis `each_player_sacrifices` uses, minus `"targeted_players"`, which no pool card spells for counters) | only under `scope = "targeted_opponent"` (exactly one target opponent, chosen as an ordinary target when the spell/ability goes on the stack, CR 601.2c); every other scope targets nothing. Counters go on the *players* in `scope`, never a permanent — CR 122.1. A player who has already lost gets nothing. Ten or more poison counters lose the game via the CR 704.5c state-based action. `"rad"` (Fallout, Feral Ghoul) has no such threshold — instead a turn-based action at the beginning of each player's precombat main phase mills that many cards, and each *nonland* card milled that way costs its controller 1 life and removes one rad counter; a short library mills only what is there, so the life loss and the removal follow the real nonland count |
| `top_up_counters_on_player` | `kind` (player-counter kind, as `put_counters_on_player`), `to` (u8) | yes (target player). A *top-up*, not a fixed add — Vraska, Betrayal's Sting's "−9: If target player has fewer than nine poison counters, they get a number of poison counters equal to the difference" is `kind = "poison", to = 9`. The number placed is `to` minus what the target already has; a target already at or above `to` gets nothing at all — no counters, no event |
| `remove_all_player_counters` | `scope` (same vocabulary as `put_counters_on_player`'s own `scope`) | only under `scope = "targeted_opponent"`, same as `put_counters_on_player`; every other scope targets nothing. "Each opponent loses all counters" (Final Act) — every counter of every kind on each player in `scope` is removed, not just poison (CR 122.1/121.2); iterates every `PlayerCounterKind`, not just poison. A player who has already lost gets nothing |
| `counter_replacement` | `add` (i32, default 0), `times` (i32, default 1), `halve` (bool, default `false` — "half that many … rounded down"; Vorinclex), `other` (bool, default `false` — "another creature you control", excludes its own source; Benevolent Hydra), `any_kind` (bool, default `false` — "one or more counters" of every kind rather than "+1/+1 counters" only; Winding Constrictor, Doubling Season, Vorinclex), `placer` (`"you"` / `"opponents"`, optional — the CR 614.1 axis of who *would put* the counters, for clauses worded "if **you would put** …" / "if an **opponent would put** …"; Vorinclex's two clauses, Innkeeper's Talent Level 3. When present it **overrides** the default recipient-side gate, so a "you"-placer replacement applies to counters you put on anyone's permanent. Omit it for passive-voice recipient-keyed wordings — "if one or more counters **would be put on** a permanent you control" (Winding Constrictor, Doubling Season) — which gate on the receiving side instead), `recipients` (`"permanents"` (default) / `"players"` / `"permanents_and_players"` — counters sit on permanents and on players, CR 122.1), `filter` (`PermanentFilter`, optional — a type gate on the receiving permanent; Ozolith's "an artifact or creature you control") | no (static; CR 614 counter replacement) |
...
| `enters_with_counters` | `count` (amount), `kind` (`"charge"`/`"story"`/`"study"`/`"vow"`/`"minus_one_minus_one"`/`"plus_one_plus_zero"`, default +1/+1 — Astral Cornucopia's "X charge counters"; Wickerbough Elder's "enters with a -1/-1 counter") | no (static; a -1/-1 counter subtracts 1/1 per counter from the entering creature's power/toughness, the mirror of the +1/+1 path — CR 704.5r's +1/+1 ↔ -1/-1 annihilation SBA isn't modeled, see `characteristics.rs`'s `pt_layers`. Fires both at the cast-resolution choke (`Game::resolve_permanent_enter`) and at the land special action's own ETB site (`Event::LandPlayed`) via the shared `Game::push_enters_with_counters` helper — Vivid Crag's "This land enters ... with two charge counters") |
...
| `proliferate` | `times` (amount, usually `1` or `"x"` — Expansion Algorithm's "Proliferate X times") | no (untargeted; pauses on a select-any-number choice over every battlefield permanent *and every player* that currently has a counter, giving each one more of every kind already there — CR 701.27 "Choose any number of permanents and/or players." Player-side counters (`put_counters_on_player` — poison, rad) and a planeswalker's loyalty (CR 306.5b) are both in the offered set; a card in exile is neither a permanent nor a player, so a suspended card's time counters stay out of reach) |
...
| `remove_all_but_one_plus_one_counter_then_gain_life` | `target` | yes (+1/+1 counters only, off `target`; keeps exactly one — a no-op with zero or one already present — and the controller gains 1 life per counter *actually removed*, not a flat amount; Lily Bowen, Raging Grandma's "Otherwise" half. ponytail: single consumer, +1/+1-only and always "keep one, gain life" — grow a `keep`/`gain_life` rider on `remove_all_counters_then_draw` instead of a new sibling if a future card needs a different keep-count or payoff) |
| `remove_counter_from_self` | `kind` (counter kind, default +1/+1) | no (removes one counter from the ability's own source, a CR 608.2c effect-internal sub-action, not an activation cost — Ingenious Prodigy's "you may remove a +1/+1 counter from it" with `kind` absent, Living Artifact's "remove a vitality counter from this Aura" with `kind = "vitality"`; a no-op with zero on it) |
...
| `create_token_copy` | `count` (amount — copies minted *per chosen target*, e.g. Rite of Replication's `{ if_kicked = 5, else = 1 }`), `target` (`"none"` on a `permanent_enters` ability — Riku of Two Reflections' "create a token that's a copy of *that creature*": untargeted, CR 603.3b's implicit "that permanent" rather than a chosen target; filled from the trigger's own entering permanent at placement, the same `fill_entering_permanent` context-fill `deal_damage_to_entering_permanent`/`attach_self_to_entering` use — every other card keeps an ordinary chosen `target`), `targets` (target count, §9 below, default `{1, 1}` — how many *distinct* targets are chosen, named apart from `count` the same way `put_counters`' `targets` sits apart from its own `count`; Twinflame's "any number of target creatures you control" is `{ strive_scaled = true }` — one copy is minted per chosen target, no special-cased resolution loop needed, the ordinary multi-target step expansion already runs the effect once per target), `sacrifice_at_next_end_step` (bool, default `false` — Determined Iteration: each minted copy is sacrificed at the beginning of the next end step via a delayed trigger, CR 603.7b), `exile_at_next_end_step` (bool, default `false` — Twinflame: "Exile those tokens at the beginning of the next end step," mirroring `create_token`'s own field of the same name; distinct from `sacrifice_at_next_end_step` because exile skips dies-triggers, mutually exclusive in every pool card), `haste` (bool, default `false` — Determined Iteration: "the token created this way gains haste," an until-EOT `Keyword::Haste` grant on each minted token) | yes, except `target = "none"` on a `permanent_enters` ability (Riku of Two Reflections), which reads the entering permanent instead |
...
| `base_power_toughness_from_amount` | `power` (amount), `toughness` (amount) | no (self-static, `type = "static"` / `mode = "base_power_toughness_from_amount"`: a **characteristic-defining ability** (CR 604.3) setting the source's *own* base power and toughness to a count — Nightmare's "power and toughness are each equal to the number of Swamps you control" is `power`/`toughness` both `{ per_permanent = { types = "land", subtypes = ["Swamp"], controller = "you" } }`. Give the card a `[kind]` box of `power = 0` / `toughness = 0`: the printed `*/*` carries no numbers and this ability is the only thing that supplies them. Applied in layer 7a, *before* everything else, so a later base-set Aura (`set_attached_base_p_t` — Darksteel Mutation) still overrides it and counters/anthems still sum on top. Resolved live on every recompute, which is what makes it defining rather than a one-shot: the creature grows the instant a Swamp arrives. Contrast `set_own_base_pt_from_amount`, which *writes* a number once on resolution and never re-reads it. Optional `when` (`"always"` default / `"attacking"` / `"not_attacking"`) gates the count on the source's combat state, so a creature printing two of these gets whichever arm applies right now — Gaea's Liege counts its controller's Forests `when = "not_attacking"` and the defending player's `when = "attacking"`. Author them as two `[[abilities]]`, the way the card prints two sentences, not as one ability with a branching amount) |
| `grant_to_attached` | `power` (amount, default 0), `toughness` (amount, default 0 — Sage's Reverie's "+1/+1 for each Aura you control that's attached to a creature" is `{ per_permanent = { subtypes = ["Aura"], controller = "you" } }`, resolved live off the attached Aura/Equipment), `keywords` (array, default `[]`), `goad` (bool, default `false` — host is goaded while attached; the Impetus cycle), `protection_from_chosen_color` (bool, default `false` — Flickering Ward's "Enchanted creature has protection from the chosen color": grants the host `{ protection = <this Aura's chosen_color> }` read live off the Aura's own `Permanent::chosen_color`, set by a `timing = "as_enters"` `choose_color`; grants nothing until a color is chosen — a card-specific dynamic-scope axis, not a general grant-a-runtime-keyword surface), `granted_ability` (default none — a `{ cost = <activation-cost table>, effects = [...] }` sub-table granting the host an activated ability; Fallen Ideal's "Sacrifice a creature: +2/+1 until end of turn", the non-mana twin of `grant_mana_ability`; or a `{ trigger = <trigger table>, optional = <bool>, cost = { mana = ... }, effects = [...] }` sub-table granting a *triggered* ability instead — Power Fist's "Whenever this creature deals combat damage to a player, put that many +1/+1 counters on it." is `{ trigger = { deals_combat_damage_to_player = { who = "this" } }, effects = [{ type = "counters", mode = "put_counters", target = "this", count = "combat_damage_dealt" }] }`; a triggered grant never occupies an activatable index. On a triggered grant, `optional = true` plus a mana `cost` is the "you may pay" pause an authored optional trigger raises — Farmstead's "Enchanted land has \"At the beginning of your upkeep, you may pay {W}{W}. If you do, you gain 1 life.\"" is `{ trigger = { upkeep = {} }, optional = true, cost = { mana = { white = 2 } }, effects = [{ type = "life", mode = "gain", amount = 1 }] }`; only the mana half of `cost` applies to a triggered grant, since nothing was activated to tap or sacrifice for. Today `deals_combat_damage_to_player` and `upkeep` (fieldless, so `{ upkeep = {} }`; "your" upkeep is the *host's* controller's) are the wired granted trigger flavors), `cant_attack` (bool, default `false` — Faith's Fetters/Prison Term: host can't be declared as an attacker while attached; the reverse of `goad`'s "must attack", read live in `declare_attackers`), `cant_block` (bool, default `false` — the block-legality twin of `cant_attack`, read live in `can_block`), `cant_be_blocked_by` (permanent filter, default none — Invisibility's "Enchanted creature can't be blocked except by Walls": the granted twin of the printed `cant_be_blocked_by` static (§6), authored the same way, with the filter naming the blockers turned away — so "except by Walls" is `{ types = "creature", exclude_subtypes = ["Wall"] }`. Read live in `Game::can_block` off the same attachment scan as `cant_attack`), `must_be_blocked_by_all` (bool, default `false` — Lure's "All creatures able to block enchanted creature do so": a blocking **requirement** (CR 509.1c), not a restriction. Gathered by `Game::required_blocks` and enforced by a second pass in `Game::declare_blockers` — a defender able to block the host may not dodge it by blocking something else, and "able" means an illegal block was never required at all), `activated_abilities` (string enum, default unset — `"none"` (Prison Term: no activated ability of the host's may be activated, mana or not) or `"mana_only"` (Faith's Fetters: every activated ability except a mana ability, CR 605.3a, is banned); read live in `ability_activation_gate`), `cant_attack_controller` (bool, default `false` — the Vow cycle's "Enchanted creature ... can't attack you or planeswalkers you control": the host can't attack *this Aura's own controller* specifically, unlike `cant_attack`'s blanket ban; read live in `declare_attackers` beside the vow-counter (`Permanent::vow_protected`) check via `Game::host_cant_attack_controller`, off the same attachment scan), `cant_be_enchanted` (bool, default `false` — Consecrate Land's "Enchanted land ... can't be enchanted by other Auras": no *other* Aura may attach to the host, so an Aura spell can't target it (CR 303.4a) and an Aura already there is put into its owner's graveyard (CR 704.5n); read live via `Game::host_cant_be_enchanted_by`, which skips the granting Aura itself), `may_attack_ignoring_defender` (bool, default `false` — Animate Wall's "Enchanted Wall can attack as though it didn't have defender": waives the `Keyword::Defender` check in `Game::can_attack` for the host while attached, via `Game::host_may_attack_ignoring_defender` off the same attachment scan as `cant_attack` above. "As though", not a keyword loss — the host still *has* defender, so anything else reading the keyword is unaffected. Pair with a subtype-scoped `enchant` filter when the card says "Enchant Wall"), `may_attack_ignoring_summoning_sickness` (bool, default `false` — Instill Energy's "Enchanted creature can attack as though it had haste": waives the summoning-sickness check in `Game::can_attack` for the host, the sibling of `may_attack_ignoring_defender` just above and read off the same attachment scan. Deliberately *not* `keywords = ["haste"]` — real haste would also free the host's `{T}` abilities, which "as though it had haste" for attacking does not), `doesnt_untap` (bool, default `false` — Paralyze's "Enchanted creature doesn't untap during its controller's untap step": the attachment-scoped form of the battlefield-wide `doesnt_untap` static (§6), whose `filter` names a *class* of permanents and so can't say "the one this Aura is on". Folded into `Game::doesnt_untap` so the untap step keeps reading exactly one scanner, and like the battlefield-wide form it is consulted *only* there — an untap **effect** (Paralyze's own pay-{4}) frees the host regardless), `legendary_only` (bool, default `false` — Champion's Helm's "As long as equipped creature is legendary, it has hexproof": restricts `keywords` only — not `power`/`toughness`/the other axes above — to while the host itself is legendary, read live off the host's own `CardDef::legendary` at keyword recompute) | no (aura/equipment static) |
| `set_attached_base_pt` | `power` (amount), `toughness` (amount — both resolved against the **host**, not the Aura, so `"source_mana_value"` reads the enchanted permanent: Animate Artifact's "power and toughness each equal to its mana value"), `noncreature_only` (bool, default `false` — Animate Artifact's "as long as enchanted artifact *isn't a creature*": the base set applies only while the host is a noncreature, so enchanting an artifact creature leaves its printed P/T alone. Checked against the host's **printed** types; an artifact animated by something else still gets set) | no (aura static; sets the host's *base* P/T while attached — see §9) |
| `all_lands_of_type_become` | `land_types` (array of land types, required — a land carrying any of them is caught; each card in the pool names exactly one), `set_subtypes` (array, default `[]` — when non-empty it *replaces* the caught land's whole subtype line, CR 305.7: Conversion's `land_types = ["Mountain"]`, `set_subtypes = ["Plains"]` costs a Badlands its Swamp half too and leaves it tapping only for `{W}`. Leave it empty for a change that adds something without swapping the type — Kormus Bell's Swamps stay Swamps), `add_types` (type set, default none — card types unioned onto every caught land, `"creature"` for the two animators; "that are still lands" is why this unions rather than replaces), `base_power` / `base_toughness` (i32, default 0 — the layer-7b base P/T a caught land takes on, read only when `add_types` makes it a creature), `add_colors` (array of colors, default `[]` — layer-5 colors unioned on, Kormus Bell's "black"; leave it off and the land animates colorless, Living Lands) | no (static, CR 613.4/613.3 — "All Mountains are Plains" / "All Swamps are 1/1 black creatures that are still lands". The only static scoped by a whole-battlefield sweep rather than by controller (`anthem`) or attachment (`set_attached_types`): "All Mountains" means everyone's. Scoped by land type instead of a §7 permanent filter on purpose — a filter would ask for the candidate's subtypes, which is the answer this effect changes; matching names against the subtype line as it is being built keeps CR 613.4 timestamp order honest and the recursion impossible. `Game::land_type_statics` is the sweep and `Game::land_type_statics_on` the per-permanent handle; `effective_subtypes`, `effective_types`, `pt_layers`, and `colors_of` each consult it, and the mana consequence rides free on `Game::land_mana_credit` — see §9) |
| `set_attached_types` | `add_types` (type set, default none — card types added to the host, Angelic Destiny → Angel; when `set_types = true` this is the host's *complete* card type list, Darksteel Mutation → `["artifact", "creature"]`), `set_types` (bool, default false — CR 613.4 "loses all other card types"; `add_types` *replaces* the host's printed card types instead of being unioned on, so an enchantment creature becomes exactly an artifact creature), `add_subtypes` (array, default `[]` — creature subtypes unioned on, Angelic Destiny → Angel "in addition to its other types"), `set_subtypes` (array, default `[]` — when non-empty, *replaces* the host's subtypes, Darksteel Mutation → `[Insect]`; on a land it replaces the basic land types too, Evil Presence → `[Swamp]`, and the land taps for the new types' mana per CR 305.7 — see §9), `set_chosen_land_type` (bool, default false — Phantasmal Terrain's "enchanted land is *the chosen type*": `set_subtypes` is instead the one basic land type this Aura's controller named as it entered via `choose_basic_land_type` (§8); the whole type change is inert while that choice is unanswered), `lose_all_abilities` (bool, default false — CR 613.1e/701; strips the host's *own* printed abilities + keywords, Darksteel Mutation → "loses all other abilities"; the Aura's own grants survive) | no (aura static; CR 613.4/613.1e type/subtype/ability layer while attached — see §9) |
...
| `target_opponent_gains_control` | `target` (the donated permanent — its first target clause, usually `{ permanent = { controller = "you" } }`, or the fixed `"this"` — Goblin Cadets donates itself, no real first choice), `player` (the recipient's target spec — its second, *independent* target clause, `"opponent"` for Zedruu/Goblin Cadets) | yes (donation, CR 720 — Zedruu the Greathearted's "Target opponent gains control of target permanent you control"; also Goblin Cadets' "blocks or becomes blocked" trigger, `target = "this"`). Two independent target clauses (CR 601.2c): the activator/trigger names the permanent as the ability's own target (a real choice, or `"this"`'s fixed reference), then chooses the recipient player as a second clause (auto-filled when only one opponent is legal) — a fixed first clause still pauses for the second, real one (`Game::place_targeted_ability`'s `ThisPermanent` branch routes through `Game::place_ability_second_clause` rather than placing immediately). Resolves as a permanent control change (no cleanup reversion, like `gain_control`), but hands the permanent to the *chosen player*, not the ability's controller — ownership is untouched (CR 108.3), so the donor keeps counting it for Zedruu's upkeep and it still dies/leaves under its owner. CR 506.4c: any control change removes the permanent from combat (`Game::remove_from_combat`, called from every control-gain apply arm) — Goblin Cadets' own reminder text |
| `exchange_control` | `first` (the "you control" permanent — its first target clause, e.g. `{ permanent = { types = "land", controller = "you" } }`), `second` (the "an opponent controls" permanent — its second, *independent* target clause, e.g. `{ permanent = { types = "creature", controller = "opponent" } }`) | yes (exchange, CR 720 — Vedalken Plotter's "exchange control of target land you control and target land an opponent controls", Chromeshell Crab's creature version). Two independent target clauses (CR 601.2c): the ability names the "you control" permanent as its own target, then chooses the "an opponent controls" permanent as a second clause (auto-filled when only one is legal); the two filters are disjoint (you vs an opponent), so the same permanent can never satisfy both. Resolves as two permanent control changes (no cleanup reversion, like `gain_control`) — each permanent's controller becomes the *other's* prior controller, freshly timestamped so the swap outranks any earlier steal (CR 800.4a). Ownership is untouched (CR 108.3); both permanents must still be on the battlefield or the whole swap is cancelled (CR 608.2b). Rides the same two-clause activated/triggered-ability surface as `target_opponent_gains_control` — author on an ETB (`timing = "etb"`) or a turn-face-up (`timing = "turned_face_up"`) trigger |
| `exchange_all_creatures_until_end_of_turn` | `target` (the "target opponent" — a single `"opponent"` target spec) | yes (mass, two-player until-EOT control exchange, CR 720 — Reins of Power's "Untap all creatures you control and all creatures target opponent controls. You and that opponent each gain control of all creatures the other controls until end of turn. Those creatures gain haste until end of turn"). The mass, two-sided twin of `gain_control_until_end_of_turn`: one target — the opponent — chosen at cast, no second clause (both creature *sets* are computed at resolution, not targeted). Snapshots every creature you control and every creature that opponent controls *before* swapping (so the first steal can't feed the second, CR 800.4a), untaps them all, hands each to the *other* player as a freshly-timestamped until-EOT steal (reverts at cleanup, CR 514.2, to whoever the next-highest source names — a swap layered over a donation reverts to the donated-to controller, not the owner), and grants haste. Ownership is untouched (CR 108.3). A single top-level `[[abilities.effects]]` covers the whole card |
| `gain_control_all_until_end_of_turn` | `filter` (§7 permanent filter, `"creatures"` for Insurrection) | no (untargeted; mass, one-sided, all-creatures-of-any-controller until-EOT control steal, CR 720 — Insurrection's "Untap all creatures and gain control of them until end of turn. They gain haste until end of turn"). The one-sided twin of `exchange_all_creatures_until_end_of_turn`: `filter` is evaluated against every creature on the battlefield regardless of controller, including the caster's own — no `you`/`opponent` scoping (unlike `untap_all`, which is hardcoded to the ability's own controller). Snapshots the matching set, untaps them all, hands each to the ability's controller as a freshly-timestamped until-EOT steal (reverts at cleanup, CR 514.2, to whoever the next-highest source names — a mass steal layered over a donated permanent reverts to the donated-to controller, not the owner, CR 800.4a), and grants haste. Ownership is untouched (CR 108.3). A single top-level `[[abilities.effects]]` covers the whole card |
...
| `exile_self_with_time_counters` | `counters` (u32), `on_expiry` (effects list, default empty) | no — "Exile [this card] with N time counters on it" (CR 702.62, Rousing Refrain): the resolving instant/sorcery exiles *itself* with `counters` time counters (visible in exile) instead of going to the graveyard. Ticked at the owner's upkeep like a `[suspend]` card. When the last counter is removed, an empty `on_expiry` grants the suspend free-cast permission (Rousing Refrain); a non-empty `on_expiry` (All Hallow's Eve's scream-counter self-exile) instead sends the card to its owner's graveyard and resolves those effects — an `[[abilities.effects]]`-shaped list under the same effect table. |
| `drain_target` | `amount` (i32), `opponent` (bool, default `false` — "target opponent") | intrinsic (target player loses / you gain) |
| `target_player_gains_life` | `amount` (§6 amount), `opponent` (bool, default `false` — restrict to "target opponent", Questing Phelddagrif's "Target opponent gains 2 life") | intrinsic (target player, no matching loss — the gain-only twin of `drain_target`). Congregate's "gains 2 life for each creature on the battlefield" is `amount = { times = 2, per = "per_creature_on_battlefield" }` (§6 `Scaled`). |
...
| `source_owner_loses_half_their_life` | — | no — "Its owner loses half their life, rounded up" (Personal Incarnation's dies trigger). The only life loss in the pool billed to the *source's owner* rather than the ability's controller, so a stolen permanent still charges the player who owns it. The halving is baked into the mode rather than expressed as an `amount`: nothing in §6 reads a life total, and one card is not a reason to add a player selector and an `Amount` variant nothing else would use. Rounded **up**, so an odd life total costs the extra point; a player already at or below zero loses nothing. |
| `each_player_loses` | `amount` (amount) | no — "Each player loses N life" (Vandal's Edit): one simultaneous loss (CR 118.9) touching **every** living player, the ability's controller included, emitted in seat order. The all-players twin of `each_opponent_loses_life`, which carves the controller out. |
| `each_player_life_becomes_highest` | — | no — CR 118.5: setting a life total to N is modeled as a gain or loss of the difference. Reads the highest current life total among living players, then routes each living player's `highest - their_current` delta through the ordinary gain/lose choke (lifegain watchers/replacements — `YouGainLife`, `OpponentGainsLife`, CR 614 life-gain replacements — fire exactly as a real gain/loss would); a player already at the highest gets no event (a zero delta isn't a life change). Arbiter of Knollridge's ETB. |
| `return_from_graveyard_to_hand` | `target`, `count` (target-count, §7, default 1) | yes (`your_graveyard`; `count = { max = 3 }` returns up to three chosen graveyard cards — Life from the Loam's "return up to three target land cards") |
...
| `reanimate_random_from_target_opponent_graveyard` | `target` (a real target — always `"opponent"` in practice) | yes (`"opponent"`, §7) — the creature card among the targeted opponent's graveyard is then picked by the injected RNG at resolution (same seeded idiom `exile_random_from_graveyard_may_play` uses), not a further chosen target. No-op if that graveyard holds no creature card. Tariel, Reckoner of Souls's "Choose a creature card at random from target opponent's graveyard. Put that card onto the battlefield under your control." |
...
| `mass_return_from_graveyard` | `filter` (card filter, §7), `all_players` (bool, default false) | no — return every matching card in a graveyard to the battlefield, each under its owner's control. `all_players = false` scans only *your* graveyard (Replenish / Eiganjo Dynastorian's back face). `all_players = true` scans EVERY player's graveyard in APNAP order, each player's cards returning under that player's own control — "each player returns all creature cards from their graveyard to the battlefield" (All Hallow's Eve). |
...
| `shuffle_target_permanent_into_library` | `target` | yes (`{ permanent = "nonland" }` — any nonland permanent; Oblation) — the no-reveal half of `shuffle_target_permanent_into_library_then_reveal` above: a real shuffle (not a fixed position, unlike `tuck_permanent_into_library`), but nothing reads the post-shuffle order here, so it's a plain pure-mint effect. Pair with a following `target_owner_draws` step in an `effects` sequence for Oblation's "then draws two cards." A token target ceases to exist instead (CR 111.7), same as its fused sibling. |
| `target_owner_draws` | `count` (amount), `controller` (bool, default `false`) | no (the *shared target's* owner draws `count` cards — Oblation's "then draws two cards" rider, reading the enclosing `effects` sequence's target like `gain_life_target_controller`; place after a preceding `shuffle_target_permanent_into_library` step so it draws from the post-shuffle library. `controller = true` scopes the draw to the target's *controller* instead — Nin, the Pain Artist's "That creature's controller draws X cards" pairs it with `count = "x"` after a `deal_damage` step. Tolerant of a token target a preceding step already vanished (CR 111.7): falls back to that step's recorded owner rather than reading a since-removed object) |
| `tuck_permanent_into_library` | `target`, `to_top` (bool, default `false` — bottom of the owner's library; `true` = top, Temporal Spring), `second_from_top` (bool, default `false` — just under the top card, Whirlpool Whelm's win rider; takes precedence over `to_top`, and lands on top of a library shorter than one card) | yes (a battlefield permanent — `{ permanent = {} }` for any permanent (Temporal Spring), or a filtered `{ permanent = { types = "creature", attacking = true } }` for "target attacking creature" (Condemn)) — the standalone tuck-only half of `shuffle_target_permanent_into_library_then_reveal` above: no shuffle, no reveal, just a fixed-position move to the target's owner's library. A token target ceases to exist instead (CR 111.7). |
| `tuck_self_and_blocked_creatures` | — (no fields) | no — Gomazoa's tap ability: "Put this creature and each creature it's blocking on top of their owners' libraries, then those players shuffle." No chosen target — enumerates the source plus every attacker `Game::attackers_blocked_by` reports for it (the reverse read of `Game::blockers_of`, which backs `blocking_only` anthems), empty if it's blocking nothing (only the source is tucked then). Each object is tucked to the top of its owner's library (a token ceases to exist instead, CR 111.7); every distinct owner shuffles exactly once, after all of that owner's tucks are minted — never mid-batch, so a second tuck to an already-shuffled owner isn't left un-randomized. |
...
| `rearrange_target_players_top` | `count` (u32) | **player** ("Look at the top three cards of target player's library, then put them back in any order" — Natural Selection, `{ type = "dig", mode = "rearrange_target_players_top", count = 3 }` — the player target is implied by the mode, there is no `target` key. The same `ArrangeTop` pause a scry uses, except the chooser and the library's owner are different seats and the rest pile has nowhere to go: an answer that leaves any card off the top is rejected, so the only thing decided is the order. Projects to a `reorder_top` pending-choice view whose second lane is the not-yet-placed pile, not a destination) |
| `may_shuffle_target_players_library` | — | **player** ("You may have that player shuffle" — Natural Selection's tail, `{ type = "dig", mode = "may_shuffle_target_players_library" }` as the next `[[abilities.effects]]` step after the reorder, reading the same target. A yes/no addressed to the **controller**, not the library's owner — the point of the clause is that the caster may throw away the ordering they just chose. The targeted player is baked into the effect when the pause is raised, so the answer knows whose library shuffles) |
| `look_at_target_players_hand` | — | **player** ("Look at target player's hand" — Glasses of Urza, `{ type = "dig", mode = "look_at_target_players_hand", target = "player" }`. A *private* look (CR 701.20), not a reveal: nothing moves, no one is asked anything, and the only product is that the ability's controller may now read those cards. The wire event says only that a look happened; the cards themselves reach that one seat through the snapshot's per-card hand gate, and only the cards that were in hand at resolution — anything drawn later stays hidden) |
...
| `reveal_top_opponent_picks_one_to_graveyard` | `count` (u8) | no (Murmurs from Beyond: "Reveal the top three cards of your library. An opponent chooses one of them. Put that card into your graveyard and the rest into your hand." Reveals the top `count` (all public; a short library reveals only what's there), then hands off to the same shared "an opponent" chooser `reveal_top_split_piles` uses. The chosen opponent picks exactly one revealed card on an `OpponentChoosesRevealedToGraveyard` pause — mandatory, no decline — which is milled to the controller's graveyard; every other revealed card goes straight to the controller's hand, with no further controller-side choice — Fact or Fiction's reveal machinery minus the pile split) |
| `goad_target` | `target` | yes |
| `tap_target` | `target`, `count` (target-count, §7 — Magma Opus's "tap two target permanents") | yes |
| `untap_target` | `target`, `count` (target-count, §7 — Garruk Wildspeaker's "+1: Untap two target lands" is `count = { min = 2, max = 2 }`) | yes |
| `remove_from_combat` | `target`, `release_solely_blocked` (bool, default `false`) | yes (CR 506.4 — Spurnmage Advocate's "Remove target attacking or blocking creature from combat and tap it": drops the target as attacker/blocker, the same combat-list cleanup CR 701.15b regeneration already applies. Pair with a following `tap_target` step sharing the same `target` for the "…and tap it" half. `release_solely_blocked = true` (False Orders) additionally forgets that the target ever blocked anything, which is the one printed exception to CR 509.1h's sticky blocked-ness: "Creatures it was blocking that had become blocked by only that creature this combat become unblocked". An attacker a *second* creature also blocked stays blocked, because that second pair is still on the list. Leave it `false` everywhere else — a creature leaving combat does not un-block what it blocked. ponytail: `target` is authored `{ permanent = { types = "creature", attacking = true } }` — no `blocking` filter axis exists yet, so a *blocking* (not attacking) creature isn't a legal choice; add one if a card needs it) |
| `untap_all` | `filter` (§7 permanent filter) | no (untaps every matching permanent the controller controls) |
| `tap_source` | — | no ("Tap this creature" as an **effect** rather than as the `{T}` in an activation cost — Demonic Hordes' unpaid-upkeep penalty, `{ type = "control", mode = "tap_source" }`. Nothing is chosen and nothing is targeted; a source that has already left the battlefield taps nothing (CR 608.2b). For "`{T}`:" in a cost, use `taps_self = true` on the ability instead) |
| `tap_all` | `filter` (§7 permanent filter) | no (the tap-side mirror of `untap_all` — taps every matching permanent the controller controls; Dread Cacodemon's "tap all other creatures you control" pairs `filter = { types = "creature", controller = "you", other = true }` — `other` excludes Dread's own permanent) |
| `tap_all_target_player_controls` | `filter` (§7 permanent filter) | intrinsic (target player). The other-seat twin of `tap_all`: `tap_all` sweeps permanents *you* control with no target at all, this one sweeps the chosen player's board and leaves yours alone — Mana Short's "tap all lands target player controls" is `filter = { types = "land" }`. A plain tap, not a tap *for mana* (CR 106.11): nothing is produced and no land-tap watch fires. Already-tapped permanents are skipped, since `Event::Tapped` is the untapped→tapped change (CR 701.21a) and a redundant one would fire a becomes-tapped watch twice. Pair with a following `{ type = "mana", mode = "lose_all_unspent" }` step sharing the one target |
...
| `each_player_draws` | `count` (amount) | no (every player draws, not just the controller — Faerie Mastermind's flat `count = 1`, Skyscribing's spell mode reading `Amount::X`) |
| `target_player_loses_life` | `amount` (i32) | intrinsic (target player, no matching gain) |
| `sacrifice_own` | `filter` (§7 permanent filter), `count` (u32), `opponent_chooses` (bool, default `false` — "sacrifice a land of **an opponent's choice**", Demonic Hordes: the controller still loses the permanent, but the `ChooseOwnSacrifices` pause is answered by the next living seat in turn order instead, and each sacrifice is credited to its own controller. The one thing CR 701.16a's "the permanents' controller chooses" default forbids. ponytail: "an opponent" names nobody, so the next seat answers — pair it with an `approximates` saying so) | no (controller sacrifices `count` of their own matching permanents — a mandatory `PendingChoice::ChooseOwnSacrifices` the controller directs when more than `count` match, CR 701.16a; with `count` or fewer, all of them go immediately, no pause — CR 700.2. `filter.other = true` excludes the ability's own source, "a creature other than this creature" — Lord of the Pit; no matching permanent sacrifices nothing at all, so an "if you can't" fallback needs a `{ type = "conditional", … }` wrapper with an `otherwise`) |
| `defenders_split_blockers_into_piles` | none | no (Raging River's "each defending player divides all creatures without flying they control into a 'left' pile and a 'right' pile. Then, for each attacking creature you control, choose 'left' or 'right.'" Two pauses in order: every defending player names their left pile on a `PendingChoice::SplitBlockersIntoPiles` (answered by `Intent::ChooseSacrifices` for its subset shape — nothing is sacrificed, either pile may be empty), then the controller labels each of their attackers on a `PendingChoice::ChoosePileForAttacker` (answered by `Intent::ChooseOpponentPile`, `0` = left, `1` = right). The *unnamed* pile becomes illegal blockers for that one attacker until end of combat. Flyers are never divided, so the printed flying exemption needs no clause. Pair with `timing = "you_attack_with_creatures"`, `at_least = 1`) |
| `defenders_divide_blockers_among_attackers` | none | no (Camouflage's "instead of declaring blockers, each defending player … divides them into a number of piles equal to the number of attacking creatures … Assign each pile to a different one of those attacking creatures at random." Each defending player answers a `PendingChoice::DivideBlockersIntoPiles` once per attacker aimed at them (`Intent::ChooseSacrifices` again, each pile drawn from what the last one left), then the piles are dealt out over the engine's derive-per-op PRNG and every creature that *can* block what its pile landed on does — written down as a finished declaration, so that seat's declare-blockers step is already sealed. Pair with `cast_only_during_declare_attackers = true`; a creature goes in at most one pile) |
...
| `pay_or_else` | `cost` (`[cost]` table, same shape as `[echo]`), `otherwise` (array of effect tables, required) | no ("…unless you pay {cost}" — Rupture Spire's `timing = "etb"`, Phantasmal Forces' and Force of Nature's `timing = "upkeep"`. Real triggered abilities (CR 603.3b), NOT the Echo keyword, though they share Echo's resolution shape: pauses on a `PendingChoice::PayOrElse`, answered by `Intent::PayOptionalCost`. Paying settles `cost` from the controller's mana pool; declining runs `otherwise` instead, with the source as its source and the controller as its controller. Most printings spell `otherwise = [{ type = "sacrifice", mode = "source" }]`, but the penalty is whatever the card prints — Force of Nature's is `[{ type = "damage", mode = "to_self", amount = 8 }]`, which leaves the creature on the battlefield) |
| `sacrifice_self_unless_return_land` | `filter` (§7 permanent filter) | no (Treva's Ruins: "sacrifice it unless you return a non-Lair land you control to its owner's hand" — the land-bounce twin of `pay_or_else`; `filter` names the qualifying lands, `{ types = "land", controller = "you", nonlair = true }` for Treva's Ruins — pauses on a `PendingChoice::SacrificeUnlessReturnLand` offering the controller's matches, answered by `Intent::ReturnLandOrSacrifice`; no matching land skips the pause and sacrifices the source outright) |
...
| `copy_target_spell` | `set_color` (color, optional — Fork's "except that the copy is red": a CR 613.3c layer-5 color SET on the copy alone, so a copied Giant Growth reads red *instead of* green; the original on the stack under it is untouched, and omitting the field leaves the copy the original's own color — Twincast) | intrinsic (an instant/sorcery spell on the stack; Twincast) |
| `copy_this_spell` | `count` (amount, default one), `cast_from_graveyard_only` (bool, default false), `optional` (bool, default false) | no (a *resolution-time* copy rider: mints `count` copies of the resolving spell itself, each offered the same CR 707.10c retarget `copy_target_spell` offers; `cast_from_graveyard_only` gates the mint on the resolving spell having been cast via flashback — Sevinne's Reclamation's "if this spell was cast from a graveyard"; `optional` pauses on a `PendingChoice::MayYesNo` before minting, declining runs nothing — Sevinne's Reclamation's "you may copy this spell". A true "when you cast this spell" copy trigger — Gravestorm/Storm and reflexive "when you do, copy this spell" riders — belongs on a `when_you_cast_this` ability via `copy_triggering_spell` below, so the copies land on the stack *above* the original; Ominous Harvest and Plumb the Forbidden use that path) |
...
| `fight` | `ally_is_shared_target` (bool, default `false`), `one_way` (bool, default `false`) | intrinsic — default shape: target creature an opponent controls; your own fighter is chosen at resolution. `ally_is_shared_target = true` mirrors this: the ability's shared cast target is instead your own creature (a preceding `pump_until_end_of_turn` step's target, e.g.), and the enemy ("fights up to one target creature you don't control") is chosen at an *optional* resolution-time pause instead — no legal enemy, or the ally no longer being a creature, is a no-op (no pause, no fight). Primal Might: `effects = [{ type = "pump_until_end_of_turn", power = "x", toughness = "x", target = "creature_you_control" }, { type = "fight", ally_is_shared_target = true }]`. `one_way = true` (Infectious Bite: "Target creature you control deals damage equal to its power to target creature you don't control") skips the enemy's damage back to the ally — this isn't a fight at all (CR 701.12 never applies; the oracle text never says "fights"), just the same cast/resolution target split and damage plumbing run in one direction: `effects = [{ type = "fight", one_way = true }, { type = "put_counters_on_player", kind = "poison", count = 1, scope = "each_opponent" }]` |
| `schedule_at_next_upkeep` | `who` (`"you"` default, `"target_spell_controller"`), `then` (one effect), `fire_at` (a `Step` name, optional — default `"upkeep"`; also wired: `"end"`, `"end_combat"`, `"main1"`) | no (delayed trigger, CR 603.7: `then` fires at the next occurrence of `fire_at`'s step for whoever's turn it is — `"upkeep"`/`"end"`/`"end_combat"` fire for any player's step, `"main1"` scopes to the delayed trigger's own controller (Scattering Stroke's "your next main phase") — Arcane Denial's draws use the `"upkeep"` default; Dragon Whelp's "sacrifice this creature at the beginning of the next end step" uses `fire_at = "end"`) |
...
| `copy_triggering_spell` | `count` (amount), `may_choose_new_targets` (bool, default false), `last_known_information` (bool, default false) | no — a `schedule_next_cast_trigger` `then` payoff, or the direct effect of a live `cast_spell` trigger / `you_cast_this`: mints `count` copies of the spell that fired the watch (not a chosen target, not this ability's own spell), each offered the CR 707.10c retarget choice `copy_target_spell` offers when `may_choose_new_targets = true`; `false` mints keeping the triggering spell's own targets instead — CR 707.10c's declined case, and the default for a copy trigger with no "you may choose new targets" clause (Ominous Harvest's Gravestorm, off `you_cast_this` with `count = "permanents_died_this_turn"`; Plumb the Forbidden's reflexive "when you do, copy this spell," off `you_cast_this` with `count = "spell_sacrifice_count"` — the untargeted spell needs no retarget anyway). No-ops if the triggering spell already left the stack (countered in response, CR 603.4) before the copying ability resolved — UNLESS `last_known_information = true`, CR 702.40a Storm's documented exception: long-established Storm rulings hold that countering the spell with Storm doesn't stop its copies, since the storm trigger is independent of the original ever resolving; the copies are minted from the spell's last-known copiable characteristics instead. Thunderclap Drake's delayed one-shot: "{2}{U}, Sacrifice this creature: When you next cast an instant or sorcery spell this turn, copy it for each time you've cast your commander from the command zone this game. You may choose new targets for the copies." — `then = [{ type = "copy_triggering_spell", count = "commander_casts_from_command_zone", may_choose_new_targets = true }]` (`last_known_information` unset — the plain CR 603.4 default). Unbound Flourishing's live `cast_spell` watch ("Whenever you cast an instant or sorcery spell … copy that spell") and Owlin Spiralmancer's optional one ("your first spell with {X} … each turn, you may copy it") instead put this straight in `abilities.effects` on a `timing = "cast_spell"` ability — `Game::queue_cast_spell_triggers` bakes the live cast's own spell id into `triggering_spell`, same context-fill machinery as the delayed-trigger case. Reaping the Graves' Storm keyword: `timing = "when_you_cast_this"`, `effects = [{ type = "copy_triggering_spell", count = "spells_cast_before_this_this_turn", may_choose_new_targets = true, last_known_information = true }]` — `Game::enqueue_triggers`'s `Event::SpellCast` arm bakes both `triggering_spell` and the storm count into context, same shape as the live-watch case. |
| `copy_triggering_ability` | `may_choose_new_targets` (bool, default false) | no — an `activate_ability` trigger's own payoff: mints one copy of the activated ability that fired the watch, put on the stack above the original (CR 707.10c — the copy isn't "activated"), carrying its source/effect/target and its chosen `{X}` unchanged (CR 706.10 — an already-doubled X isn't re-doubled). No-ops if the triggering ability already left the stack (countered in response, CR 603.4). `may_choose_new_targets = true` offers a real re-pick (CR 707.10c) when the copied ability actually targets — Nin, the Pain Artist's "target creature" under Unbound Flourishing — via the same `place_targeted_ability` pause a fresh triggered ability uses (protection included, CR 702.16b); `false`, or a targetless copy, keeps the original's target(s) unchanged. Unbound Flourishing's ability half: `timing = "activate_ability"`, `caster = "you"`, `effects = [{ type = "copy_triggering_ability", may_choose_new_targets = true }]`. |
...
| `damaging_creature_controller_may_draw` | `count` (u32) | no (`deals_combat_damage_to_player` payoff, pairs with `who = "any_creature_damaging_your_opponent"`: the controller of the creature that dealt the damage — not this ability's controller — may draw `count`, Edric's "its controller may draw a card." The drawer is baked in at trigger placement, CR 603.10a) |
...
| `damage/to_dying_enchanted_creatures_controller` | `amount` (amount) | no (author as `type = "damage"`, `mode = "to_dying_enchanted_creatures_controller"`; an `enchanted_creature_dies` payoff: damage the player who controlled the Aura's host when it died — Creature Bond's "this Aura deals damage equal to that creature's toughness to the creature's controller." The recipient is snapshotted at trigger placement (CR 603.10a): the host is a graveyard card by resolution, where its controller reads as its owner, so a stolen creature would otherwise bill the wrong seat. Usually paired with `amount = "dying_enchanted_creature_toughness"`) |
| `damage/to_triggering_player` | `amount` (amount) | no (author as `type = "damage"`, `mode = "to_triggering_player"`; damage the one player the trigger names, filled in at placement. Under an `each_upkeep` ability that is the player whose upkeep it is — Copper Tablet's "At the beginning of each player's upkeep, this artifact deals 1 damage to that player."; under a `land_put_into_graveyard` ability it is the dead land's controller — Dingus Egg's "deals 2 damage to that land's controller". `damage/each_player` would bill every seat on every upkeep, which is once per seat per round instead of once). A player-relative `amount` here counts the **recipient's** things, not the source controller's — Karma's "damage to that player equal to the number of Swamps they control" is `amount = { per_permanent = { types = "land", subtypes = ["Swamp"], controller = "you" } }`, where `controller = "you"` means the taxed player |
| `damage/to_entering_permanent_controller` | `amount` (amount) | no (author as `type = "damage"`, `mode = "to_entering_permanent_controller"`; the player twin of `deal_damage_to_entering_permanent` above, off the same `permanent_enters` context slot: damage the *controller* of the permanent that just entered, not this ability's controller and not a chosen target — Ankh of Mishra's "Whenever a land enters, this artifact deals 2 damage to that land's controller." Pair with `controller = "any_player"` on the trigger for the symmetrical printing) |
...
| `destroy/that_creature` | `at` (step, default absent — `at = "end_combat"` postpones this already-filled payload once more, re-scheduling it as a CR 603.7 delayed triggered ability at that step: Cockatrice's "destroy that creature **at end of combat**". The id is already baked in, so nothing is re-chosen when it fires, and the re-scheduled copy carries `at` absent. Same schedule-or-do-it-now knob as `destroy/target`'s `at`) | no (author as `type = "destroy"`, `mode = "that_creature"`; "destroy *that creature*" over an id baked in when the ability was placed or scheduled, never a chosen target: a `deals_combat_damage_to_creature` payoff destroys the creature this permanent's combat damage just landed on, and a `blocks_or_becomes_blocked_by` payoff destroys the creature on the other side of the block (CR 603.10a last-known information) — Stinkweed Imp's and Cockatrice's "destroy that creature." `destroy/target`'s own `at` schedules this same payload for its already-chosen target, so authoring this mode by hand is only for the trigger case. An ordinary destroy: indestructible ignores it, a regeneration shield replaces it (same as `destroy/target`). No-ops if that creature is no longer on the battlefield when the trigger resolves, e.g. bounced/exiled in response, or if `destroy/target`'s `attack_rider` rode along and the creature's attack this turn came out the wrong way) |
...
| `schedule_return_reanimated_to_hand` | — | no (must follow a `reanimate_to_battlefield` step in the same `effects` sequence: reads the permanent that step just put onto the battlefield, grants it haste — an until-end-of-turn `TempBoost`, exact here because the pool's only consumer always leaves the battlefield this same end step — and schedules a CR 603.7 delayed trigger firing at the next end step. No-ops if the reanimation target was illegal (no `reanimate_to_battlefield` event this resolution). The delayed payload itself (`return_object_to_hand`, internal only, `object` filled in at schedule time) no-ops if that object has since left the battlefield some other way — Cauldron Dance's "That creature gains haste. Return it to your hand at the beginning of the next end step.") |
| `search_library` | `filter` (card filter, §7), `to_zone` (§7), `tapped` (bool, default `false`), `count` (u8, default `1` — "up to N" finds, one shuffle), `searcher` (`"you"` default, `"target_controller"` — Path to Exile's compensation search, `"all_players"` — Veteran Explorer's "each player may search"), `overflow` (§7, default none — where every find *after the first* goes, if different from `to_zone`; Cultivate), `count_amount` (amount, §6, default none — a dynamic cap that *overrides* `count` when set, resolved once as the search begins and carried to every seat of an `all_players` fan-out; Collective Voyage's "up to X basic land cards, where X is the total amount of mana paid this way" is `count_amount = "mana_paid_this_way"`), `optional` (bool, default `false` — printed "may search": pauses on a yes/no *before* searching; declining skips the search and its shuffle; White Orchid Phantom. Distinct from ability-level `optional`, which gates the whole ability before the stack) | no (searcher picks; tutors/ramp/fetchlands) |
| `each_player_sacrifices` | `scope` (§7, default `"all_players"`), `filter` (§7, default `"creature"`), `keep_one` (bool, default `false`), `life_loss` (i32, default `0`), `count` (§6 amount, default 1 — how many creatures each affected player sacrifices; a player with fewer sacrifices all of them; not combined with `keep_one`. Malfegor's "sacrifices a creature … for each card discarded this way" is `count = "cards_discarded_this_way"`), `down_to_fewest` (bool, default `false` — Balance's "equal to the number of lands controlled by the player who controls the fewest, then sacrifices the rest": *overrides* `count`, so each affected player sacrifices their own excess over the smallest matching battlefield in `scope`. That floor is measured once as the effect starts, before anybody sacrifices, so every seat is judged against the same number; a player already at it is skipped outright rather than asked for zero), `lose_game_if_short` (bool, default `false` — Lich's "sacrifice that many nontoken permanents. If you can't, you lose the game": an affected player who controls fewer than `count` matching permanents is eliminated (CR 104.3b) instead of sacrificing what they have, which is what every other edict does with a short board. Checked before the prompt is raised, so nobody is asked to pay a bill they can't cover), `then` (array of effects, default `[]`) | no (multi-player sacrifice edict; see below) |
...
| `control_player_to_play_card_from_hand` | `target` | yes (Word of Command's "Look at target opponent's hand and choose a card from it… the player plays that card if able": the **caster** answers, the **targeted opponent** plays. The one pause where the answering seat and the paying seat differ — the chosen card is cast from its own controller's hand, out of their own mana, ignoring priority and its printed timing. An unaffordable or otherwise unplayable pick is a legal answer that simply does nothing) |
...
| `join_forces_pay_mana` | — | no (join forces, CR 101.4 — Collective Voyage's "Starting with you, each player may pay any amount of mana": each living player, in turn order starting with the caster, pauses on a pay-any-amount choice and pays that much generic mana (CR 202.2 — generic accepts any type). Carries no payoff of its own: put the shared X to work in a following `effects` step reading `"mana_paid_this_way"` (§6), resumed once every player has answered — the mana twin of `councils_dilemma_vote`. Also drives a triggered ability's join forces, not just a spell's: Mana-Charged Dragon's `"attacks_or_blocks"` trigger pairs it with a `pump_self_until_end_of_turn` step) |
| `triggering_player_may_attach_this_aura_to_chosen` | `filter` (§7 permanent filter — the hosts on offer) | no (Kudzu's "When enchanted land becomes tapped, destroy it. That land's controller may attach this Aura to a land of their choice." Pair it with `timing = "enchanted_permanent_becomes_tapped"` and a preceding `destroy` / `target` step over `target = "enchanted_creature"`. The chooser is the *triggering permanent's* controller (filled in at placement, the same slot `damage` / `to_triggering_player` fills for Psychic Venom), not this Aura's, and the choice is a **may** — declining leaves the Aura unattached and CR 704.5m sweeps it to the graveyard on the next check, as does having no legal host to offer (no pause is raised at all then). Raises the same `PendingChoice::ChooseAttachHost` a deployed Aura raises, so the answer, the attachment and the state-based exemption that keeps the Aura alive between its host's destruction and the answer all come from there. `filter` is evaluated for the chooser, so a `controller = "you"` axis would read as *their* battlefield) |
| `triggering_player_may_pay` | `cost` (`[cost]` table, same shape as `[echo]`), `then` (array of effect tables, required) | no (Paralyze's "that player may pay {4}. If the player does, untap the creature", CR 603.3b. The upside twin of `pay_or_else` — paying *buys* `then` rather than dodging a penalty — and the fixed-cost twin of `triggering_player_may_pay_any_amount_to_prevent` just below, with which it shares its payer: the offer goes to the player the trigger is *about*, not to the ability's controller, which is the one thing an ability-level `[abilities.cost]` (Mana Vault's "you may pay {4}") cannot express — so pair it with `condition = { type = "enchanted_permanents_controllers_upkeep" }`. Pauses on a `PendingChoice::PayCost`, answered by `Intent::PayOptionalCost`; paying places `then` on the stack as an ability under the **payer**, so a `target = "enchanted_creature"` in `then` still reads off the Aura) |
| `triggering_player_may_pay_any_amount_to_prevent` | `prevent_up_to` (§6 amount — the damage the following step deals) | no (Power Leak's "that player may pay any amount of mana. This Aura deals 2 damage to that player. Prevent X of that damage, where X is the amount of mana that player paid this way", CR 615. `join_forces_pay_mana`'s single-seat twin: the same pay-any-amount pause and the same generic payment, offered only to the player the trigger is about (the enchanted permanent's controller, filled in at placement — pair it with `condition = { type = "enchanted_permanents_controllers_upkeep" }` and a following `damage` / `to_triggering_player` step). What the payment buys is a prevention shield on that same player worth what they paid, armed between this step and the damage step, capped at `prevent_up_to` so an overpayment can't bank prevention against an unrelated hit later in the turn — "prevent X of *that* damage". Set `prevent_up_to` to whatever the following damage step deals) |
...
| `each_player_chooses_war_or_peace` | — | no (`timing = "as_enters"` choice, CR 614.12/101.4 — Archangel of Strife's "As this creature enters, each player chooses war or peace": each living player, in APNAP order (no "starting with you" wording, unlike `councils_dilemma_vote`), pauses on a `["war", "peace"]` ballot reusing the council's-dilemma `CastVote`/`ChooseMode` wire shape. Carries no payoff of its own — each answer writes straight to that player's own `Player::war_choices` keyed by the asking permanent, for that permanent's own `anthem_static`'s `war_choice` (§7) to read live, not a tally) |
...
| `each_player_shuffles_hand_and_graveyard_then_draws` | `count` (§6 amount) | no (Timetwister's "Each player shuffles their hand and graveyard into their library, then draws seven cards": the recycling sibling of `each_player_discards_hand_then_draws` — same APNAP fan-out and same redraw, but hand *and* graveyard are tucked back into the library and it is shuffled before that player draws `count`, so nothing reaches a graveyard and `you_discard` triggers stay quiet. A shuffled-back card can be drawn again. The spell itself is on the stack while this resolves, so it isn't swept up (CR 608.2m) — no effect is needed for Timetwister's "(Then put Timetwister into its owner's graveyard.)" reminder) |
| `each_player_discards` | `scope` (§7, default `"all_players"`), `down_to_fewest` (bool, default `false`) | no (multi-player fan-out — Syphon Mind's "Each other player discards a card" is `scope = "each_opponent"`: each scoped player, in APNAP order, discards one card of their choice, pausing on that player's own card-pick — an empty-hand player is skipped, contributing 0. Carries no payoff of its own: put the reflexive payoff as a following `effects` step reading `"cards_discarded_this_way"` (§6), resumed once every scoped player has answered — the discard twin of `each_player_exiles_from_graveyard`. `down_to_fewest` is the discard half of Balance's "Players discard cards … the same way", the same shape `each_player_sacrifices` carries: instead of one card each, every scoped player pitches their own excess over the smallest hand among them, measured once before anybody discards) |
| `discard_your_hand` | — | no (the effect's controller discards their *entire* hand — Malfegor's "discard your hand". Choiceless (never pauses), emitting the normal discard markers so `you_discard` triggers still fire, and setting `"cards_discarded_this_way"` (§6) to the hand's size so a following `effects` step reads it — the controller-scoped sibling of `each_player_discards_hand_then_draws`) |
...
| `may_exile_discarded_nonland_may_play` | — | no (the `you_discard_nonland` payoff: the controller may exile **one of** the nonland cards discarded this event (still in their graveyard) face-up with impulse-play permission until end of turn, or decline — Conspiracy Theorist's "you may exile one of them from your graveyard. If you do, you may cast it this turn". The impulse-play twin of `may_return_from_graveyard`; the discarded nonland card ids are baked in at trigger placement, so this effect takes no fields. Answered by `Intent::ChooseSacrifices` — empty declines, one entry exiles that card. No card still in the graveyard skips the pause) |
| `may_return_from_graveyard` | `filter` (card filter, §7), `if_you_sacrificed_this_way` (bool, default `false`), `mandatory` (bool, default `false`) | no (resolution-time rider: the controller returns one card from *their own* graveyard matching `filter` to hand. Default (`mandatory = false`) is optional — "you may return … to your hand" (Deadly Brew, Witch of the Moors) — declining is legal. `mandatory = true` is a plain "you return …" (Witherbloom Command mode 0, CR 700.2): with a legal card in the graveyard the controller *must* choose one, declining (an empty `Intent::ChooseSacrifices`) is rejected; with **no** legal card it still quietly does nothing (no pause), the same skip the optional case takes. `if_you_sacrificed_this_way` gates the whole rider — no pause at all, same as declining — on the controller having actually sacrificed a permanent during this resolution's own preceding `each_player_sacrifices` edict, tracked by an edict-scoped scratch flag that resets each edict and is set only by the edict's own controller's sacrifice) |
...
| `reflexive_trigger_if_nonland_exiled` | `then` (array of effects) | no (the count-gated twin of `reflexive_trigger`, CR 603.3b: put this step after an `each_player_exiles_from_graveyard` step; when it resolves it places each `then` effect as its **own separate, respondable ability** (its own priority window, its own target chosen at placement, CR 601.2c) **only when that fan-out exiled one or more nonland cards** — none at all when the count is zero. The settled `nonland_cards_exiled_this_way` count is baked into each `then` effect's `count = "nonland_cards_exiled_this_way"` at placement, so the follow-up reads the number even though it resolves in its own later frame — Augusta, Order Returned's "When one or more nonland cards are exiled this way, …") |
...
| `may_put_counter_on_creature` | — | no (resolution-time optional, CR 601.2c — the controller may put a single +1/+1 counter on any one creature on the battlefield, chosen *during resolution* and never advertised as a stack target; declining does nothing. Unlike `may_sacrifice`/`may_discard` it carries no `then` — Zimone's Hypothesis' "You may put a +1/+1 counter on a creature" primer, whose follow-up (the parity mass-bounce) is the next `Sequence` step and runs whether or not the counter landed, not "if you do". Answered by `Intent::ChooseCopyTarget` (one object or none) and projected onto the `ChooseCopyTarget` client view with a `put_counter_on_creature` discriminator so the prompt reads as counter placement instead of copy selection. ponytail: fieldless — the pool's one such primer always places exactly one counter on any creature; add a `filter`/`count` when a second, differently-shaped card lands) |
| `may_block_attacker_of_your_choice` | — | no (resolution-time optional, CR 601.2c — False Orders' "You may have it block an attacking creature of your choice". Fieldless like `may_put_counter_on_creature` above: the creature is the enclosing `Sequence`'s shared target — the one the preceding `remove_from_combat` step just pulled out of combat — and the candidate list is read live off combat when the choice is raised, filtered to the attackers that creature could legally have been declared as blocking (CR 509.1a, judged against its *own* controller, so the re-aim can't launder a block that was never available). Declining leaves it out of combat. A chosen attacker gets a real `Event::BlockerDeclared`, so "blocks" / "becomes blocked" triggers fire and it is blocked for the rest of combat. Answered by `Intent::ChooseCopyTarget` (one object or none) and projected onto the `ChooseCopyTarget` client view with a `choose_block_target` discriminator, the same reuse `may_put_counter_on_creature` makes) |
...
| `put_creature_from_hand` | `subtypes` (string list, default any), `keep` (bool, default `false`) | no (the creature sibling of `put_land_from_hand`: controller may put a creature card from hand onto the battlefield, entering via the same ETB path as any other put-onto-the-battlefield effect. `subtypes` restricts eligibility to those creature subtypes (empty = any). With `keep = false` (Cauldron Dance) the deployed creature gains haste — an until-end-of-turn `TempBoost` — and is scheduled for a CR 603.7 delayed `sacrifice/object` at the next end step; with `keep = true` (Kaalia) neither happens, it is kept. On an `attacks` trigger the threaded defending player is baked in, so the creature enters **tapped and attacking that opponent** (CR 508.4, reusing the `TokenEnteredAttacking` put-in-attacking path). No eligible creature in hand is a no-op. Cauldron Dance: "You may put a creature card from your hand onto the battlefield. That creature gains haste. Its controller sacrifices it at the beginning of the next end step." Kaalia of the Vast: "you may put an Angel, Demon, or Dragon creature card from your hand onto the battlefield tapped and attacking that opponent.") |
...
| `monstrosity` | `count` (u8) | no ("Monstrosity N", CR 701.28a — Alpha Deathclaw's "{5}{B}{G}: Monstrosity 4": if the source isn't already monstrous, puts `count` +1/+1 counters on it through the same counter-replacement pipeline as `put_counters` (Doubling Season/Hardened Scales apply) and sets `Permanent::monstrous`; if it's already monstrous, a total no-op (CR 701.28c) — not even the counters, and no `"becomes_monstrous"` trigger fires. `monstrous` is a one-way flag, never cleared at Untap; a permanent that leaves and returns is a new object and starts unmonstrous again. Always on a `timing = "activated"` ability; pair with a `"becomes_monstrous"` trigger above for "…or becomes monstrous…" riders) |
...
| `change_text` | `words` (`"basic_land_type"` \| `"color"`, required — which vocabulary the replaced word comes from), `target` (target spec, required — `"spell_or_permanent"` for both printed cards) | yes (CR 612.1 "change the text of target spell or permanent by replacing all instances of one basic land type with another" — Magical Hack; the color-word twin is Sleight of Mind. Resolving raises the `ChooseCreatureType` picker **twice** over `words`' candidate list — the word being replaced, then its replacement — and writes the pair onto the target as an indefinite text swap, so it ends when that object does (CR 400.7). The swap reaches the enums the card model already stores: a permanent's land subtypes (and so its mana, §9), its printed landwalk land type and protection color, and the color a `prevent_next_damage` (§6) names. A word buried anywhere else in an effect tree — a filter's color, a land-type count, an Aura's keyword *grant* — does not move; flag the card rather than widening it blind) |
| `choose_basic_land_type` | — | no (`timing = "as_enters"`, CR 614.12 "as this Aura enters, choose a basic land type" — the same `ChooseCreatureType` picker as the row below, narrowed to `BASIC_LAND_TYPES` (the five, in WUBRG order); the answer lands on the source's own `Permanent::chosen_subtype` and is read back by a `set_chosen_land_type` `set_attached_types` (§9) — Phantasmal Terrain. Naming anything off that list is `Reject::IllegalChoice`) |
| `choose_creature_type` | — | no (`timing = "as_enters"`, CR 614.12 "as ~ enters, choose a creature type" — pauses on a `ChooseCreatureType` choice for the controller over the pool's known creature types, `CREATURE_TYPES` — kept in step with the pool by a `cards` test, so authoring a creature whose type isn't on that list fails the suite instead of quietly narrowing every such prompt; the chosen type is stored on the ability's own source, `Permanent::chosen_subtype`, read back by `anthem_static`'s `chosen_subtype` axis — Patchwork Banner) |
| `choose_color` | — | no (pauses on a `ChooseColor` choice for the controller over the fixed five colors; the chosen color is stored on the ability's own source, `Permanent::chosen_color`, read back by `grant_to_attached`'s and the self-static `protection_from_chosen_color` axes. On `timing = "as_enters"` it is CR 614.12's "as ~ enters, choose a color" — Flickering Ward, Voice of All; it is also an ordinary resolution step sequenced ahead of a step that reads the color inside an activated ability or spell — Mother of Runes, Bathe in Light) |
| `choose_opponent` | — | no (Black Vise's "As this artifact enters, choose an opponent": pauses on the shared "an opponent ..." picker for the controller — the same one Fact or Fiction and clash use — and stores the answer on the ability's own source, `Permanent::chosen_opponent`, read back by the `"chosen_players_upkeep"` condition (§5). With one opponent left alive the pause collapses and the lone opponent is chosen. Chosen, not targeted (CR 115.10a): no shroud/protection check, no target-legality recheck) |
...
| `conditional` | `condition` (§5 shape), `then` (array of effects), `negate` (bool, default `false`), `otherwise` (array of effects, default `[]`) | no (runs `then` only if `condition` holds — or, with `negate = true`, only if it *doesn't* — when *this step* resolves; a per-step gate inside an `effects` sequence. Plain: Zimone's "draw two instead". Negated: Court Hussar's "sacrifice it unless {W} was spent to cast it" is `condition = { type = "color_was_spent_to_cast_this", color = "white" }`, `negate = true`, `then = [{ type = "sacrifice", mode = "source" }]` — CR's "unless" is `negate`d "if". `otherwise` (Lily Bowen, Raging Grandma's "if its power is 16 or less. Otherwise, …") runs instead of `then` when the condition doesn't hold — `condition` is still evaluated exactly once either way, unlike two independently-conditioned `conditional` steps sharing one condition in the same `effects` sequence (Whirlpool Whelm's clash-win split), which is only safe when `then`'s own effect can't change what the shared condition reads; use `otherwise` whenever it can, as Lily's own power does) |
| `get_emblem` | `emblem` (§8 token profile — a Scryfall oracle id resolved against `cards/data/tokens/`) | no (CR 114.1 — "You get an emblem with \"…\"", Garruk, Cursed Huntsman's `−6`. Creates the named object in the resolving controller's command zone; its abilities are whatever the profile's `[[abilities]]` carry, and nothing else, so the profile is authored with `[kind] type = "sorcery"` (an empty `TypeSet` — CR 114.5, an emblem has no characteristics other than its abilities). Emblem profiles share `data/tokens/` because that registry is just "a named `CardDef` keyed by Scryfall oracle id", and Scryfall models emblems as token-set cards; name the file `emblem_<card>.toml`. Read back by `Game::emblems(player)` — a command-zone `Object::Card` with `commander = false`, which is never castable. No removal/copy/target counterpart exists, by CR 114.5) |

`counter_replacement`: adder → `add = 1`; doubler → `times = 2` (omit `add`); halver → `halve = true`. Applications are ordered additions → multipliers → halvings (CR 616.1's ordering choice is not offered). Read the oracle wording to pick the gate: "if you/an opponent **would put**" is placer-keyed (`placer`), "if counters **would be put on** …" is recipient-keyed (`recipients` / `filter`).
...
`down_to_fewest = true` swaps the fixed `count` for a levelling one: the smallest matching
battlefield (or, on `each_player_discards`, the smallest hand) among the scoped seats is measured
once as the effect starts, and each seat pitches its own excess over it. Balance is three of these
in printed order, and the middle one is the discard twin — same flag, same fan-out, same floor:
...
| `"spell_or_permanent"` | a spell on the stack *or* a permanent on the battlefield, unrestricted — the only spec that spans both zones. Any controller's, either side | `target_becomes_color` (the lace cycle's "target spell or permanent"). Unfiltered because the five cards that print the wording restrict nothing; narrow it with a new spec if a card ever does |
...
One more target exists as intrinsic-only (it is never a settable `target =` field — the effect
has no `target` field at all and always targets this): `counter_target_spell` always targets a
spell on the stack (narrowed by its own `filter` field, §6).
...
fully declinable "up to N"; Volcanic Salvo's "up to two" is `count = { max = 2 }`). Both spell- and
...
`min = 0` count (declining, or there being no legal target to begin with) still puts the ability on
the stack with no target chosen (CR 601.2c treats choosing zero of "up to one" as a complete, legal
choice) — *unless* every step of the ability needs that same missing target, in which case it drops
outright as a pure no-op (`Effect::has_target_independent_step`; Killian, Decisive Mentor's "tap up
to one target creature and goad it" — `tap_target`'s `count = { max = 1 }`, shared by the following
untyped `goad_target` step since a `Sequence` shares one target, and goad has nothing to goad
without a tapped creature). An ability with a *target-independent* step alongside the declinable
one still runs that step (Kinetic Ooze's "destroy up to one target artifact or enchantment … If X is
5 or more, you draw a card" — the draw runs even when nothing gets destroyed). A multi-target
ability declined the same way (0 of a `min = 0` count chosen) follows the identical rule at
resolution: it drops outright unless a target-independent step rides along, in which case that step
still runs. (A "you may … a creature" primer that must *not* be a stack target — Zimone's
Hypothesis' "You may put a +1/+1 counter on a creature" ahead of its untargeted mass parity-bounce
— is instead a resolution-time `may_put_counter_on_creature` step in a `Sequence`, §6, not a
declinable cast-time target.)
...
multi-target, chooses its targets the same post-cast way a non-modal multi-target spell does. The
other modes keep their ordinary single-target-or-none shape.
...
independently at resolution (CR 608.2b — one target going illegal doesn't stop the other clause
from resolving). This is distinct from a *shared*-target `Sequence` (Prismari Command mode 1's
"target player draws two cards, then discards two cards" — the discard step's `target` spec is the
same `Player` as the draw step's, so it's read back as the one already-chosen target, not a second
clause); the engine tells the two shapes apart by comparing each step's target spec against the
one immediately before it — same spec back-to-back is a shared-target rider, a spec change starts
a new independent clause. `nth_mode`'s one-ability-per-printed-mode indexing is unchanged — a
multi-clause mode is still one `[[abilities]]` block, just with several targeted
...
the six `_scaled` flags. `strive` is the *mechanic*, not the keyword: Fireball's un-keyworded
"This spell costs {1} more to cast for each target beyond the first" is the same
`strive = { generic = 1 }` + `strive_scaled = true` pair.

`multikicker_scaled = true` (default `false`) is Multikicker's own sibling (CR 601.2c/702.33c) —
unlike `x_scaled`/`sacrifice_scaled`/`strive_scaled` (each "exactly N", substituting the declared
or derived count directly), Multikicker's target count is always "one base target, plus one more
for each kick": Comet Storm's "Choose any target, then choose another target for each time this
spell was kicked" is `deal_damage/target`'s `count = { multikicker_scaled = true }`. `min`/`max`
need not be given; at cast time they're always overridden to "exactly 1 + N", N being the caster's
declared `Intent::Cast.multikicker_count` (`Game::spell_multikicker_count`), clamped to
`MAX_TARGETS` like `x_scaled` above. `min`/`max`/other `_scaled` flags don't matter when
`multikicker_scaled = true`; no pool card sets more than one of the six `_scaled` flags.

`kicked_scaled = true` (default `false`) is a plain-Kicker sibling of `multikicker_scaled` for a
*second, independent* target clause that only exists when the spell was kicked (CR 702.33g), as
opposed to Multikicker's single clause that grows — Orim's Thunder's "Destroy target artifact or
enchantment. If this spell was kicked, it deals damage equal to that permanent's mana value to
target creature." is two `[[abilities.effects]]` steps: the mandatory `destroy/target` (default
`{1, 1}`) and a second `damage/target` with `count = { min = 1, max = 1, kicked_scaled = true }`.
Unkicked, the clause is forced to `(0, 0)` — zero targets, not "cap down to the mandatory minimum"
— since the two clauses target unrelated things (an artifact/enchantment vs. a creature); this is
the opposite false-case behavior from `main_phase_scaled` below, whose second target is the *same*
clause as its mandatory first. `min`/`max` are given normally (the kicked case); only the unkicked
case is overridden.

`main_phase_scaled = true` (default `false`) is the "cast during your main phase" conditional-count
sibling (CR 601.2c; no card-specific CR subrule number — this is ordinary spell-timing state read
at target-selection time, the same way `spell_was_kicked` is read for `kicked_scaled`) — Return to
Dust's "Exile target artifact or enchantment. If you cast this spell during your main phase, you
may exile up to one other target artifact or enchantment." is a *single* `exile/target` clause,
`count = { min = 1, max = 2, main_phase_scaled = true }`: CR 601.2c's built-in same-clause
distinctness gives the oracle's "other" for free. Cast outside the caster's main phase, `max` caps
down to `min` (1) instead of forcing `(0, 0)` — the first target is still mandatory either way,
only the *extra* one is conditional. `Game::spell_cast_during_main_phase` reads the ambient
`Spell.cast_during_main_phase` flag set at the real cast site (active player, `Step::Main1`/`Main2`
— mirrors `cast_from_hand`), not recomputed at resolution.

`total_mv_max` (an amount, default unset) is a **set-level** cap on the chosen targets' *summed*
mana value (CR 601.2c: legality is evaluated over the whole chosen set, not per target) —
distinct from every `_scaled` flag above, which caps the *number* of targets, not their combined
cost. Rampaging Yao Guai's "destroy any number of target artifacts and/or enchantments with total
mana value X or less" is `destroy/target`'s `target = { permanent = { types =
"artifact_or_enchantment" } }`, `count = { max = 255, total_mv_max = "x" }` — "any number" has no
printed upper bound, so `max` takes the field's own `u8` ceiling (255) rather than an arbitrary
smaller one; `min` defaults to 0, matching the printed "any number" (declining is legal). The
budget is resolved once, against the ability's own source, when the whole set is answered; an
over-budget set is rejected outright (`Reject::IllegalChoice`), not truncated to fit. Composes with
`min`/`max` on any targeted effect (a `_scaled` count could in principle carry a `total_mv_max`
too); no pool card needs both today.
...
- a characteristic read: `"source_power"` / `"source_toughness"` (the effect's own permanent's
  power/toughness — Goldvein Hydra; Tanazir's attack P/T snapshot), `"source_mana_value"` (the
  effect's own permanent's mana value — Animate Artifact, where `set_attached_base_pt` resolves its
  amounts against the *host*, so "its mana value" is the enchanted artifact's),
  ...
  creatures — Volcanic Salvo's self cost reduction),
  `"greatest_power_among_creatures_you_control"` (the single largest live power among the
  controller's creatures, 0 with none — Garruk, Primal Hunter's "−3: Draw cards equal to the
  greatest power among creatures you control"),
  `"permanents_you_own_opponents_control"` (battlefield permanents you own but an opponent controls
  — Zedruu the Greathearted's upkeep "permanents you own that your opponents control"; each donated
  permanent counts once), `"commander_color_count"` (colors in your
  ...
  On a `timing = "enchanted_creature_dies"` ability, `"dying_enchanted_creature_toughness"` reads the
  *host's* CR 603.10a snapshot — the toughness it last had on the battlefield, pumps and counters
  included (Creature Bond); only meaningful on that timing, and pair it with the
  `damage/to_dying_enchanted_creatures_controller` mode, which bills the same creature's controller.
  ...
  timing).
- a player-counter read: `"opponents_poison_counters"` (the total poison counters across the
  effect controller's living opponents — Phyrexian Swarmlord's "for each poison counter your
  opponents have"; a sum, not a per-opponent maximum, and a player who has already lost
  contributes nothing) and `"controllers_poison_counters"` (the poison counters on the *one*
  player the amount is relative to — Phyresis Outbreak's "for each poison counter its controller
  has". On `weaken_each_creature` that player is each affected creature's own controller, so
  opponents on different poison totals take different `-N/-N`; on a plain effect it reads the
  effect's controller)
- `"untapped_lands_at_turn_start"` — how many untapped lands the player this amount is relative
  to controlled when the current turn began (Power Surge's "X is the number of untapped lands they
  controlled at the beginning of this turn"). A snapshot taken as the upkeep begins, not a live
  count, so tapping out in response to the trigger changes nothing. Pair it with the
  `damage/to_triggering_player` mode (§6), which makes "they" the taxed player.
- a turn tally: `"life_gained_this_turn"` / `"spells_cast_this_turn"` /
  `"damage_taken_this_turn"` (damage dealt to the effect's controller this turn, combat and
  noncombat alike — CR 120.1; Simulacrum reads it twice, once as life gained and once as damage
  handed to its own creature. Damage, not life loss: a drain or a paid life cost never counts) /
  `"creatures_died_this_turn"` (died under your control — Gorma) /
  `"creatures_died_this_turn_any_controller"` (the same tally summed over every player — Scavenging
  Ghoul's "for each creature that died this turn", which names no controller) /
...
- `"spell_multikicker_count"` — how many times the resolving spell's Multikicker cost was paid (CR
  ...
  is `{ times = 2, per = "spell_multikicker_count" }`. Unlike its `spell_*_count` siblings, this one
  also reads `Permanent::entered_multikicker_count` (the value copied over as the permanent enters,
  the same "read the spell's own info before it's gone" idiom `entered_with_x` uses for `{X}`), so
  an ETB trigger — which resolves after the casting spell is already gone — still sees it.
- `"revealed_creature_mana_value"` — the mana value of the creature card revealed to pay the
  resolving spell's `reveal_creature_from_hand` additional cost (CR 601.2g), 0 with no such cost;
  only meaningful on that spell's own effect (Disaster Radius's "X is the revealed card's mana
  value")
- `"spells_cast_before_this_this_turn"` — CR 702.40a's storm count: the game-wide tally (every
  player, not just the caster) of spells cast before this one this turn, snapshotted when a
  `when_you_cast_this` ability goes on the stack (immune to a response cast in front of it, or to
...
- `"nonland_cards_exiled_this_way"` — how many *nonland* cards *this same resolution's own*
  ...
  sequence.
- `"cards_discarded_this_way"` — how many cards *this same resolution's own* preceding discard step
  discarded: an `each_player_discards` fan-out across the scoped seats (Syphon Mind's "You draw a
  card for each card discarded this way"), or a `discard_your_hand` step's whole-hand discard
  (Malfegor's "for each card discarded this way"). Resolution-scoped like `nonland_cards_exiled_this_way`
  above; pair with that discard step in the same `effects = [...]` sequence.
- `"creatures_sacrificed_this_way"` — how many creatures *this same resolution's own* preceding
  `each_player_sacrifices` edict actually sacrificed across every affected player (Syphon Flesh's
  "create a … Zombie … token for each creature sacrificed this way"). Resolution-scoped; pair with
  the `each_player_sacrifices` step in the same `effects = [...]` sequence.
- `"cards_exiled_by_search_this_way"` — how many cards *this same resolution's own* preceding
  `search_library` step just moved to an `to_zone = "exile"` destination (Trench Gorger: "this
  creature has base power and base toughness each equal to the number of cards exiled this way").
  Resolution-scoped like `nonland_cards_exiled_this_way` above; pair it with the `search_library`
  step (`to_zone = "exile"`) in the same `effects = [...]` sequence, typically feeding a following
  `set_own_base_pt_from_amount` step.
- `"mana_paid_this_way"` — the total mana every player paid to *this same resolution's own*
  preceding `join_forces_pay_mana` step (Collective Voyage's "up to X basic land cards, where X is
  the total amount of mana paid this way"; Mana-Charged Dragon's "this creature gets +X/+0 until
  end of turn, where X is the total amount of mana paid this way" — same shape, driving a
  `pump_self_until_end_of_turn` step instead of a search, off a `"attacks_or_blocks"` trigger
  instead of a spell). Resolution-scoped; pair with the `join_forces_pay_mana` step in the same
  `effects = [...]` sequence.
...
- `"returned_nonland_card_mana_value"` — the mana value of the **nonland** card *this same
  resolution's own* preceding `return_from_graveyard_to_hand` step just put into its owner's hand
  (Vengeful Rebirth's "If you return a nonland card to your hand this way, Vengeful Rebirth deals
  damage equal to that card's mana value to any target"). `0` when a land came back or the return
  fizzled, which *is* the "if you return a nonland card" gate — a source that would deal 0 damage
  deals none at all (CR 120.8). Resolution-scoped; pair with that step in the same
  `effects = [...]` sequence.
...
- `"your_life_total"` — the effect's own controller's current life total, read live as the effect
  resolves (Lich's `lose_life` `amount`: "you lose life equal to your life total"). No target
  needed. A controller already at or below 0 resolves to that number, so on a `lose_life` it would
  *gain* the difference back — no pool card reaches that, and it isn't clamped, because clamping it
  would be inventing a rule none of them print.
- `"triggering_damage_dealt"` — the amount of damage — combat or noncombat alike — that just fired
  the watch reading it: the enchanted host's, on a `timing = "enchanted_creature_deals_damage"`
  ability (Armadillo Cloak's `gain_life` `amount`: "you gain that much life"), or the controller's
  own, on a `timing = "you_are_dealt_damage"` one (Lich's `each_player_sacrifices` `count`:
  "sacrifice that many nontoken permanents"). Only meaningful on that ability's own effect;
...
- a main-phase-branch amount: `{ if_main_phase = <Amount>, else = <Amount> }` — `if_main_phase` if
  the resolving spell was cast during its caster's main phase, else `else` (Sulfurous Blast's "If
  you cast this spell during your main phase, Sulfurous Blast deals 3 damage... instead":
  `{ if_main_phase = 3, else = 2 }`). Reads `Game::spell_cast_during_main_phase` off the effect's
  `source`, the `if_kicked`/`else` table's main-phase-conditional sibling. In a
  `damage/each_creature` effect, this (like `if_kicked`) is pre-resolved against the true spell
  source before per-creature substitution swaps `source` for the creature being damaged — reading
  it after that swap would silently default to `else` for every creature (there's no ambient
  cast-timing on a permanent).
- `"spell_first_target_mana_value"` — the mana value of the permanent the resolving spell's first
  target clause chose (Orim's Thunder's "it deals damage equal to that permanent's mana value":
  read after that permanent has already left the battlefield, so it's the target *object's*
  last-known mana value, CR 706.11a-style, not a live characteristic read).
- a scaled amount: `{ times = <i32>, per = <Amount> }` — `times` multiplied by the resolved `per`
  amount (Congregate's "gains 2 life for each creature on the battlefield" is `{ times = 2, per =
  "per_creature_on_battlefield" }`). `per` is any Amount above, so this supplies the per-unit
...
- an offset amount: `{ offset = <Amount>, delta = <int, default 0> }` — the resolved inner amount
  plus `delta`, floored at zero (Black Vise's "X is the number of cards in their hand minus 4" is
  `{ offset = "cards_in_your_hand", delta = -4 }`). The floor is unconditional: every offset the
  pool prints feeds a damage or count slot where a negative result is no damage anyway (CR 120.8).
  Wraps another amount the way `half` does, so the two share the "answered before the
  exactly-one-of table" shape and can nest.
...
| `subtypes` | array of strings, default `[]` | restrict to permanents carrying any of these printed subtypes (Goldspan Dragon's "Treasures you control" — `["Treasure"]`); empty matches all. Reaches a land's own type line too (CR 305.6), which lives under `[kind].subtypes` rather than the card's top-level `subtypes` — `{ types = "land", subtypes = ["Plains"] }` is Flashfires' "Destroy all Plains", catching the basic and every dual that shares the type |
| `controller` | `"any"` (default), `"you"`, `"opponent"`, `"active_player"`, `"defending_player"` | whose permanents qualify, relative to the effect's controller — except `"active_player"`, which is absolute: whoever is the active player when the filter runs (Siren's Call's "creatures the active player controls"). Deliberately not the same as `"opponent"`, which at a four-player table reaches three boards; every card printing this clause names exactly one player. Re-read each time the filter runs, so a sweep scheduled this turn still names the same player at that turn's end step. `"defending_player"` is likewise absolute and likewise names one player: whoever the filter's own **source** is attacking (Gaea's Liege's "the number of Forests defending player controls"), read off the declared attacker/defender pair. When the source isn't an attacking creature at all — Blaze of Glory is a *spell* — it falls back to the one player the current combat is aimed at, and matches nothing when there is no combat or when the attack is split across two players (with nobody to choose between them the phrase has no referent, CR 506.2) |
...
| `mv_min` | u8, optional | mana-value floor (Austere Command's "mana value 4 or greater", the sibling of `mv_max`); omit to not gate on mana value |
...
| `has_mana_ability` | bool, default `false` | `true` requires the permanent make mana when tapped — Power Sink's "lands with mana abilities they control", which spares a Fabled Passage. Read live off the same predicate the client's tap-for-mana affordance uses, so a land whose type line was changed under it is judged by what it produces *now*. `false` imposes no restriction |
| `power_max` | u8, optional | power ceiling (Silverquill Charm's "power 2 or less"); omit to not gate on power |
| `power_min` | u8, optional | power floor (Meekstone's "creatures with power 3 or greater"); omit to not gate on power. The mirror of `power_max`, and read just as live — a creature that shrinks below the floor walks free of whatever the filter gated. |
| `power_parity` | `"even"`/`"odd"`, optional | power parity gate (Zimone's Hypothesis; zero is even); omit to not care |
| `noncreature` | bool (default `false`) | `true` excludes creature-typed permanents ("noncreature artifact" — Haywire Mite). Sugar for `exclude = "creature"`; unions into the same field. |
| `exclude` | a card-type name or list, same spelling as `types` (§7's `types` axis), default `[]` | general type exclusion — the permanent must have *none* of these types (Terror/Shriekmaw/Ashes to Ashes's "nonartifact creature" is `types = "creature", exclude = "artifact"`). An Artifact Creature fails an `exclude` containing artifact even though it also carries the creature type — the negation reads the *same* type set `types` does. |
...
| `blocking` | bool (default `false`) | `true` restricts to creatures currently blocking some attacker (Righteousness — "target blocking creature"). Reads `CombatState::blocks`, the declared-blocks list `anthem_static`'s own `blocking_only` axis consults; that axis stays where it is, since an anthem carries its own inline filter fields rather than a `PermanentFilter` |
| `unblocked` | bool (default `false`) | `true` restricts to creatures **no** creature is blocking (Forcefield — "an unblocked creature of your choice"). The mirror of `blocking` above, read off the same `CombatState::blocks` list from the other end. On its own it also matches every creature sitting at home, which nothing is blocking either — pair it with `attacking` or `attacking_you` |
...
| `toughness_less_than_source_power` | bool (default `false`) | `true` requires toughness strictly less than the filter's own source permanent's *power* — what the source can lift (Stone Giant's "target creature you control with toughness less than this creature's power"); same no-source no-op as `power_less_than_source` above, so likewise only meaningful on a targeted ability's own target filter |
...
| `did_not_attack_this_turn` | bool (default `false`) | `true` requires that the permanent was not declared as an attacker this turn (CR 508.1 — Siren's Call's "creatures … that didn't attack this turn"). Cleared at the untap step, so it only says anything within the turn the attack would have happened in. `destroy/target`'s `attack_rider` asks the same question of one already-chosen creature; this asks it of a whole board scan, where the answer is re-read per permanent as the sweep fires |
| `controlled_since_turn_start` | bool (default `false`) | `true` requires that the permanent's controller has controlled it continuously since this turn began (CR 302.6 — Nettling Imp's "the active player has controlled continuously since the beginning of the turn"). Reads the summoning-sickness flag, which *is* that fact, so it only says what it means about a permanent whose controller has already untapped this turn — the active player's. Every card printing this clause is restricted to the active player's turn, so that is always whose permanents it reads |
...
| `exclude_subtypes` | string array (default `[]`) | excludes permanents carrying **any** of these subtypes (Keldon Warlord's "non-Wall creatures you control" is `{ types = "creature", exclude_subtypes = ["Wall"], controller = "you" }`); matched against the same layered subtype view the positive `subtypes` axis reads, so a Darksteel Mutation Insect is excluded by `["Insect"]` and no longer by its printed subtype. Distinct from `nonlair` below, which deliberately reads a land's *printed* type line |
...
| `without_flying` | bool (default `false`) | `true` excludes creatures with flying (Breath of Darigaaz's "each creature without flying") |
| `without_keyword` | keyword (default absent) | Excludes creatures with that keyword, spelled exactly as a printed keyword is (`without_keyword = { landwalk = "island" }`). A *second* exclusion slot alongside `without_flying` above, for a filter that has to bar two keywords at once — Island Sanctuary's "except by creatures with flying **and/or** islandwalk" inverts to a banned set lacking both |
| `with_flying` | bool (default `false`) | `true` restricts to creatures with flying (Firespout's "each creature with flying") — the positive sibling of `without_flying` above |
| `creature_or_vehicle` | bool (default `false`) | `true` matches a creature **or** a permanent with the Vehicle artifact subtype (Ao, the Dawn Sky mode 2 — Vehicle is not a card type). When set, replaces the ordinary `types` axis for the match |
| `snow` | bool (default `false`) | `true` requires a snow permanent (CR 205.4g); reads [`CardDef::snow`] |
| `shares_type_with_dying_permanent` | bool (default `false`) | `true` requires the permanent share at least one card type with the ability's own triggering dying permanent's last-known card types (CR 603.10a — Martyr's Bond's "shares a card type with it"). A *dynamic* type gate: unlike `types` above, the actual required set isn't known until the ability fires. Only meaningful on an `each_player_sacrifices` `filter` behind a `nonland_permanent_you_control_dies_including_this` ability — the engine fills the real type set in from the dying permanent when the trigger goes on the stack, before any player ever sees the choice. |
...
`"shares_type_with_dying_permanent"` (Martyr's Bond's dynamic edict filter — see the table row above),
and any single type name. Examples:
`filter = "creatures"`, `filter = { types = "creature", enchanted = false }` (Winds of Rath),
`filter = { creature_or_vehicle = true, controller = "you" }` (Ao mode 2),
`filter = { types = "nonland", mv_max = 2 }` (Culling Ritual),
`target = { permanent = { types = "nonland", token = "nontoken", controller = "opponent", mv_max = 4 } }`
(Skyclave Apparition), `target = { permanent = { types = "creature", exclude = "artifact", not_color = "black" } }`
(Terror/Shriekmaw's "nonartifact, nonblack creature"). For mass effects, each destroyed permanent goes to its owner's graveyard (a
...
Loyalty), `"each_opponent"` (Witch of the Moors, Lorehold Charm), `"you"` (Lich's damage tax —
the effect's own controller alone, a one-seat fan-out rather than a separate single-player mode,
so the prompt, the derived count and the shortfall check are the ones every other edict already
uses), or `"targeted_players"` (Priest of Forgotten Gods' "any number of target players" — a
controller-chosen subset of living players, possibly empty or including the controller themselves,
in place of a scope-derived set). Its `filter` is the same permanent filter (default
`"creature"`).
...
`"creature"`, `"any_card"`, `"instant_or_sorcery"` (Mystic Sanctuary), `"sorcery"` (sorcery cards
only, no instants — Anarchist's "target sorcery card from your graveyard"),
`{ sorcery_with_color = "red" }` / `{ instant_with_color = "blue" }` (the same speed axis narrowed
to one color of the card's color identity — Nucklavee's "target red sorcery card" / "target blue
instant card"), `"enchantment"` (an Aura counts —
Replenish), `"permanent"` (any card with a permanent type, no mana-value bound — Deadly Brew's
"another permanent card"), `{ permanent_with_subtype = [...] }` (`"permanent"` narrowed to a
subtype intersection, no card-type restriction — Bladewing the Risen's "target Dragon permanent
card" is `{ permanent_with_subtype = ["Dragon"] }`), `"noncreature_nonland"` (neither a creature nor a land — Quintorius,
...
card", which a nonbasic Plains-typed dual like Eclipsed Steppe does NOT match), `"snow_land"` (a
land with `snow = true` — Into the North's "snow land card"; a snow creature like Ohran Frostfang
does not match), or one of the
MV-gated tables:
...
`{ creature_with_mana_value_at_least = 6 }` (Fierce Empath: "a creature card with mana value 6 or
greater" — the inclusive-lower-bound twin of `{ creature_with_mana_value_at_most = N }`),
...
**`search_library` `to_zone =`:** `"hand"` (tutors), `"battlefield"` (ramp/fetchlands;
`tapped = true` to enter tapped), `"graveyard"` (Buried Alive: "put them into your graveyard"
— routed through the same library-to-graveyard choke mill effects use, so the arrival never
counts as "from the battlefield" and can't fire Dies), or `"exile"` (Trench Gorger: "search your
library for any number of land cards, exile them" — routed through the same
[`Event::MovedToExile`] choke a graveyard-to-exile move uses). The searcher picks up to `count`
matches (default `1`; or none — "fail to find" is always legal at any point and ends the search),
each moved to `to_zone` as it's found; the library is shuffled once, after the last pick (CR
701.19f — Land Tax's "up to three basics" is `count = 3`; `count = "any"`, CR 701.19's "any number
of" — Trench Gorger — is sugar for an effectively unbounded count: no real library holds enough
cards to hit the internal ceiling, so the search just re-pauses until the searcher fails to find or
the matches run out). `searcher = "target_controller"` hands the search
to the ability's shared target's controller instead (Path to Exile's/Assassin's Trophy's
basic-land compensation). `searcher = "all_players"` fans one search out to every living player
in APNAP order (CR 101.4) — Veteran Explorer's "each player may search their library for up to
two basic land cards": each player searches their own library, makes their own picks (their own
decline is legal and doesn't abort the others' searches), shuffles their own library once, and
finds enter under their own control, before the next player's search begins. In this model a
search reveals nothing to opponents (each player's own matches are private to them, same as any
other `search_library`).
...
- `"target_opponent"` — the same shape as `"target_player"` above, restricted to an opponent (CR
  ...
  opponent.
- `"each_other_player"` — every player *other than* the ability's chosen Player target: the
  target itself is excluded, every other living player (the ability's own controller included)
  gets a token (Death by Dragons: "Each player other than target player creates a 5/5 red Dragon
  creature token with flying."). Same `TargetSpec::Player`/`Target::Player` shape as
...
- **`grant_to_attached`**: adds `power`/`toughness` and `keywords` to the host. `power`/
  ...
  a runtime scope read live off `Permanent::chosen_color` (set by the Aura's `timing = "as_enters"` `choose_color`),
  ...
  host); a one-effect grant runs directly, multiple as a `Sequence`. `cost` and `trigger` are
  mutually exclusive: `trigger = <trigger table>` instead of `cost` grants a *triggered* ability
  (Power Fist's Equipment grant: "Whenever this creature deals combat damage to a player, put that
  many +1/+1 counters on it." — `granted_ability = { trigger = { deals_combat_damage_to_player =
  { who = "this" } }, effects = [...] }`), read by a scanner's own granted-triggers accessor
  instead of the activated-ability one, so it never becomes an `ability_at` index on the host.
  ponytail: only `deals_combat_damage_to_player` is wired as a granted trigger tag today (Power
  Fist is the pool's one consumer) — extend `de::GrantedTriggerTag` the moment a second flavor
  lands.
  ...
  `cant_attack_controller` (bool, default `false`) is the Vow cycle's "Enchanted creature ... can't
  attack you or planeswalkers you control" — narrower than `cant_attack`: the host is merely
  forbidden from attacking *this Aura's own controller*, not banned from attacking outright, so it
  can still attack a different opponent. Read live in `declare_attackers` beside the landed vow-counter
  (`Permanent::vow_protected`) restriction, off the same attachment scan, so it lifts the instant the
  Aura leaves. ponytail: only the "can't attack you" clause is enforced; "or planeswalkers you
  control" is unobservable while every attack target is a `PlayerId` (no planeswalker permanent
  exists in the pool) — wire it through when planeswalker defenders land.
  `cant_be_enchanted` (bool, default `false`) is Consecrate Land's "Enchanted land ... can't be
  enchanted by other Auras": while attached, no *other* Aura may attach to the host — an Aura spell
  can't choose it as its cast target (CR 303.4a), and an Aura already there becomes illegally
  attached and is put into its owner's graveyard (CR 704.5n). Read live off the attachment scan via
  `Game::host_cant_be_enchanted_by`, which skips the granting Aura itself — that skip is what makes
  it "*other* Auras" — so it lifts the instant the Aura leaves. Equipment is unaffected: it attaches,
  it doesn't enchant.
  `legendary_only` (bool, default `false`) gates `keywords` — only `keywords`, not `power`/
  `toughness`/the other axes above — on the host's own legendary status (Champion's Helm's "As
  long as equipped creature is legendary, it has hexproof"): read live off the host's own
  `CardDef::legendary` at keyword recompute, the host-object twin of `anthem_static`'s `condition`
  axis. It can't reuse `condition`/`Condition` because those evaluate against a `TriggerContext`'s
  controller-scoped facts, not an arbitrary object's own characteristics; ponytail: a single
  "host is legendary" bool rather than a general host-object condition axis — grow into a real
  `Condition` variant carrying the host id if a second host-based gate turns up in the pool.
...
- **`set_attached_types`**: adds card types and/or changes the host's creature subtypes while
  ...
  *replaces* the host's own creature subtypes (Darksteel Mutation's "is an Insect", dropping the
  host's printed types). `set_types` (bool, default false — CR 613.4 "loses all other card types")
  makes `add_types` the host's *complete* card type list rather than an addition, so mutating an
  enchantment creature (Doomwake Giant) leaves exactly an artifact creature, not an enchantment
  creature artifact; list every kept type in `add_types` (`["artifact", "creature"]`). `lose_all_abilities` (bool, default false — CR 613.1e/701) strips the
  ...
  *addition* / "loses all abilities except …" / partial removal is still not expressible — flag it.
  On a **land** host the same `set_subtypes` replaces its basic land types (a land's types live
  under `[kind].subtypes`, §3, and the engine's live subtype read unions them in). CR 305.7 then
  follows for free: an enchanted Mountain that is now a Swamp taps for `{B}` and no longer for
  `{R}`, because every read of a land's base `produces` goes through `Game::land_mana_credit`,
  which derives the credit from the land's *effective* basic types whenever they differ from its
  printed ones. Evil Presence is the whole card. A dual loses the half that went with the type it
  no longer has (Badlands → a Swamp, `{B}` only). `set_chosen_land_type = true` swaps the printed
  `set_subtypes` list for the one type the Aura's controller named as it entered (Phantasmal
  Terrain, paired with a `choose_basic_land_type` as-enters ability, §8) — the same single answer
  the picker offers, so it needs no list of its own and does nothing until the choice is answered.
- **`all_lands_of_type_become`**: the same land-type change with no Aura and no controller —
  "All Mountains are Plains" (Conversion), "All Swamps are 1/1 black creatures that are still
  lands" (Kormus Bell), "All Forests are 1/1 creatures that are still lands" (Living Lands). It is
  scoped by `land_types` rather than a §7 permanent filter because a filter would have to ask for
  the candidate's subtypes, which is the very answer the effect changes; matching names against
  the subtype line as it is being built instead keeps CR 613.4 timestamp order meaningful (a
  second conversion sees what the first left) and makes the recursion impossible. `set_subtypes`
  replaces the whole line per CR 305.7 exactly as the Aura form does, so the `{W}`-not-`{R}`
  consequence rides free on `Game::land_mana_credit`; `add_types` + `base_power`/`base_toughness`
  + `add_colors` are the animator riders, unioned on so "still lands" holds. Multiple such statics
  apply oldest source first.
...
- **Delayed triggered abilities** beyond "at the beginning of the next upkeep / end step / end of
  combat / main phase" (`schedule_at_next_upkeep`'s `fire_at`, §6) — no "in two turns" scheduling
  or firing at an arbitrary future step beyond those four.
