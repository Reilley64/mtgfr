...
| `destroy/that_creature` | — | no (author as `type = "destroy"`, `mode = "that_creature"`; "destroy *that creature*" over an id baked in when the ability was placed or scheduled, never a chosen target: a `deals_combat_damage_to_creature` payoff destroys the creature this permanent's combat damage just landed on (CR 603.10a last-known information) — Stinkweed Imp's "destroy that creature." `destroy/target`'s own `at` schedules this same payload for its already-chosen target, so authoring this mode by hand is only for the trigger case. An ordinary destroy: indestructible ignores it, a regeneration shield replaces it (same as `destroy/target`). No-ops if that creature is no longer on the battlefield when the trigger resolves, e.g. bounced/exiled in response, or if `destroy/target`'s `only_if_it_attacked` rode along and the creature never attacked) |
