...
| `proliferate` | `times` (amount, usually `1` or `"x"` — Expansion Algorithm's "Proliferate X times") | no (untargeted; pauses on a select-any-number choice over every battlefield permanent with a counter, giving each one more of every kind already there — CR 701.27; the offered set is permanents-only — players can now carry poison counters (`put_counters_on_player`), but proliferate still can't choose them, see increment #17) |
