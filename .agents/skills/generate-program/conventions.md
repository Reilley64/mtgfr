...
> **Note:** This file is a snapshot of the conventions practiced in the live Foldkit codebase. If anything here contradicts what `client/node_modules/foldkit` or upstream [foldkit/foldkit examples](https://github.com/foldkit/foldkit/tree/main/examples) actually do, the live code is canonical. In this repo, prefer `client/app/` and `client/node_modules/foldkit/dist/` over memory.
...
- `empty` and `keyed` are properties on the `h` builder the runtime passes to the view. They are not top-level exports of `foldkit/html`.
