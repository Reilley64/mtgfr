...
> **Note:** This file is a snapshot of the architecture as practiced in the live Foldkit codebase. If anything here contradicts what `client/node_modules/foldkit` or upstream [foldkit/foldkit examples](https://github.com/foldkit/foldkit/tree/main/examples) actually do, the live code is canonical. In this repo, prefer `client/app/` and `client/node_modules/foldkit/dist/` over memory.
...
- **With args, fallible** (HTTP fetch): [examples/weather/src/main.ts](https://github.com/foldkit/foldkit/blob/main/examples/weather/src/main.ts) (`FetchWeather`)
- **With args, infallible** (random + return): [examples/kanban/src/command.ts](https://github.com/foldkit/foldkit/blob/main/examples/kanban/src/command.ts) (`GenerateCardId`)
- **With args, storage**: [examples/kanban/src/command.ts](https://github.com/foldkit/foldkit/blob/main/examples/kanban/src/command.ts) (`SaveBoard`)
- **Argless, DOM side effect**: [examples/kanban/src/command.ts](https://github.com/foldkit/foldkit/blob/main/examples/kanban/src/command.ts) (`FocusAddCardInput`)
...
- **Always-active keyboard input**: [examples/snake/src/main.ts](https://github.com/foldkit/foldkit/blob/main/examples/snake/src/main.ts)
- **Animation-frame-driven game tick**: [examples/canvas-art/src/main.ts](https://github.com/foldkit/foldkit/blob/main/examples/canvas-art/src/main.ts)
- **Conditional WebSocket connection** (active only when a session exists): [examples/websocket-chat/src/main.ts](https://github.com/foldkit/foldkit/blob/main/examples/websocket-chat/src/main.ts)
- **Production multi-stream pattern**: `client/app/game/stream-subscription.ts`, or [packages/typing-game/client/src/subscription.ts](https://github.com/foldkit/foldkit/blob/main/packages/typing-game/client/src/subscription.ts)
...
Use these instead of raw `document.querySelector`, `setTimeout`, `Date.now()`, or `Math.random()`. They compose naturally inside `Command.define`. For canonical wiring, see [examples/kanban/src/command.ts](https://github.com/foldkit/foldkit/blob/main/examples/kanban/src/command.ts) (`FocusAddCardInput` wraps `Dom.focus`) and [examples/stopwatch/src/main.ts](https://github.com/foldkit/foldkit/blob/main/examples/stopwatch/src/main.ts) (`Clock.currentTimeMillis` inside an `Effect.gen`).
...
For the canonical update-handler shapes (the exact `UrlRequest` tag names, how to dispatch `pushUrl` vs an external load Command, and how to derive the route from a `Url`), see [examples/routing/src/main.ts](https://github.com/foldkit/foldkit/blob/main/examples/routing/src/main.ts) or `client/app/routes.ts` in this repo.
