...
10. **Host embedding**: the program runs inside another app ("a widget in our React app", "embed this in an existing page", "the host needs to control it") → `Runtime.makeElement` plus the `Runtime.embed` lifecycle handle, with Flags for initial data and Ports for ongoing communication in both directions. [foldkit/foldkit `examples/embedding/`](https://github.com/foldkit/foldkit/tree/main/examples/embedding) is the canonical reference: a plain TypeScript host driving a Foldkit widget end to end
...
Two codebases are the _quality bar_ for generated apps in this repo. Not just "patterns to copy" but "the level of craft to match":

- `client/app/`: production multi-page app with Submodels, OutMessage, update/view decomposition, subscription patterns, and domain modules.
- [foldkit/foldkit `packages/typing-game/client/src/`](https://github.com/foldkit/foldkit/tree/main/packages/typing-game/client/src): upstream production reference for curried handler extraction and real-time patterns.
- [foldkit/foldkit `packages/website/src/`](https://github.com/foldkit/foldkit/tree/main/packages/website/src): upstream production Foldkit website for page organization and route-driven rendering.
...
Read [examples/counter/src/main.ts](https://github.com/foldkit/foldkit/blob/main/examples/counter/src/main.ts)

**Tier 2: Timers, subscriptions, simple stateful apps:**
Read [examples/stopwatch/src/main.ts](https://github.com/foldkit/foldkit/blob/main/examples/stopwatch/src/main.ts) (timer via subscription, `Duration` field pattern) and [examples/todo/src/main.ts](https://github.com/foldkit/foldkit/blob/main/examples/todo/src/main.ts) (CRUD with localStorage via flags)

**Tier 3: Async operations, loading/error states, API calls, form validation:**
Read [examples/weather/src/main.ts](https://github.com/foldkit/foldkit/blob/main/examples/weather/src/main.ts) (HTTP with `HttpClient`) and [examples/form/src/main.ts](https://github.com/foldkit/foldkit/blob/main/examples/form/src/main.ts) (uses `foldkit/fieldValidation`; see the Form Validation section in Phase 4)

**Tier 4: URL routing, multiple pages, query parameters:**
Read [examples/routing/src/main.ts](https://github.com/foldkit/foldkit/blob/main/examples/routing/src/main.ts) and [examples/query-sync/src/main.ts](https://github.com/foldkit/foldkit/blob/main/examples/query-sync/src/main.ts)

**Tier 5: Complex state, nested domain models, CRUD, drag-and-drop:**
Read [examples/shopping-cart/src/main.ts](https://github.com/foldkit/foldkit/blob/main/examples/shopping-cart/src/main.ts) (nested domain schemas, cart state) and [examples/kanban/src/main.ts](https://github.com/foldkit/foldkit/blob/main/examples/kanban/src/main.ts) (CRUD with `Ui.DragAndDrop`, flags restoring from localStorage, subscriptions)

**Tier 6: Submodels, OutMessage, multi-step forms, auth flows, multi-module apps:**
Read [examples/auth/src/main.ts](https://github.com/foldkit/foldkit/blob/main/examples/auth/src/main.ts) (login/signup with Submodels, OutMessage, protected routes) and [examples/job-application/src/main.ts](https://github.com/foldkit/foldkit/blob/main/examples/job-application/src/main.ts) (multi-step form with deeply nested Submodels in `step/`, `Ui.DatePicker`, `Ui.FileDrop`, `Ui.Menu`, `Calendar` module for date handling)

**Tier 7: Real-time, WebSocket, Managed Resources, production-grade:**
Read `client/app/update.ts` and `client/app/game/stream-subscription.ts`, or [packages/typing-game/client/src/update.ts](https://github.com/foldkit/foldkit/blob/main/packages/typing-game/client/src/update.ts) and its `page/home/` and `page/room/` directories for the full Submodel/OutMessage pattern.
...
- [examples/ui-showcase/src/main.ts](https://github.com/foldkit/foldkit/blob/main/examples/ui-showcase/src/main.ts): root wiring, `Got*` delegation, `toParentMessage` helpers
- [examples/ui-showcase/src/message.ts](https://github.com/foldkit/foldkit/blob/main/examples/ui-showcase/src/message.ts): how UI component Messages are structured
- [examples/ui-showcase/src/model.ts](https://github.com/foldkit/foldkit/blob/main/examples/ui-showcase/src/model.ts): how UI component Models are composed
- [examples/ui-showcase/src/update.ts](https://github.com/foldkit/foldkit/blob/main/examples/ui-showcase/src/update.ts): how UI component updates are delegated
- [examples/ui-showcase/src/toast.ts](https://github.com/foldkit/foldkit/blob/main/examples/ui-showcase/src/toast.ts): read when using `Ui.Toast`: Toast is unique in that it's parameterized on a payload schema via `Ui.Toast.make(PayloadSchema)`, returning a typed module you import from
...
For work inside this repo, add or extend files under `client/app/` instead of scaffolding a new project.
...
client/node_modules/foldkit/dist/runtime/runtime.d.ts # ApplicationInit, RoutingApplicationInit, makeApplication, makeElement

# If using routing
client/node_modules/foldkit/dist/route/parser.d.ts   # literal, slash, string, int, Route.root, Route.mapTo, Route.oneOf, Route.parseUrlWithFallback
client/node_modules/foldkit/dist/url/index.d.ts      # toString
...
client/node_modules/foldkit/dist/subscription/index.d.ts # Subscription.make<Model, Message>, Subscription.lift, Subscription.aggregate

# If using mount / managed-resource / custom-element
client/node_modules/foldkit/dist/mount/index.d.ts             # Mount.define: for per-instance VNode lifecycle
client/node_modules/foldkit/dist/managedResource/public.d.ts  # ManagedResource.make / lift / aggregate + tag: for stateful runtime objects keyed on Model condition
client/node_modules/foldkit/dist/customElement/index.d.ts     # CustomElement.define: for typed bindings to native web components

# If using forms
client/node_modules/foldkit/dist/fieldValidation/public.d.ts # Field (tagged union), makeRules({required?, rules}), validate, allValid; rule constructors on the Rule namespace (Rule.url(options), Rule.email, Rule.minLength, Rule.pattern, Rule.fromSchema, ...)
...
client/node_modules/foldkit/dist/ui/<component>/public.d.ts  # Model, Message, init, update, view (Submodel-shaped) or ViewConfig (render-helper-shaped), OutMessage when applicable
...
# If using dates
client/node_modules/foldkit/dist/calendar/index.d.ts # CalendarDate, today.local (returns Effect<CalendarDate>); for raw millis use Clock.currentTimeMillis
```
...
- For apps with multiple domain entities referenced across modules, extract shared schemas into `src/domain/` (e.g., `domain/product.ts`, `domain/session.ts`). See the shopping-cart and auth examples for this pattern, and read [packages/website/src/page/projectOrganization.ts](https://github.com/foldkit/foldkit/blob/main/packages/website/src/page/projectOrganization.ts) for guidance on when and how to structure domain modules
...
Canonical reference: [examples/form/src/main.ts](https://github.com/foldkit/foldkit/blob/main/examples/form/src/main.ts) (async email uniqueness check with version-based cancellation) and [examples/job-application/src/step/](https://github.com/foldkit/foldkit/tree/main/examples/job-application/src/step) (validated multi-step forms across submodels).
...
- Canonical reference: [examples/job-application/src/step/attachments.ts](https://github.com/foldkit/foldkit/blob/main/examples/job-application/src/step/attachments.ts)
...
- [examples/weather/src/story.test.ts](https://github.com/foldkit/foldkit/blob/main/examples/weather/src/story.test.ts): simple Command resolution (happy path + error path)
- [examples/auth/src/page/loggedOut/page/login.story.test.ts](https://github.com/foldkit/foldkit/blob/main/examples/auth/src/page/loggedOut/page/login.story.test.ts): Submodel with OutMessage assertions, field validation
- [packages/website/src/search/story.test.ts](https://github.com/foldkit/foldkit/blob/main/packages/website/src/search/story.test.ts): multi-step interactions (arrow key cycling, stale result handling)
- `client/app/board/story.test.ts`, `client/app/shell/lobby/story.test.ts`: this repo's Story test patterns
...
- [examples/weather/src/scene.test.ts](https://github.com/foldkit/foldkit/blob/main/examples/weather/src/scene.test.ts): basic Scene flow with form interaction and Command resolution
- [examples/auth/src/scene.test.ts](https://github.com/foldkit/foldkit/blob/main/examples/auth/src/scene.test.ts): a multi-page app's root-level Scene driving the login flow through the root view
- [examples/kanban/src/scene.test.ts](https://github.com/foldkit/foldkit/blob/main/examples/kanban/src/scene.test.ts): scoped queries with `within`, `toHaveValue`, explicit test data
- `client/app/board/scene.test.ts`, `client/app/shell/surfaces.test.ts`: this repo's Scene test patterns
...
2. .agents/skills/generate-program/architecture.md
3. .agents/skills/generate-program/conventions.md
4. .agents/skills/generate-program/checklist.md
