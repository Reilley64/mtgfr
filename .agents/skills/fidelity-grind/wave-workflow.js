...
const plan = await agent(
  ...
  `NEVER paraphrase a card's oracle text, mana cost, or P/T from memory in a brief — the Xira grind's briefs got five cards' costs/text wrong and only live re-verification saved them, and eight of mirror mastery's ~36 increments described a card that does not exist. Quote text fetched from Scryfall this session, or instruct the implementer to fetch it; implementers must verify against live Scryfall regardless. THE BACKLOG SECTION ITSELF IS SUSPECT: tell each implementer that if the increment's premise contradicts live Scryfall, it must CORRECT that backlog section in place before writing code.\n\n` +
...
for (const inc of plan.increments) {
  ...
  const res = await agent(
    ...
    `Before writing code, open live Scryfall for every card your increment names and check the increment's premise against it. If the backlog section is wrong, fix that section in place, then build the real card.\n` +
