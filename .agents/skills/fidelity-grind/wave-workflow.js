...
const plan = await agent(
  ...
  `The backlog is {{BACKLOG_FILE}} — increments {{BACKLOG_RANGE}} (deps may reference landed increments in docs/FIDELITY_BACKLOG.md; verify against code) plus FIDELITY_BACKLOG.md's "Per-card exotics" section. Increments marked "**LANDED**" are DONE; do not repick them. VERIFY eligibility against the card TOMLs and the code, not backlog prose — "still blocked" lists go stale.\n\n` +
...
const verify = await agent(
  ...
  `4. Update {{BACKLOG_FILE}} (and docs/FIDELITY_BACKLOG.md only if a tier increment was touched) from the ACTUAL TOML diffs: LANDED marks with dated Landed:/Still blocked: lines; XL slices get dated progress notes instead.\n` +
