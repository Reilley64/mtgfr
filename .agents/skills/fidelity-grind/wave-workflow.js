...
// {{BACKLOG_RANGE}} (e.g. "#135-#160"), and {{SHARED}} (absolute path of the filled
...
const plan = await agent(
  ...
  `The backlog is docs/FIDELITY_BACKLOG.md — increments {{BACKLOG_RANGE}} plus the "Per-card exotics" section. Increments marked "**LANDED**" are DONE; do not repick them. VERIFY eligibility against the card TOMLs and the code, not backlog prose — "still blocked" lists go stale.\n\n` +
  ...
  `1. An increment is eligible ONLY if (a) not already landed, (b) every "Depends on:" dep is landed, and (c) at least one real pool card in crates/cards/data/*.toml would become faithful or measurably closer (grep the approximates notes for a live consumer — flag-don't-force, never add a dead variant).\n` +
...
for (const inc of plan.increments) {
  ...
  const res = await agent(
    ...
    `Invoke the tdd and card-dsl skills. TDD: failing engine test in crates/engine/tests/game.rs FIRST.\n` +
...
const verify = await agent(
  ...
  `4. Update docs/FIDELITY_BACKLOG.md from the ACTUAL TOML diffs: LANDED marks with dated Landed:/Still blocked: lines; XL slices get dated progress notes instead.\n` +
  `5. Run \`just engine-cr-index\` so docs/CR_INDEX.md includes the wave's citations.\n` +
  `6. After any edits, re-run \`cargo test\` and \`cargo fmt\` so the tree ends green and formatted.\n\n` +
