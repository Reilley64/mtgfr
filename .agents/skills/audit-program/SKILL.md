...
4. **Tier-matching exemplar**. Pick at least one to read alongside the audited code. Prefer `client/app/` when auditing this repo; otherwise use upstream [foldkit/foldkit](https://github.com/foldkit/foldkit) examples:
   - Tier 1-2 → `client/app/` root files, or [examples/counter](https://github.com/foldkit/foldkit/tree/main/examples/counter), [examples/stopwatch](https://github.com/foldkit/foldkit/tree/main/examples/stopwatch)
   - Tier 3 → [examples/weather](https://github.com/foldkit/foldkit/tree/main/examples/weather), [examples/form](https://github.com/foldkit/foldkit/tree/main/examples/form)
   - Tier 4 → `client/app/routes.ts`, or [examples/routing](https://github.com/foldkit/foldkit/tree/main/examples/routing)
   - Tier 5 → `client/app/board/`, or [examples/kanban](https://github.com/foldkit/foldkit/tree/main/examples/kanban)
   - Tier 6 → `client/app/shell/`, or [examples/auth](https://github.com/foldkit/foldkit/tree/main/examples/auth), [examples/job-application](https://github.com/foldkit/foldkit/tree/main/examples/job-application)
   - Tier 7 → `client/app/board/submodel.ts`, `client/app/game/stream-subscription.ts`, or [packages/typing-game/client/src/page/room](https://github.com/foldkit/foldkit/tree/main/packages/typing-game/client/src/page/room)
5. **Foldkit UI integration**. If any `Ui.*` is imported, also read [examples/ui-showcase/src/main.ts](https://github.com/foldkit/foldkit/blob/main/examples/ui-showcase/src/main.ts) for the wiring pattern.
