...
## Cards & objects
- **Card** — a data-driven script (identity + rules behavior). Canonical **id** is Scryfall's oracle id; **name** is the printed name for display and search. The north star is to support any card faithfully (card-dsl-and-card-pool spec); the implemented pool grows from real cards, one at a time.
...
- **Effective power/toughness** — a creature's P/T computed on demand and additively (engine-core-and-event-model spec, no CR 613 layers): base + counters + until-EOT pumps + anthems + attachment grants. A **set-base** effect (Darksteel Mutation) replaces the printed base with a fixed value, then those additive modifiers still apply on top.

## Choices
- **Choice / pending choice** — a decision the engine pauses for. Represented as data; the awaited player answers with an intent, resuming the engine. See choices-actions-and-resolution spec. While a choice is pending, no other action is legal. Kinds include: **order triggers**, **choose target**, **may** / **pay cost** (optional abilities), **assign combat damage**, **arrange top** (scry/surveil), **search library**, **choose mode**, **discard**, **sacrifice edict**, and more — see `PendingChoice` in `crates/engine/src/types.rs`.
...
- **Auto-pass** — the server passing priority on a player's behalf through windows where they have no meaningful action, so priority advances without a manual Pass each time. The engine stays intent-only; the server just submits the passes (see turn-priority-and-stack spec). Distinct from **stack yield**.
...
- **Helpless dwell** — a seat with no meaningful response hovering the stack during a hold, which may postpone resolution up to a hard cap (turn-priority-and-stack spec).
- **Stack Pass** — one-shot `pass_priority` while you can act on a non-empty stack (Space/Enter and the stack **Pass** button).
- **Stack yield** — per-seat flag: the server auto-passes that seat for the rest of the current stack; clears when the stack empties. Armed once from the **priority context bar** (no cancel control there). Not the same as server **auto-pass** for helpless seats (turn-priority-and-stack spec) or **turn yield**.
- **Turn yield** — per-seat standing toggle: the server auto-passes this seat through every priority window until that seat becomes the **active player** again (start of their turn), until this seat is **attacked** (an attacker declared at them — not at another player), or until this seat takes an intentional action (cast, activate, manual **Stack Pass** / **Next**, or a non-empty combat declaration); then it clears. Empty “No attackers” / “No blockers” declarations are structural and do not clear it. Independent of **stack yield**. Distinct from server **auto-pass** for helpless seats. When armed **on your own turn**, the same flag is **End Turn** (turn-priority-and-stack spec).
- **End Turn** — Arena-style pass-the-turn: while you are active, arming **turn yield** auto-passes the rest of your turn (phases still open for opponents). Clears if another player casts/activates, when you act or cancel, or at the turn boundary (does not become until-my-turn). Opponents with empty-stack instant-speed plays still get priority windows, unless they have armed **turn yield** themselves — until-my-turn wins, and End Turn walks past them. Enter toggles it; **Next** / Space remain one priority pass.
- **Attack-response window** — after attackers are declared, each defending seat's Declare Attackers priority lists empty-stack instants as meaningful (turn-priority-and-stack spec) so auto-pass stops when they can respond before blockers; helpless defenders auto-pass through to blockers whether or not turn yield was armed.
...
- **Priority context bar** — the primary board control cluster for advancing or yielding priority: **Next** / combat confirms / **Stack Pass**, plus **stack yield**, **End Turn** / **turn yield**, and related context items. Always docked bottom-right. In paint order it sits above the **Stack view** (even when the stack is empty / that layer is vacant); prompt forms sit above this bar. Not stack-only chrome — empty-stack **Next** lives here too.
...
- **Lands row** — lands.
- **Row packing** — when a battlefield row has more permanents than fit at full spacing, that row alone compresses horizontal spacing so its cards stay inside the seat band (MTGA-style overlap / peek). Prefer a comfortable step, but keep compressing below it if needed — even to near-total overlap. Other rows are unaffected; the seat does not widen and cards do not spill into a neighbour.
- **Hover raise** — lifting the hovered **hand** tile above its neighbours for paint and hit-testing; does not change layout positions. Battlefield rows do not hover-raise — a buried permanent is reached by engagement splitting it out of its **permanent cluster**.
- **Permanent cluster** — several identical permanents in one battlefield row rendered as a single face with a count. Used only when the row would otherwise overflow full spacing; if the row still fits, every permanent stays its own card. Once the row overflows, every eligible identical group in that row collapses (not a minimum subset); if still over capacity, **row packing** compresses what’s left. Identity is “indistinguishable on what the table shows” except object identity, and only among permanents with no **attachment stack**. The face is the lowest-id member. A member **committed** to something — a declared or staged attacker, a declared or staged blocker, a blocked attacker, the target of a stack object, or a drafted target — never joins the cluster: it takes its own slot with its own arrow, ring, and hit box, and the face becomes the next free member. So each drop or pick reaches a distinct permanent, and a member rejoins when its commitment clears. Holding Shift on a combat drop commits every member at once. Activating an ability splits nothing — the **activation radial** offers each ability while any member can still use it, marked `×k`, and routes to one that can. Not a **Pile** (zone) and not an **Attachment stack**.
...
- **PlayMotion** — the client module that owns **play-committed** canvas motion (hand→battlefield / hand→stack / leave-stack flights, play entrance parking, play origins). Non-play glides (zone-sourced, creator-origin, opponent-from-avatar) stay on the table surface. See the flights and board-composition specs.
