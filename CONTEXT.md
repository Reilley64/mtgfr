...
- **Turn yield** — per-seat standing toggle: the server auto-passes this seat through every priority window until that seat becomes the **active player** again (start of their turn), until this seat is **attacked** (an attacker declared at them — not at another player), or until this seat takes an intentional action (cast, activate, manual **Stack Pass** / **Next**, etc.); then it clears. Being attacked also forces a Declare Attackers priority stop for that seat so they can cast instants before blockers. Independent of **stack yield**. Distinct from server **auto-pass** for helpless seats.
