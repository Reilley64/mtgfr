...
- **Turn yield** — per-seat standing toggle: the server auto-passes this seat through every priority window until that seat becomes the **active player** again (start of their turn); then it clears. Also clears if this seat takes an intentional action (cast, activate, manual **Stack Pass** / **Next**, etc.). Independent of **stack yield**. Distinct from server **auto-pass** for helpless seats.
